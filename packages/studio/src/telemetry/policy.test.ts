...
const DOCUMENTED_OPT_OUT = "smashcut-studio:telemetryDisabled";
const LEGACY_OPT_OUT = "sc-studio-telemetry-opt-out";
...
describe("browserTelemetryAllowed", () => {
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("VITE_SMASHCUT_NO_TELEMETRY", "");
  ...
  it.each(["1", "true", "yes", "on", " ON "])(
    "refuses when VITE_SMASHCUT_NO_TELEMETRY=%s",
    async (value) => {
      vi.stubEnv("VITE_SMASHCUT_NO_TELEMETRY", value);
      vi.resetModules();
      ({ browserTelemetryAllowed } = await import("./policy"));
      expect(browserTelemetryAllowed()).toBe(false);
    },
  ...
  it("ignores an unset or unrelated VITE_SMASHCUT_NO_TELEMETRY value", async () => {
    vi.stubEnv("VITE_SMASHCUT_NO_TELEMETRY", "0");
    vi.resetModules();
    ({ browserTelemetryAllowed } = await import("./policy"));
    expect(browserTelemetryAllowed()).toBe(true);
  });
