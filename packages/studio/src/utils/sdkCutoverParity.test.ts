...
async function applySdkDomCutover(source: string, ops: PatchOperation[]): Promise<string> {
  ...
  session.batch(() => {
    for (const op of patchOpsToSdkEditOps("sc-target", ops)) {
      session.dispatch(op);
    }
  });
...
const cases: Array<{ name: string; source: string; ops: PatchOperation[] }> = [
  {
    ...
    source: shell(
      '<div data-sc-id="sc-target" style="opacity: 0.5; inset: 0; transform-origin: 0 0">Old<span data-sc-id="sc-child">Child</span></div>',
    ),
    ops: [
  ...
  },
  {
    ...
    source: shell(
      '<div data-sc-id="sc-target" style="opacity: 0.5; inset: 0; transform-origin: 0 0">Old</div>',
    ),
  ...
  },
  {
    ...
    source: shell(
      '<div data-sc-id="sc-target" style="background: url(data:image/svg+xml;utf8,<svg></svg>); color: red; opacity: 0.5">Old</div>',
    ),
    ops: [{ type: "inline-style", property: "color", value: "blue" }],
  },
  {
    name: "sets direct text content",
    source: shell('<div data-sc-id="sc-target">Old</div>'),
    ops: [{ type: "text-content", property: "text", value: "New" }],
  },
  {
    name: "sets text on the single child target used by the legacy path",
    source: shell('<button data-sc-id="sc-target"><span data-sc-id="sc-child">Old</span></button>'),
    ops: [{ type: "text-content", property: "text", value: "New" }],
  },
  {
    name: "sets text on the single child target while preserving parent text",
    source: shell('<div data-sc-id="sc-target">Lead <span data-sc-id="sc-child">Old</span></div>'),
    ops: [{ type: "text-content", property: "text", value: "New" }],
  },
  {
    name: "sets data attributes through attribute ops",
    source: shell('<div data-sc-id="sc-target" data-old="1">Old</div>'),
    ops: [{ type: "attribute", property: "mode", value: "hero" }],
  },
  {
    name: "removes data attributes through attribute ops",
    source: shell('<div data-sc-id="sc-target" data-mode="hero">Old</div>'),
    ops: [{ type: "attribute", property: "mode", value: null }],
  },
  {
    name: "sets allowed html attributes",
    source: shell('<a data-sc-id="sc-target" href="https://example.com">Old</a>'),
  ...
  },
  {
    ...
    source: shell(
      '<div data-sc-id="sc-target" style="top: 10px; right: 10px; bottom: 10px; left: 10px; opacity: 1">Old</div>',
    ),
    ops: [{ type: "inline-style", property: "inset", value: "0" }],
  },
  {
    name: "sets longhand over existing shorthand (top over inset)",
    source: shell('<div data-sc-id="sc-target" style="inset: 0; opacity: 1">Old</div>'),
    ops: [{ type: "inline-style", property: "top", value: "10px" }],
  },
  {
    name: "mixed-type batch: inline-style + text-content in one op list",
    source: shell('<div data-sc-id="sc-target" style="color: red">Old</div>'),
    ops: [
  ...
  },
  {
    ...
    source: shell('<div data-sc-id="sc-target" style="opacity: 0.5" data-mode="default">Old</div>'),
    ops: [
...
describe("SDK cutover DOM serialization parity", () => {
  it.each(cases)("$name", async ({ source, ops }) => {
    const legacy = patchElementInHtml(source, { hfId: "sc-target" }, ops);
