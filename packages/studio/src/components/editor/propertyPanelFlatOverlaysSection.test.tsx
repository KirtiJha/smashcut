...
function registryItem(
  name: string,
  tags: string[],
  type: RegistryItem["type"] = "smashcut:component",
  ...
  const base = {
    ...
    tags,
    preview,
    files: [{ path: `${name}.html`, target: `${name}.html`, type: "smashcut:snippet" as const }],
  };
  if (type === "smashcut:component") return { ...base, type };
  if (type === "smashcut:block") {
...
describe("FlatOverlaysSection", () => {
  ...
  it("loads tagged overlays and delegates installation by Registry name", async () => {
    const items = [
      registryItem("camcorder-hud", [MEDIA_TREATMENT_OVERLAY_TAG], "smashcut:block", {
