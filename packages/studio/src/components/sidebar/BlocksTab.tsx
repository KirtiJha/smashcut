...
function buildAgentPrompt(
  ...
  const isComponent = blockType === "smashcut:component";
  const kind = isComponent ? "component" : "block";
  const compositionInfo = formatCompositionContext(context);

  const categoryPrompts: Record<string, string> = {
    captions: [
      `Using /smashcut, add the "${title}" caption style (registry: ${name}) to my composition.`,
      `${description}`,
    ...
    vfx: [
      `Using /smashcut, add the "${title}" VFX (registry: ${name}) as a full-screen overlay on my composition.`,
      `${description}`,
    ...
    transitions: [
      `Using /smashcut, add the "${title}" transition (registry: ${name}) between my scenes.`,
      `${description}`,
    ...
    effects: [
      `Using /smashcut, add the "${title}" effect (registry: ${name}) as an overlay on my composition.`,
      `${description}`,
    ...
    social: [
      `Using /smashcut, add the "${title}" template (registry: ${name}) to my composition.`,
      `${description}`,
    ...
    data: [
      `Using /smashcut, add the "${title}" visualization (registry: ${name}) to my composition.`,
      `${description}`,
    ...
    scenes: [
      `Using /smashcut, add the "${title}" scene (registry: ${name}) to my composition.`,
      `${description}`,
  ...
  const instruction =
    ...
    [
      `Using /smashcut, add the "${title}" ${kind} (registry: ${name}) to my composition.`,
      `${description}`,
