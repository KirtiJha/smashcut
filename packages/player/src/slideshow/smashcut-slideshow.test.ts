...
// packages/player/src/smashcut-player.test.ts.)

describe("<smashcut-slideshow>", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  function makeEl(opts: {
    ...
    const el = document.createElement("smashcut-slideshow") as any;
  ...
  it("is registered as a custom element", () => {
    expect(customElements.get("smashcut-slideshow")).toBeDefined();
  });
  ...
  it("renders prev/next buttons and counter in a single nav cluster after controller injection", () => {
    ...
    const cluster = el.querySelector("[data-sc-nav-cluster]");
    expect(cluster).toBeTruthy();
    expect(el.querySelector("[data-sc-prev]")).toBeTruthy();
    expect(el.querySelector("[data-sc-next]")).toBeTruthy();
    // No breadcrumb, no separate back button in chrome
    expect(el.querySelector("[data-sc-breadcrumb-item]")).toBeNull();
    expect(el.querySelector("[data-sc-back]")).toBeNull();
    el.remove();
  });
  ...
  it("renders an icon-only present button inside the nav cluster in normal mode", () => {
    const el = makeEl({ index: 1, total: 3 });
    const cluster = el.querySelector("[data-sc-nav-cluster]");
    const presentBtn = el.querySelector("[data-sc-present]");
    expect(cluster).toBeTruthy();
    ...
    expect(presentBtn?.getAttribute("data-sc-tooltip")).toBe("Present");
    el.remove();
  });
  ...
  it("does not render the present button in audience mode", () => {
    ...
    expect(el.querySelector("[data-sc-present]")).toBeNull();
    expect(el.querySelector("[data-sc-fullscreen]")).toBeTruthy();
    el.remove();
  });

  it("renders counter text", () => {
    const el = makeEl({ index: 2, total: 5 });
    const counter = el.querySelector("[data-sc-counter]");
  ...
  it("adds hover tooltips to all nav control buttons", () => {
    ...
    const expected: [string, string][] = [
      ["[data-sc-mute]", "Mute"],
      ["[data-sc-prev]", "Previous slide"],
      ["[data-sc-next]", "Next slide"],
      ["[data-sc-present]", "Present"],
      ["[data-sc-fullscreen]", "Full screen"],
    ];
    for (const [selector, label] of expected) {
      ...
      expect(button?.getAttribute("data-sc-tooltip")).toBe(label);
    }
    el.remove();
  });
  ...
  it("does NOT render a mute button when the `sound` attribute is absent", () => {
    ...
    expect(el.querySelector("[data-sc-mute]")).toBeNull();
    el.remove();
  });
  ...
  it("renders a mute button inside the nav capsule when `sound` attribute is present", () => {
    ...
    const cluster = el.querySelector("[data-sc-nav-cluster]");
    const muteBtn = el.querySelector("[data-sc-mute]");
    expect(cluster).toBeTruthy();
  ...
  it("mute button click toggles `muted` getter, sets data-sc-muted, and dispatches sc-sound event", () => {
    ...
    el.addEventListener("sc-sound", (e: Event) => {
      events.push((e as CustomEvent<{ muted: boolean }>).detail);
    });

    expect((el as any).muted).toBe(false);
    expect(el.hasAttribute("data-sc-muted")).toBe(false);

    // Click once — mute
    const muteBtn = el.querySelector("[data-sc-mute]") as HTMLElement;
    expect(muteBtn).toBeTruthy();
    muteBtn.click();

    expect((el as any).muted).toBe(true);
    expect(el.hasAttribute("data-sc-muted")).toBe(true);
    expect(events).toHaveLength(1);
    expect(events[0]).toEqual({ muted: true });

    // Click again — unmute
    const muteBtnAfter = el.querySelector("[data-sc-mute]") as HTMLElement;
    muteBtnAfter.click();
  ...
  it("mute and stopMedia affect only media owned by that slideshow", () => {
    ...
    const attachPlayerMedia = (slideshow: HTMLElement, id: string) => {
      const player = document.createElement("smashcut-player") as any;
      player.id = id;
      player.muted = false;
    ...
    const slideshowA = document.createElement("smashcut-slideshow") as any;
    const slideshowB = document.createElement("smashcut-slideshow") as any;
    ...
    const muteBtn = slideshowA.querySelector("[data-sc-mute]") as HTMLElement;
    muteBtn.click();
    ...
    const muteBtnAfter = slideshowA.querySelector("[data-sc-mute]") as HTMLElement;
    muteBtnAfter.click();
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> presenter mode", () => {
  beforeEach(async () => {
    localStorage.clear();
    await import("./smashcut-slideshow.js");
  });
  ...
  function makeMediaSyncedEl(mode: "presenter" | "audience", opts: { playRejects?: boolean } = {}) {
    ...
    const player = document.createElement("smashcut-player");
  ...
  it("present() opens an audience TAB with noopener/noreferrer", () => {
    ...
    expect(el.getAttribute("data-sc-presenting")).toBe("true");
  ...
  it("present() aborts (no presenter state) without user activation", () => {
    ...
    try {
      ...
      expect(el.getAttribute("data-sc-presenting")).toBeNull();
    } finally {
  ...
  it("built-in nav present button opens presenter mode and then hides itself", () => {
    ...
    const { el } = makePresenterEl();
    const presentBtn = el.querySelector("[data-sc-present]") as HTMLButtonElement;
    expect(presentBtn).toBeTruthy();
  ...
  it("presenter chrome contains current slide notes when present", () => {
    ...
    const text = el.querySelector("[data-sc-chrome]").textContent as string;
  ...
  it("presenter counters use the shared sans-serif number style", () => {
    ...
    const counter = el.querySelector("[data-sc-presenter-counter]");
    const elapsed = el.querySelector("[data-sc-presenter-elapsed]");
  ...
  it("presenter notes are editable and reload from localStorage", () => {
    ...
    const notes = el.querySelector("[data-sc-presenter-notes]");
    ...
    const storageKey = textarea.getAttribute("data-sc-presenter-notes-key");
    ...
    const reloadedNotes = reloaded.querySelector("[data-sc-presenter-notes]");
...
describe("<smashcut-slideshow> deferred init (Bug 1)", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("connectedCallback defers init to a macrotask so parser-appended children are found", async () => {
    ...
    const el = document.createElement("smashcut-slideshow") as HTMLElement & {
  ...
  it("does NOT call init if element is disconnected before the deferred init fires", async () => {
    ...
    expect(el.querySelector("[data-sc-chrome]")).toBeNull();
  });
  ...
  it("renders initial nav chrome from the manifest before scene metadata arrives", async () => {
    ...
    const fakePlayer = document.createElement("smashcut-player");
    ...
    const chrome = el.querySelector("[data-sc-chrome]");
    const counter = el.querySelector("[data-sc-counter]");
    expect(chrome).toBeTruthy();
    expect(counter?.textContent).toContain("1");
    expect(counter?.textContent).toContain("2");
    expect(el.querySelector("[data-sc-prev]")).toBeNull();
    expect(el.querySelector("[data-sc-next]")).toBeNull();
    const loading = el.querySelector("[data-sc-nav-loading]");
    expect(loading).toBeTruthy();
    expect(loading?.getAttribute("aria-label")).toBe("Loading slides");
    expect(el.querySelector("[data-sc-present]")).toBeTruthy();
    expect(el.querySelector("[data-sc-fullscreen]")).toBeTruthy();

    await vi.advanceTimersByTimeAsync(2600);

    expect(el.querySelector("[data-sc-nav-loading]")).toBeNull();
    expect(el.querySelector("[data-sc-next]")).toBeTruthy();
  ...
  describe("retry at DOMContentLoaded when init ran before the children were parsed", () => {
    ...
    it("binds once DOMContentLoaded fires with the children present", async () => {
      const el = document.createElement("smashcut-slideshow");
      document.body.appendChild(el);
      ...
      expect(el.querySelector("[data-sc-chrome]")).toBeTruthy();
      expect(el.querySelector("[data-sc-counter]")?.textContent).toContain("1");
    ...
    it("retries when the island was only partly streamed in", async () => {
      ...
      el.insertAdjacentHTML(
        "beforeend",
        `<script type="application/smashcut-slideshow+json">{ "slides": [ { "sceneId": "in</script>`,
      );
      document.body.appendChild(el);
...
describe("slideshow parts (pure)", () => {
  const island = `<script type="application/smashcut-slideshow+json">
    { "slides": [ { "sceneId": "intro", "startTime": 0, "endTime": 1 } ] }
  </script>`;
  ...
  it("locateSlideshowParts reports what is missing, in the order init needs it", () => {
    ...
    expect(locateSlideshowParts(subtree("<smashcut-player></smashcut-player>"))).toEqual({
      kind: "incomplete",
      reason: "no-island",
    });
    expect(
      locateSlideshowParts(
        subtree(
          `<smashcut-player></smashcut-player>` +
            `<script type="application/smashcut-slideshow+json">{ "slides": [ {</script>`,
        ),
      ),
  ...
  it("locateSlideshowParts is ready when both the player and the island are there", () => {
    const root = subtree(`<smashcut-player></smashcut-player>${island}`);
    const parts = locateSlideshowParts(root);
    expect(parts.kind).toBe("ready");
    if (parts.kind !== "ready") return;
    expect(parts.player).toBe(root.querySelector("smashcut-player"));
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix 1 — hotspot id/label XSS escape", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("hotspot id containing double-quote does not break out of the attribute", () => {
    ...
    const chrome = el.querySelector("[data-sc-chrome]") as HTMLElement;
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix 2 — presenter text XSS escape", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("a slide note containing HTML renders as inert text (no element node created)", () => {
    ...
    el.setAttribute("data-sc-presenting", "true");
    (el as any).render?.();
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix 3 — controller dispose on lifecycle", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix 5 — waitForReady timeout", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix 6 — keydown form-control guard", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix 7 — audience mode ignores window postMessage nav", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix #1 — chrome re-appended on reconnect", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("after disconnect+reconnect chrome is re-appended (not left detached)", () => {
    ...
    const firstChrome = el.querySelector("[data-sc-chrome]");
    ...
    const newChrome = el.querySelector("[data-sc-chrome]");
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix #14 — controller and offChange nulled on disconnect", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix #2 — malformed island try/finally", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("malformed parseSlideshowManifest does not leave initInFlight stuck (second init can run)", async () => {
    ...
    el.innerHTML = '<script type="application/json+sc-slideshow">{ NOT VALID JSON }</script>';
    el.appendChild(fakePlayer);
    document.body.appendChild(el);
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix #4/#6/#9 — epoch counter cancels stale init", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
...
describe("dropInvalidSlides — phantom slide filtering", () => {
  ...
  ): import("@smashcut/core/slideshow").ResolvedSlide {
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> conditional prev/next buttons", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("last slide of main deck (canNext=false): next button absent, prev button present", () => {
    ...
    expect(el.querySelector("[data-sc-next]")).toBeNull();
    expect(el.querySelector("[data-sc-prev]")).toBeTruthy();
    expect(el.querySelector("[data-sc-counter]")?.textContent).toContain("11");
    el.remove();
  });
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> Fix 4 — back affordance (postMessage only; chrome breadcrumb/back removed)", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("nav cluster (prev/counter/next) is present regardless of branch depth", () => {
    const el = makeElWithBreadcrumb({ breadcrumbLength: 2 });
    expect(el.querySelector("[data-sc-nav-cluster]")).toBeTruthy();
    expect(el.querySelector("[data-sc-prev]")).toBeTruthy();
    expect(el.querySelector("[data-sc-next]")).toBeTruthy();
    expect(el.querySelector("[data-sc-counter]")).toBeTruthy();
    el.remove();
  });
...
// ---------------------------------------------------------------------------
// Mechanical `interactive` attribute on inner <smashcut-player>
// ---------------------------------------------------------------------------
// The slideshow auto-applies the `interactive` attribute to every inner
// <smashcut-player>, so clickable controls, links, native media controls,
...
// ---------------------------------------------------------------------------
describe("<smashcut-slideshow> auto-sets `interactive` on inner <smashcut-player>", () => {
  beforeEach(async () => {
    await import("./smashcut-slideshow.js");
  });
  ...
  it("inner <smashcut-player> gets `interactive` attribute after mount", async () => {
    const el = document.createElement("smashcut-slideshow");
    const player = document.createElement("smashcut-player");
    el.appendChild(player);
    document.body.appendChild(el);
  ...
  it("dynamically-inserted <smashcut-player> children also get `interactive`", async () => {
    const el = document.createElement("smashcut-slideshow");
    document.body.appendChild(el);
