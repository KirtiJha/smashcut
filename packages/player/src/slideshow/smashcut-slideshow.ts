...
export function locateSlideshowParts(root: Element): SlideshowParts {
  const player = root.querySelector("smashcut-player");
...
const PRESENTER_NOTES_STORAGE_PREFIX = "sc-slideshow:presenter-notes:v1:";
...
function injectKeyframesOnce(): void {
  ...
  style.textContent = `
    ...
    @keyframes sc-nav-spin {
      to { transform: rotate(360deg); }
    }
    @media (prefers-reduced-motion: reduce) {
      .sc-hotspot-pill,
      .sc-nav-spinner { animation: none !important; }
    }
    /* Nav-button hover (replaces inline onmouseover/onmouseout — CSP-safe).
       !important beats the inline base color set on each button. */
    [data-sc-nav-cluster] button:hover {
      background: rgba(255,255,255,0.12) !important;
      color: #fff !important;
    }
    [data-sc-nav-cluster] button[data-sc-tooltip] {
      position: relative;
    }
    [data-sc-nav-cluster] button[data-sc-tooltip]::before,
    [data-sc-nav-cluster] button[data-sc-tooltip]::after {
      position: absolute;
      left: 50%;
      opacity: 0;
      pointer-events: none;
    ...
    [data-sc-nav-cluster] button[data-sc-tooltip]:hover::before,
    [data-sc-nav-cluster] button[data-sc-tooltip]:hover::after,
    [data-sc-nav-cluster] button[data-sc-tooltip]:focus-visible::before,
    [data-sc-nav-cluster] button[data-sc-tooltip]:focus-visible::after {
      opacity: 1;
    ...
    [data-sc-muted] [data-sc-mute]:hover {
...
export class SmashcutSlideshow extends HTMLElement {
  ...
  /** Whether audio is currently muted. Reflects `data-sc-muted` attribute. */
  get muted(): boolean {
    return this._muted;
  }
  ...
  connectedCallback(): void {
    ...
    // parsed before we query for <smashcut-player>. This matters when the
    // bundle is loaded synchronously (e.g. <script src> in <head>), where
    // connectedCallback fires while the parser is still inside the
    // <smashcut-slideshow> open tag — before its children exist. A microtask
   ...
   */
  present(): void {
    if (this.resolveMode() === "audience" || this.getAttribute("data-sc-presenting") === "true") {
      return;
    }
    ...
    if (!this.openAudienceTab(url.href)) {
      console.warn("[smashcut-slideshow] present(): browser blocked the audience tab");
      return;
    }
    this.setAttribute("data-sc-presenting", "true");
  ...
  private updateElapsed(): void {
    ...
    const el = this.chrome?.querySelector("[data-sc-presenter-elapsed]");
    if (el) {
  ...
  private async init(): Promise<void> {
    ...
    try {
      ...
      if (errors.length > 0) {
        console.warn("[smashcut-slideshow] manifest errors:", errors);
      }
      const cleaned = dropInvalidSlides(resolved);
      if (cleaned.slides.length === 0 && manifest.slides.length > 0) {
        console.error(
          "[smashcut-slideshow] no main-line slides resolved — the scene timeline may not have loaded in time, or sceneIds/timing are invalid:",
          errors,
        );
      }
  ...
  private mediaPlayerElements(): (Partial<PlayerElement> & HTMLElement)[] {
    return Array.from(this.querySelectorAll("smashcut-player")).filter(
      (player): player is Partial<PlayerElement> & HTMLElement => player instanceof HTMLElement,
    );
  }

  /**
   * Inner `<smashcut-player>` instances inside a slideshow need the
  ...
  private ensureInteractivePlayers(): void {
    for (const player of this.querySelectorAll("smashcut-player")) {
  ...
  /**
   * Watch for `<smashcut-player>` children added after the initial mount
  ...
  private warnIframeKeyForwardingUnavailable(): void {
    ...
    console.warn(
      "[smashcut-slideshow] iframe keyboard forwarding is unavailable for this composition, likely because the player iframe is cross-origin. Arrow shortcuts work when focus is outside the iframe.",
    );
  }
  ...
  private onKey = (e: KeyboardEvent): void => {
    ...
    const multiInstance = document.querySelectorAll("smashcut-slideshow").length > 1;
  ...
  private render(): void {
    ...
    if (this.getAttribute("data-sc-presenting") === "true") {
    ...
    const hotspotsHtml = currentSlide.hotspots
      .map((h) => {
        const posStyle = h.region
          ...
          : "right:5%;bottom:18%;";
        return `<button
          class="sc-hotspot-pill"
          data-hotspot-id="${escHtml(h.id)}"
          data-hotspot-target="${escHtml(h.target)}"
          type="button"
          style="position:absolute;${posStyle}display:inline-flex;align-items:center;gap:6px;padding:8px 14px;background:var(--sc-slideshow-accent,rgba(255,255,255,0.92));color:#111;border:none;border-radius:999px;font-size:13px;font-weight:600;letter-spacing:0.01em;cursor:pointer;pointer-events:auto;box-shadow:0 4px 16px rgba(0,0,0,0.35);animation:sc-hotspot-pulse 1.8s ease-in-out infinite;white-space:nowrap;"
  ...
  private paintChrome(html: string): void {
    ...
    if (!this.chrome) {
      ...
      this.chrome.setAttribute("data-sc-chrome", "");
  ...
  private buildNavCluster(
    ...
    const muteBtnHtml = showSound
      ? `<button
          ...
          data-sc-tooltip="${this._muted ? "Unmute" : "Mute"}"
    ...
    const prevBtnHtml = showPrev
      ? `<button
          ...
          data-sc-tooltip="Previous slide"
    ...
    const loadingHtml = showLoading
      ? `<span
        ...
        ><span class="sc-nav-spinner" aria-hidden="true" style="width:14px;height:14px;border:2px solid rgba(255,255,255,0.32);border-top-color:rgba(255,255,255,0.92);border-radius:999px;animation:sc-nav-spin 0.8s linear infinite;"></span></span>`
      : "";
  ...
  private wireChromeButtons(): void {
    ...
    if (!chrome) return;
    this.wireChromeClick(chrome, "[data-sc-mute]", () => this.toggleMute());
    this.wireChromeClick(chrome, "[data-sc-prev]", () => this.controller?.prev());
    this.wireChromeClick(chrome, "[data-sc-next]", () => this.controller?.next());
    this.wireChromeClick(chrome, "[data-sc-present]", () => this.present());
    this.wireChromeClick(chrome, "[data-sc-fullscreen]", () => this.toggleFullscreen());
  ...
  private wirePresenterNotes(chrome: HTMLDivElement): void {
    const notesInput = chrome.querySelector("[data-sc-presenter-notes]");
    if (notesInput instanceof HTMLTextAreaElement) {
      const key = notesInput.getAttribute("data-sc-presenter-notes-key");
  ...
  private onFsChange = (): void => {
    ...
    const btn = this.chrome?.querySelector("[data-sc-fullscreen]");
    if (!btn) return;
    const isFs = document.fullscreenElement === this;
    ...
    btn.setAttribute("data-sc-tooltip", label);
  ...
  private shouldShowPresentControl(): boolean {
    return this.resolveMode() !== "audience" && this.getAttribute("data-sc-presenting") !== "true";
  }

  private toggleMute(): void {
    this._muted = !this._muted;
    if (this._muted) {
      this.setAttribute("data-sc-muted", "");
    } else {
      this.removeAttribute("data-sc-muted");
    }
    this.applyGlobalMute(this._muted);
    this.dispatchEvent(
      new CustomEvent("sc-sound", {
  ...
  private stepAutoplay(safeId: string, state: AutoplayPollState): boolean {
    ...
    void video.play().catch((err: unknown) => {
      ...
      if (name !== "AbortError" && name !== "NotAllowedError" && !state.warned) {
        state.warned = true;
        console.warn("[smashcut-slideshow] autoplay play() failed:", err);
      }
    });
    return false;
  }
  ...
  private presenterNotesDeckKey(): string {
    ...
    const playerSrc = this.querySelector("smashcut-player")?.getAttribute("src") ?? "";
  ...
  private renderPresenter(): void {
    ...
    const playerEl = this.querySelector("smashcut-player");
...
if (!customElements.get("smashcut-slideshow")) {
  customElements.define("smashcut-slideshow", SmashcutSlideshow);
}
