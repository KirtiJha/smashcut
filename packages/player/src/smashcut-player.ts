...
class SmashcutPlayer extends HTMLElement {
   ...
   * `<smashcut-player>` element itself returns `null` (Shadow DOM).
   */
  ...
  setRuntimeData(channel: string, payload: unknown): void {
    ...
    if (typeof structuredClone !== "function") {
      throw new Error(
        "SmashCut runtime data requires structuredClone support; refusing an unverified payload",
      );
    }
  ...
  private _isSlideshowPlayer(): boolean {
    return this.closest("smashcut-slideshow") !== null;
  }
  ...
  private _trySetRuntimeDataDirect(channel: string, payload: unknown, requestId: number): boolean {
    try {
      const bridge = (
        this.iframe.contentWindow as (Window & { __smashcut?: RuntimeDataBridge }) | null
      )?.__smashcut;
  ...
  private _rescale() {
    ...
    if (!applied && this._ready && !this._rescaleWarned) {
      ...
      console.warn("[smashcut-player] rescale no-op after ready — zero-size player element", {
...
if (!customElements.get("smashcut-player")) {
  customElements.define("smashcut-player", SmashcutPlayer);
}

export { SmashcutPlayer };
