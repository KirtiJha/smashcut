...
describe("SmashcutPlayer parent-frame media", () => {
  ...
  beforeEach(async () => {
    await import("./smashcut-player.js");
    ...
    player = document.createElement("smashcut-player") as PlayerElement;
  });
  ...
  it("does not mute iframe media on autoplay fallback inside presenter slideshow", () => {
    const slideshow = document.createElement("smashcut-slideshow");
...
describe("SmashcutPlayer shader transition options", () => {
  ...
  it("passes shader options through src query parameters", () => {
    const player = document.createElement("smashcut-player") as PlayerWithIframe;
    ...
    expect(url.searchParams.get("__sc_shader_capture_scale")).toBe("0.5");
    expect(url.searchParams.get("__sc_shader_loading")).toBe("player");
  ...
  it("injects shader options into srcdoc before composition scripts run", () => {
    ...
    expect(srcdoc.indexOf("data-smashcut-player-shader-options")).toBeLessThan(
...
describe("SmashcutPlayer adoptedStyleSheets", () => {
  ...
  it("shares a single CSSStyleSheet across multiple player instances", () => {
    const a = document.createElement("smashcut-player") as PlayerWithShadow;
    const b = document.createElement("smashcut-player") as PlayerWithShadow;
  ...
  it("does not inject a per-instance <style> when adoption succeeds", () => {
    const player = document.createElement("smashcut-player") as PlayerWithShadow;
    document.body.appendChild(player);
...
describe("SmashcutPlayer media MutationObserver scoping", () => {
  ...
  it("attaches the observer to each top-level composition host (not the body)", () => {
    ...
    const player = document.createElement("smashcut-player") as PlayerInternal;
    document.body.appendChild(player);
...
describe("SmashcutPlayer parent-proxy time-mirror coalescing", () => {
  type DriftEntry = {
  ...
  beforeEach(async () => {
    await import("./smashcut-player.js");
    player = document.createElement("smashcut-player") as PlayerInternal;
    document.body.appendChild(player);
  ...
  it("initializes new parent-media entries with driftSamples=0", () => {
    ...
    const fresh = document.createElement("smashcut-player") as PlayerInternal;
...
describe("SmashcutPlayer seek() sync path", () => {
  type SyncPlayerStub = {
    seek?: (t: number) => void;
    play?: () => void;
    pause?: () => void;
...
describe("SmashcutPlayer loop end-state handling", () => {
  type PlayerInternal = HTMLElement & {
    iframe: HTMLIFrameElement;
    play: () => void;
...
describe("SmashcutPlayer srcdoc attribute", () => {
  type PlayerInternal = HTMLElement & {
    iframe: HTMLIFrameElement;
    _ready: boolean;
  };
  ...
  it("includes srcdoc in observedAttributes", () => {
    ...
    const ctor = customElements.get("smashcut-player") as
  ...
  it("uses a configured runtime source for loopback srcdoc", () => {
    ...
    player.setAttribute("runtime-src", "http://127.0.0.1:8900/smashcut.runtime.iife.js");
    ...
    expect(player.iframe.getAttribute("srcdoc")).toContain(
      '<script src="http://127.0.0.1:8900/smashcut.runtime.iife.js"></script>',
    );
  ...
  it("falls back to the pinned runtime for a foreign-origin runtime source", () => {
    ...
    const player = document.createElement("smashcut-player") as PlayerInternal;
    player.setAttribute("runtime-src", "https://evil.example.com/smashcut.runtime.iife.js");
    player.setAttribute("srcdoc", "<!doctype html><html><head></head><body></body></html>");
    document.body.appendChild(player);

    const srcdoc = player.iframe.getAttribute("srcdoc") ?? "";
    expect(srcdoc).not.toContain("evil.example.com");
    expect(srcdoc).toContain("smashcut.runtime.iife.js");
  ...
  it("forwards an initial srcdoc attribute to the iframe on connect", () => {
    ...
    expect(player.iframe.getAttribute("srcdoc")).toContain("smashcut.runtime.iife.js");
...
describe("SmashcutPlayer volume and mute", () => {
  ...
  beforeEach(async () => {
    ...
    player = document.createElement("smashcut-player") as typeof player;
  });
...
describe("SmashcutPlayer audio lock", () => {
...
describe("SmashcutPlayer runtime ready handshake", () => {
  ...
  function readyMessage() {
    return new MessageEvent("message", {
      source: frameWindow,
      data: { source: "sc-preview", type: "ready" },
    });
  }
...
describe("SmashcutPlayer audio lock — Claude desktop UA fallback", () => {
...
describe("SmashcutPlayer playback rate", () => {
  let player: HTMLElement & {
    playbackRate: number;
    iframeElement: HTMLIFrameElement;
  };
...
describe("SmashcutPlayer composition dimension attributes", () => {
  ...
  beforeEach(async () => {
    await import("./smashcut-player.js");
    player = document.createElement("smashcut-player") as PlayerWithDimensions;
    document.body.appendChild(player);
  });
...
describe("SmashcutPlayer retained runtime data", () => {
  ...
  beforeEach(async () => {
    await import("./smashcut-player.js");
    player = document.createElement("smashcut-player") as RuntimeDataPlayer;
  ...
  it("uses the same-origin registry directly and falls back to postMessage otherwise", () => {
    const direct = vi.fn();
    (window as Window & { __smashcut?: unknown }).__smashcut = {
