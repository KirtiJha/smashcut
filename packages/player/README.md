...
Embeddable web component for playing SmashCut compositions. Zero dependencies, works with any framework.

## Install

```bash
npm install @smashcut/player
```

Or load directly via CDN:

```html
<script type="module" src="https://cdn.jsdelivr.net/npm/@smashcut/player"></script>
```

If you need a classic `<script>` tag instead of ESM, use the explicit global build:

```html
<script src="https://cdn.jsdelivr.net/npm/@smashcut/player/dist/smashcut-player.global.js"></script>
```

## Usage

```html
<smashcut-player src="./my-composition/index.html" controls></smashcut-player>
```
...
```typescript
import "@smashcut/player";

// The custom element is now registered — use it in your markup
// React: <smashcut-player src="..." controls />
// Vue:   <smashcut-player :src="url" controls />
```
...
When a composition uses `@smashcut/shader-transitions`, the player can own preview-only shader capture settings:
...
This is the canonical way to bridge the player into tools like [`@smashcut/studio`](../studio). The studio exports a `resolveIframe` helper that works with both iframe refs and web-component refs:

```ts
import { useTimelinePlayer, resolveIframe } from "@smashcut/studio";
...
The player renders compositions in a sandboxed `<iframe>` inside a Shadow DOM. It communicates with the SmashCut runtime via `postMessage`. If the composition has GSAP timelines (`window.__timelines`) but no runtime, the player auto-injects it from CDN.

## Distribution

| Format | File                           | Use case                       |
| ------ | ------------------------------ | ------------------------------ |
| ESM    | `smashcut-player.js`        | Bundlers (Vite, webpack, etc.) |
| CJS    | `smashcut-player.cjs`       | Node.js / require()            |
| IIFE   | `smashcut-player.global.js` | `<script>` tag, CDN            |
