...
AWS Lambda adapter for SmashCut distributed rendering. Ships three
things together:
...
3. An **`aws-cdk-lib` L2 construct** (`SmashcutRenderStack`) that
...
│   handler.mjs                                                    │
│     ├─ Action="plan"        → @smashcut/producer/distributed  │
│     ├─ Action="renderChunk" → @smashcut/producer/distributed  │
│     └─ Action="assemble"    → @smashcut/producer/distributed  │
...
The handler reads `SMASHCUT_LAMBDA_CHROME_SOURCE` at boot. The build
...
SmashCut' renderer drives Chrome via the CDP
...
```ts
import { deploySite, getRenderProgress, renderToLambda } from "@smashcut/aws-lambda";

// One-time upload per project version.
const site = await deploySite({
  projectDir: "./my-composition",
  bucketName: "smashcut-render-bucket",
});
...
const handle = await renderToLambda({
  ...
  stateMachineArn: "arn:aws:states:us-east-1:123:stateMachine:smashcut-render",
  config: {
    fps: 30,
    width: 1920,
    height: 1080,
...
import { SmashcutRenderStack } from "@smashcut/aws-lambda/cdk";

const app = new App();
const stack = new Stack(app, "MyApp");
const render = new SmashcutRenderStack(stack, "Render", {
...
`@smashcut/aws-lambda/cdk`.

## What's still ahead

- `smashcut lambda` CLI (deploy / sites create / render / progress / destroy) — PR 6.5.
