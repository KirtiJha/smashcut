/**
 * Structural snapshot of {@link SmashcutRenderStack}.
 *
...
function doSynth(): {
  ...
  const zipDir = mkdtempSync(join(tmpdir(), "sc-cdk-snap-"));
  writeFileSync(join(zipDir, "handler.zip"), "fake zip bytes");
  const app = new App();
  const stack = new Stack(app, "TestStack");
  new SmashcutRenderStack(stack, "Render", { handlerZipPath: join(zipDir, "handler.zip") });
...
describe("SmashcutRenderStack — snapshot", () => {
