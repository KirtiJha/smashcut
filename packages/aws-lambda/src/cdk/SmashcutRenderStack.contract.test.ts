/**
 * Contract tests for {@link SmashcutRenderStack}.
 *
...
import { SmashcutRenderStack } from "./SmashcutRenderStack.js";
...
function synthFixture(): Template {
  const zipDir = mkdtempSync(join(tmpdir(), "sc-cdk-test-"));
  const zipPath = join(zipDir, "handler.zip");
  writeFileSync(zipPath, "fake zip bytes");
  const app = new App();
  const stack = new Stack(app, "TestStack");
  new SmashcutRenderStack(stack, "Render", { handlerZipPath: zipPath });
  return Template.fromStack(stack);
}

describe("SmashcutRenderStack — contract", () => {
  beforeAll(() => {
  ...
  it("honours reservedConcurrency when supplied", () => {
    ...
    const app = new App();
    const stack = new Stack(app, "TestStack");
    new SmashcutRenderStack(stack, "Render", {
