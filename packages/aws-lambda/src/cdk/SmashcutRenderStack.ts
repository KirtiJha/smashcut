/**
 * `SmashcutRenderStack` — aws-cdk-lib L2 construct that emits the same
 * topology as `examples/aws-lambda/template.yaml`.
 *
 * Adopters who embed SmashCut inside their own CDK app can extend this
 ...
 * `@smashcut/aws-lambda` don't pull the CDK tree at runtime.
 *
 * Drift from the SAM template is guarded by the snapshot test
 * (`SmashcutRenderStack.snapshot.test.ts`), which diffs the synthed
...
/** Construction-time props for {@link SmashcutRenderStack}. */
export interface SmashcutRenderStackProps {
  /** Name prefix applied to function / state-machine / alarm names. Default `"smashcut"`. */
  projectName?: string;
...
export class SmashcutRenderStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: SmashcutRenderStackProps = {}) {
    super(scope, id);

    const projectName = props.projectName ?? "smashcut";
    ...
    this.renderFunction = new lambda.Function(this, "RenderFunction", {
      ...
      environment: {
        ...
        TMPDIR: "/tmp",
        SMASHCUT_LAMBDA_CHROME_SOURCE: chromeSource,
        SMASHCUT_RENDER_BUCKET: this.bucket.bucketName,
      },
    });
 ...
 * `node_modules/@smashcut/aws-lambda/` install.
 */
