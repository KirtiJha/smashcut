...
import { FONT_ALIAS_MAP, resolveAliasDisplayName } from "@smashcut/core/fonts/aliases";
import {
  locateSystemFontVariants,
  SYSTEM_FONT_SIZE_LIMIT,
} from "@smashcut/core/fonts/system-locator";
...
// FONT_ALIASES derives from the shared alias map in @smashcut/core.
...
export { FONT_ALIAS_KEYS } from "@smashcut/core/fonts/aliases";
...
// warm process and reuse it across invocations. Honor SMASHCUT_FONT_CACHE_DIR
// as an explicit override for any environment.
function resolveFontCacheRoot(): string {
  if (process.env.SMASHCUT_FONT_CACHE_DIR) {
    return process.env.SMASHCUT_FONT_CACHE_DIR;
  }
  if (process.env.AWS_LAMBDA_FUNCTION_NAME) {
    lambdaFontCacheRoot ??= mkdtempSync(join(tmpdir(), "smashcut-fonts-"));
    return lambdaFontCacheRoot;
  }
  return join(homedir(), ".cache", "smashcut", "fonts");
}
...
function fontCacheDir(slug: string): string {
  ...
  if (!existsSync(dir)) {
    try {
      ...
      ephemeralFontCacheRoot ??= mkdtempSync(join(tmpdir(), "smashcut-fonts-"));
...
export async function injectDeterministicFontFaces(
  ...
  styleEl.setAttribute("data-smashcut-deterministic-fonts", "true");
