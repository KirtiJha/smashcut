...
beforeAll(() => {
  previousCacheDir = process.env.SMASHCUT_FONT_CACHE_DIR;
  testCacheDir = mkdtempSync(join(tmpdir(), "sc-font-retries-"));
  process.env.SMASHCUT_FONT_CACHE_DIR = testCacheDir;
});

afterAll(() => {
  if (previousCacheDir === undefined) delete process.env.SMASHCUT_FONT_CACHE_DIR;
  else process.env.SMASHCUT_FONT_CACHE_DIR = previousCacheDir;
...
describe("deterministic Google Fonts retries", () => {
  ...
  it("recovers when the Google Fonts CSS retry succeeds", async () => {
    ...
    expect(result).toContain("data-smashcut-deterministic-fonts");
  ...
  it("keeps fail-open mode at one attempt with no typed failure", async () => {
    ...
    expect(result).not.toContain("data-smashcut-deterministic-fonts");
    expect(calls).toBe(1);
  });
});
