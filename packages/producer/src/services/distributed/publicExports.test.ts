 ...
 *
 *   1. `import { plan, renderChunk, assemble } from "@smashcut/producer"`
 *      — the canonical package entry. Includes the three activity functions
 *      and their result types.
 *
 *   2. `import { plan, renderChunk, assemble } from "@smashcut/producer/distributed"`
 ...
 * `../../index.js` paths rather than `"@smashcut/producer"` because the
...
describe("@smashcut/producer/distributed (subpath)", () => {
  ...
  it("exports the plan protocol contract", () => {
    ...
    expect(distributedSubpath.PLAN_HASH_SCHEMA).toBe("smashcut-plan-hash-v1");
    ...
    expect(distributedSubpath.PLAN_V2_HASH_SCHEMA).toBe("smashcut-plan-manifest-hash-v2");
    ...
    expect(distributedSubpath.CURRENT_PLAN_PROTOCOL).toEqual({
      ...
      hashSchema: "smashcut-plan-hash-v1",
    });
...
describe("@smashcut/producer (main entry)", () => {
