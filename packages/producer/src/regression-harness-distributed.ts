 ...
 *     `smashcut render` CLI takes. This is what produced every existing
 *     `tests/<name>/output/output.mp4` golden baseline.
 *
 *   - `distributed-simulated` — calls `plan()` → `renderChunk()` per chunk
 *     → `assemble()` from `@smashcut/producer/distributed`. No Temporal
 ...
 *   - `lambda-local` — drives the OSS `@smashcut/aws-lambda` handler
...
export async function runDistributedSimulatedRender(
  ...
  const planResult = await plan(
    input.projectDir,
    {
      ...
      // composition that reads `window.__scVariables` diverges.
