...
// Snippet checks cannot prove what `window.__smashcut` ends up carrying.
...
const smashcut = (dom.window as unknown as { __smashcut?: Record<string, unknown> })
  .__smashcut;
assert(smashcut, "Runtime did not attach window.__smashcut");
...
for (const helper of requiredHelpers) {
  assert(
    typeof smashcut[helper] === "function",
    `window.__smashcut.${helper} is not attached`,
  );
}

const pretext = smashcut.pretext as Record<string, unknown> | undefined;
assert(pretext, "window.__smashcut.pretext is not attached");
...
for (const fn of requiredPretextFns) {
  assert(typeof pretext[fn] === "function", `window.__smashcut.pretext.${fn} is not attached`);
}
...
for (const forbidden of ["clearCache", "setLocale"]) {
  assert(
    ...
    `window.__smashcut.pretext.${forbidden} must not be exposed (mutates shared state)`,
  );
}

const scriptDir = resolve(fileURLToPath(new URL(".", import.meta.url)));
const manifestPath = resolve(scriptDir, "../dist/smashcut.manifest.json");
try {
