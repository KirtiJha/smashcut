...
function testCliJsonOutput() {
  const tempDir = mkdtempSync(path.join(tmpdir(), "sc-core-lint-script-"));
  try {
    const fixturePath = path.join(tempDir, "index.html");
    writeFileSync(fixturePath, VALID_COMPOSITION, "utf8");
    const stdout = execFileSync("bun", ["run", "check:smashcut-html", "--json", fixturePath], {
      cwd: ROOT,
      encoding: "utf8",
    });
...
async function main() {
  ...
  testCliJsonOutput();
  console.log("smashcut linter tests passed");
}
