...
import { SMASHCUT_RUNTIME_ARTIFACTS } from "../src/inline-scripts/smashcut";
...
assert(
  manifest.artifacts?.iife === SMASHCUT_RUNTIME_ARTIFACTS.iife,
  "Manifest iife artifact name is not strict expected value",
);
assert(
  manifest.artifacts?.esm === SMASHCUT_RUNTIME_ARTIFACTS.esm,
...
console.log(
  JSON.stringify({
    event: "smashcut_runtime_parity_verified",
