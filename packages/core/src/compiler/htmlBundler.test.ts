...
<html><head>
  ...
  it("produces a self-contained runtime script when no SMASHCUT_RUNTIME_URL is set", async () => {
...
<html><body>
  <div data-composition-id="root" data-width="320" data-height="180"></div>
    ...
    const previousUrl = process.env.SMASHCUT_RUNTIME_URL;
    delete process.env.SMASHCUT_RUNTIME_URL;
    let bundled: string;
    try {
      bundled = await bundleToSingleHtml(dir);
    } finally {
      if (previousUrl !== undefined) process.env.SMASHCUT_RUNTIME_URL = previousUrl;
    }
...
<html><head></head><body>
  <div data-composition-id="root" data-width="320" data-height="180"></div>
    ...
    const original = getSmashcutRuntimeScript();
