...
import { serializeGsapAnimations, keyframesToGsapAnimations } from "@smashcut/parsers";
...
export function generateSmashcutStyles(
  ...
  const googleFontsLink = googleFontsUrl
    ? `<link data-sc-fonts="true" rel="preconnect" href="https://fonts.googleapis.com">
  <link data-sc-fonts="true" rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link data-sc-fonts="true" href="${googleFontsUrl}" rel="stylesheet">`
    : "";
...
export function generateSmashcutHtml(
  elements: TimelineElement[],
  totalDuration: number,
  options: SerializeOptions = {},
): string {
  ...
  let googleFontsLink = "";
  if (includeStyles) {
    const styles = generateSmashcutStyles(sortedElements, resolution, customStyles);
    googleFontsLink = styles.googleFontsLink;
    styleTags = [
      styles.coreCss
        ? `  <style data-sc-core="true">
    ${styles.coreCss
...
function generateElementHtml(element: TimelineElement, keyframes?: Keyframe[]): string {
  const baseAttrs = [
    ...
    `data-sc-id="${escapeHtmlAttributeValue(String(element.id))}"`,
