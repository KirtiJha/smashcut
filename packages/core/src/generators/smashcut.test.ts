...
import { parseHtml } from "@smashcut/parsers";
...
describe("generateSmashcutHtml", () => {
  ...
  it("round-trips element attribute values without creating event handlers", () => {
    ...
    const doc = new DOMParser().parseFromString(generateSmashcutHtml([element], 1), "text/html");
    const video = doc.querySelector("video");
    for (const name of ["id", "data-sc-id", "data-name", "src"])
  ...
  it("preserves supported rich text while removing executable markup", () => {
    ...
    const html = generateSmashcutHtml([makeTextElement({ content })], 1);
  ...
  it.each([
    ...
    expect(() => generateSmashcutHtml([makeVideoElement({ src })], 1)).toThrow(
  ...
  it("keeps composition identifiers inside their attribute and round-trips entities", () => {
    ...
    const doc = new DOMParser().parseFromString(
      generateSmashcutHtml([], 1, { compositionId }),
      "text/html",
    );
  ...
  it("contains resolution values supplied by JavaScript callers inside their attribute", () => {
    ...
    const html = Reflect.apply(generateSmashcutHtml, undefined, [
      [],
      1,
  ...
  it("round-trips composition variable metadata through the public parser", () => {
    ...
    const html = generateSmashcutHtml([element], 1);
  ...
  it("generates valid HTML with proper data attributes", () => {
    ...
    const html = generateSmashcutHtml(elements, 5);
  ...
  it("includes GSAP CDN script tag when includeScripts is true", () => {
    const elements = [makeTextElement()];
    const html = generateSmashcutHtml(elements, 5, { includeScripts: true });
  ...
  it("sets resolution data attribute", () => {
    ...
    const landscapeHtml = generateSmashcutHtml(elements, 5, { resolution: "landscape" });
    expect(landscapeHtml).toContain('data-resolution="landscape"');

    const portraitHtml = generateSmashcutHtml(elements, 5, { resolution: "portrait" });
  ...
  it("generates video elements with proper tags", () => {
    ...
    const html = generateSmashcutHtml(elements, 10);
  ...
  it("includes custom compositionId", () => {
    const elements = [makeTextElement()];
    const html = generateSmashcutHtml(elements, 5, { compositionId: "test-comp-123" });
  ...
  it("includes style tags when includeStyles is true", () => {
    const elements = [makeTextElement()];
    const html = generateSmashcutHtml(elements, 5, { includeStyles: true });

    expect(html).toContain('<style data-sc-core="true">');
  });
  ...
  it("includes custom styles when provided", () => {
    ...
    expect(html).toContain('<style data-sc-custom="true">');
  ...
  it("serializes keyframes as data attributes", () => {
    ...
    const html = generateSmashcutHtml(elements, 5, { keyframes });
  ...
  it("serializes zoom keyframes on zoom container", () => {
    ...
    const html = generateSmashcutHtml(elements, 10, { stageZoomKeyframes });
...
describe("generateSmashcutStyles", () => {
  it("generates core CSS with stage dimensions for landscape", () => {
    const elements = [makeTextElement()];
    const { coreCss } = generateSmashcutStyles(elements, "landscape");
  ...
  it("generates core CSS with stage dimensions for portrait", () => {
    const elements = [makeTextElement()];
    const { coreCss } = generateSmashcutStyles(elements, "portrait");
  ...
  it("includes custom CSS when provided", () => {
    const elements = [makeTextElement()];
    const { customCss } = generateSmashcutStyles(
      elements,
      "landscape",
      ".custom { color: blue; }",
    );
  ...
  it("generates Google Fonts link for Inter (always included)", () => {
    const elements = [makeTextElement()];
    const { googleFontsLink } = generateSmashcutStyles(elements, "landscape");
