...
describe("stableClipId", () => {
  it("prefers id, falls back to data-sc-id, else null", () => {
    const withId = document.createElement("div");
    withId.id = "real";
    withId.setAttribute("data-sc-id", "sc-1");
    expect(stableClipId(withId)).toBe("real");

    const hfOnly = document.createElement("div");
    hfOnly.setAttribute("data-sc-id", "sc-2");
    expect(stableClipId(hfOnly)).toBe("sc-2");
...
describe("createClipTree", () => {
  ...
  // Regression: id-less children (root index.html uses data-sc-id, not id) must
  // get their data-sc-id as the node id — not a synthetic `__clip-N` — so the
  // tree aligns with __clipManifest (which also keys on data-sc-id) and inline
  // expansion can join parent↔child.
  it("ids id-less timed elements by data-sc-id and links them to their parent", () => {
    document.body.innerHTML = `
      <div data-composition-id="root" data-duration="10" data-start="0" id="root">
        <div data-start="0" data-duration="5" id="scene">
          <h1 data-start="0" data-sc-id="sc-headline" class="headline">Hi</h1>
        </div>
      </div>`;
    ...
    const child = scene!.children.find((n) => n.id === "sc-headline");
