...
describe("readMediaRenderId", () => {
  it("prefers the stamped render id over the author id", () => {
    expect(readMediaRenderId(videoWith({ id: "clip", [MEDIA_RENDER_ID_ATTR]: "clip__sc2" }))).toBe(
      "clip__sc2",
    );
  });
...
describe("renderFrameElementId", () => {
  ...
  it("wraps the render id in the injector's sibling id format", () => {
    ...
    expect(renderFrameElementId(videoWith({ id: "c", [MEDIA_RENDER_ID_ATTR]: "c__sc2" }))).toBe(
      "__render_frame_c__sc2__",
    );
  });
});
...
describe("findInjectedRenderFrame", () => {
  ...
  it("resolves each colliding video to its own frame, not the first one's", () => {
    ...
    document.body.innerHTML =
      ...
      `<video id="clip" ${MEDIA_RENDER_ID_ATTR}="clip__sc2"></video>` +
      `<img id="__render_frame_clip__sc2__" class="__render_frame__">`;
    ...
    expect(findInjectedRenderFrame(second!)?.id).toBe("__render_frame_clip__sc2__");
  });
