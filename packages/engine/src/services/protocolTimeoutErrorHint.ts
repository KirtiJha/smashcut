 ...
 * points at the SmashCut-specific knobs. Puppeteer's stock error text
 ...
 * SmashCut, or what the currently-applied effective value is — so field
...
export function augmentProtocolTimeoutError(err: unknown, effectiveTimeoutMs: number): Error {
  ...
  const augmented = new Error(
    `${err.message}\n\n` +
      `SmashCut effective protocolTimeout: ${effectiveTimeoutMs} ms.\n\n` +
