...
describe("resolveConfig", () => {
  ...
  describe("extraction cache env", () => {
    ...
    it("uses an explicit extract cache path from env", () => {
      setEnv("SMASHCUT_EXTRACT_CACHE_DIR", "/tmp/custom-sc-cache");

      expect(resolveConfig().extractCacheDir).toBe("/tmp/custom-sc-cache");
    });

    it("converts SMASHCUT_EXTRACT_CACHE_MAX_MB to bytes", () => {
      setEnv("SMASHCUT_EXTRACT_CACHE_MAX_MB", "512");
...
describe("resolveExtractCacheDir", () => {
  ...
  it("threads a positive env value through verbatim (source: env)", () => {
    const res = resolveExtractCacheDir({
      SMASHCUT_EXTRACT_CACHE_DIR: "D:/sc-cache",
    });
    expect(res.disabled).toBe(false);
    expect(res.source).toBe("env");
    if (!res.disabled) {
      expect(res.dir).toBe("D:/sc-cache");
      expect(res.rawValue).toBe("D:/sc-cache");
    }
  });
  ...
  it.each(EXTRACT_CACHE_DIR_DISABLED_ALIASES.flatMap((v) => [v, v.toUpperCase(), `  ${v}  `]))(
    ...
    (value) => {
      const res = resolveExtractCacheDir({ SMASHCUT_EXTRACT_CACHE_DIR: value });
