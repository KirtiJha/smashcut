...
// Moved from @smashcut/core/core.types in the parsers extraction refactor.
 ...
 * stage (`@smashcut/producer`) and `suggestMatchingPreset` here.
 *
 ...
 * compile stage (`@smashcut/producer`) for the canonical remap.
 */
 ...
 * every entrypoint that emits a `DistributedRenderConfig` (`smashcut
 * cloudrun render`, `smashcut lambda render` / `render-batch`, the local
