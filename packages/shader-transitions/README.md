# @smashcut/shader-transitions

WebGL shader transitions for SmashCut compositions. Renders GPU-accelerated scene-to-scene transitions using fragment shaders, driven by GSAP timelines.

## Install

```bash
npm install @smashcut/shader-transitions
```
...
```typescript
import { init } from "@smashcut/shader-transitions";
...
Browser preview capture scale and transition-prep loading UI ownership are controlled by `<smashcut-player>` (`shader-capture-scale`, `shader-loading`) instead of composition code. Direct non-player previews keep the built-in full-fidelity loading fallback.
...
```typescript
import { SHADER_NAMES } from "@smashcut/shader-transitions";
...
- [`@smashcut/core`](../core) -- types, parsers, runtime
