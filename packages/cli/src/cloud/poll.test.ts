...
import type { SmashcutRenderDetail } from "./_gen/types.js";

function makeDetail(overrides: Partial<SmashcutRenderDetail>): SmashcutRenderDetail {
  return {
    render_id: "hfr_test",
    status: "queued",
    format: "mp4",
...
function stubClient(details: SmashcutRenderDetail[]): SmashcutCloudClient {
  const stack = [...details];
  return {
    async getRender() {
