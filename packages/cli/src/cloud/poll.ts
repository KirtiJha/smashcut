/**
 * Poll a SmashCut render until it reaches a terminal state.
 *
...
import type { SmashcutCloudClient } from "./_gen/client.js";
import type { SmashcutRenderDetail, SmashcutRenderStatus } from "./_gen/types.js";

export interface PollOptions {
  intervalMs?: number;
  maxWaitMs?: number;
  /** Called once per tick with the latest render state. */
  onTick?: (detail: SmashcutRenderDetail, elapsedMs: number) => void;
...
const TERMINAL_STATUSES: ReadonlySet<SmashcutRenderStatus> = new Set(["completed", "failed"]);

export class PollTimeoutError extends Error {
  readonly lastDetail: SmashcutRenderDetail;
  constructor(lastDetail: SmashcutRenderDetail, elapsedMs: number) {
    super(
...
export function isTerminal(status: SmashcutRenderStatus): boolean {
  return TERMINAL_STATUSES.has(status);
}

/**
 * Poll `GET /v3/smashcut/renders/{id}` until status is `completed` or
