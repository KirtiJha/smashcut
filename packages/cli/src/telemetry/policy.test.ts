...
describe("telemetry policy", () => {
  ...
  it.each(["1", "true", "TRUE", " yes ", "on"])(
    ...
    async (value) => {
      process.env["SMASHCUT_NO_TELEMETRY"] = value;
      const { effectiveTelemetryStatus } = await loadPolicy();
      expect(effectiveTelemetryStatus(true).source).toBe("SMASHCUT_NO_TELEMETRY");
    },
  );
  ...
  it("reports SMASHCUT_NO_TELEMETRY as the effective source", async () => {
    process.env["SMASHCUT_NO_TELEMETRY"] = "true";
    const { effectiveTelemetryStatus } = await loadPolicy();
    expect(effectiveTelemetryStatus(true)).toEqual({
      enabled: false,
      source: "SMASHCUT_NO_TELEMETRY",
    });
  });
