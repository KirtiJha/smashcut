...
async function loadShouldTrack(env: { SMASHCUT_NO_TELEMETRY?: string; DO_NOT_TRACK?: string }) {
  vi.resetModules();
  ...
  vi.stubEnv("SMASHCUT_NO_TELEMETRY", env.SMASHCUT_NO_TELEMETRY ?? "");
...
describe("telemetry client policy", () => {
  ...
  it.each([
    ["SMASHCUT_NO_TELEMETRY", "true"],
    ["SMASHCUT_NO_TELEMETRY", " yes "],
    ["DO_NOT_TRACK", "TRUE"],
    ["DO_NOT_TRACK", "on"],
