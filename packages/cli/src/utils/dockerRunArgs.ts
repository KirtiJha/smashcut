/**
 * Build the argument array for `docker run` that invokes the Smashcut
 ...
 */
import { fpsToFfmpegArg, type Fps } from "@smashcut/core";
...
export interface DockerRenderOptions {
  /**
   * Frame rate as an exact rational; see `Fps` in @smashcut/core. The
 ...
 *
 * Honors `SMASHCUT_DOCKER_PLATFORM` as an escape hatch (typed loosely so
...
export function resolveDockerPlatform(
  ...
  const override = env.SMASHCUT_DOCKER_PLATFORM;
