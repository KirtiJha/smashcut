...
//
// It is skipped unless SMASHCUT_E2E_API_URL is set, and it requires the runner to be
// authenticated first (`smashcut auth login`, or a HEYGEN credential the auth store can
...
//
//   SMASHCUT_E2E_API_URL=https://api2.heygen.com \
//     bunx vitest run src/utils/publishProject.e2e.test.ts
const E2E_API_URL = process.env["SMASHCUT_E2E_API_URL"];
const describeE2E = E2E_API_URL ? describe : describe.skip;

describeE2E("publish stable-URL round trip (live server)", () => {
  const priorPublishBase = process.env["SMASHCUT_PUBLISHED_PROJECTS_API_URL"];

  beforeAll(() => {
    // Point the publish client at the E2E target for the duration of this suite.
    process.env["SMASHCUT_PUBLISHED_PROJECTS_API_URL"] = E2E_API_URL;
  });

  afterAll(() => {
    if (priorPublishBase === undefined)
      delete process.env["SMASHCUT_PUBLISHED_PROJECTS_API_URL"];
    else process.env["SMASHCUT_PUBLISHED_PROJECTS_API_URL"] = priorPublishBase;
  });

  async function fetchPublicProject(projectId: string): Promise<Record<string, unknown>> {
    const response = await fetch(
      `${getPublishApiBaseUrl()}/v1/smashcut/projects/${projectId}/public`,
    );
  ...
  it("two identities in a shared team space converge on one URL, and other spaces can't hijack it", async () => {
    const dir = mkdtempSync(join(tmpdir(), "sc-e2e-team-"));
    const originalKey = process.env["SMASHCUT_API_KEY"];
    try {
      // Member A publishes into the shared team space.
      writeFileSync(join(dir, "index.html"), "<html>team v1</html>", "utf-8");
      process.env["SMASHCUT_API_KEY"] = "e2e-team-member-a";
      ...
      process.env["SMASHCUT_API_KEY"] = "e2e-team-member-b";
      ...
      process.env["SMASHCUT_API_KEY"] = "e2e-outsider";
      ...
      if (originalKey === undefined) delete process.env["SMASHCUT_API_KEY"];
      else process.env["SMASHCUT_API_KEY"] = originalKey;
      rmSync(dir, { recursive: true, force: true });
    }
  }, 120_000);
});
