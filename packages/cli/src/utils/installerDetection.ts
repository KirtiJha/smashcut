/**
 * Detect how the running `smashcut` binary was installed so auto-update can
...
/**
 * True when invoked via `npx smashcut` / `bunx smashcut`. These don't
 ...
 * symlinks `/opt/homebrew/bin/smashcut` into `…/Cellar/smashcut/<v>/…`
 * (or `/usr/local/Cellar/` on Intel). Either path wins the match.
 */
function isHomebrewInstall(realEntry: string): boolean {
  return normalizePath(realEntry).includes("/Cellar/smashcut/");
}
...
export function detectInstaller(): InstallerInfo {
  ...
  if (isHomebrewInstall(realEntry)) {
    return {
      ...
      installCommand: () => "brew upgrade smashcut",
  ...
  if (normalizedEntry.includes("/.bun/")) {
    return {
      kind: "bun",
      installCommand: (version) => `bun add -g smashcut@${version}`,
  ...
  if (normalizedEntry.includes("/pnpm/global/")) {
    return {
      kind: "pnpm",
      installCommand: (version) => `pnpm add -g smashcut@${version}`,
      reason: `pnpm global install detected at ${realEntry}`,
    };
  }

  // npm's default global prefix is `<prefix>/lib/node_modules/smashcut/…`
  // where `<prefix>` is `/usr/local` (macOS Intel), `/opt/homebrew` (Apple
  // Silicon, non-brew-formula npm), or a user-configured directory.
  if (
    normalizedEntry.includes("/lib/node_modules/smashcut/") ||
    normalizedEntry.includes("/npm/node_modules/smashcut/")
  ) {
    return {
      kind: "npm",
      installCommand: (version) => `npm install -g smashcut@${version}`,
...
export function installInvocation(kind: InstallerKind, version: string): InstallInvocation | null {
  switch (kind) {
    case "npm":
      return { bin: "npm", args: ["install", "-g", `smashcut@${version}`] };
    case "bun":
      return { bin: "bun", args: ["add", "-g", `smashcut@${version}`] };
    case "pnpm":
      return { bin: "pnpm", args: ["add", "-g", `smashcut@${version}`] };
    case "brew":
      // brew has no per-version install; `brew upgrade` moves to the tap's
      // current formula (a no-op if the tap hasn't caught up).
      return { bin: "brew", args: ["upgrade", "smashcut"] };
    case "skip":
      return null;
  }
}
