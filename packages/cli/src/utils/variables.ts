 ...
 * and validation helpers used by both `smashcut render` (in-process) and
 * `smashcut lambda render` (distributed). The Lambda CLI mirrors the
 ...
 * `smashcut render` and `smashcut lambda render` so the UX is
