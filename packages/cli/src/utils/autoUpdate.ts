 ...
 *
 * Flow across two runs of `smashcut`:
 *
 ...
 *     via `smashcut upgrade`.
 *   - Skip on CI, non-TTY, dev mode, unknown installer, ephemeral exec (npx),
 *     or when `SMASHCUT_NO_AUTO_INSTALL` / `SMASHCUT_NO_UPDATE_CHECK`
 *     is set.
 *   - If a previous install is still in flight (less than 10 min old), don't
 *     re-launch.
 *   - Installer output is redirected to `~/.smashcut/auto-update.log` for
...
function isAutoInstallDisabled(): boolean {
  ...
  if (process.env["SMASHCUT_NO_AUTO_INSTALL"] === "1") return true;
  return false;
}
...
function launchDetachedInstall(
  ...
  const child = spawn(process.execPath, ["-e", nodeScript], {
    ...
    windowsHide: true,
    env: { ...process.env, SMASHCUT_NO_UPDATE_CHECK: "1", SMASHCUT_NO_AUTO_INSTALL: "1" },
  });
  child.unref();
...
export function reportCompletedUpdate(): void {
  ...
  if (done.ok) {
    process.stderr.write(`  smashcut auto-updated to v${done.version}\n\n`);
  } else if (!done.reported) {
    // Failed installs are surfaced once too — the user should know why the
    // auto-update didn't take.
    process.stderr.write(
      `  smashcut auto-update to v${done.version} failed. Run \`smashcut upgrade\` to retry.\n\n`,
    );
  }
}
