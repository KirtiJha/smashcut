...
const MODELS_DIR = join(homedir(), ".cache", "smashcut", "whisper", "models");
...
function findFromEnv(): WhisperResult | undefined {
  const envPath = process.env["SMASHCUT_WHISPER_PATH"];
...
const BUILD_DIR = join(homedir(), ".cache", "smashcut", "whisper", "whisper.cpp");
...
function buildFromSource(onProgress?: (msg: string) => void): WhisperResult {
  ...
  if (!existsSync(BUILD_DIR)) {
    ...
    mkdirSync(join(homedir(), ".cache", "smashcut", "whisper"), {
      recursive: true,
    });
