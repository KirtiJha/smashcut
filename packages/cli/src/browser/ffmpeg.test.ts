...
// are covered by @smashcut/parsers ffBinaries.test.ts. These tests pin the
...
describe("findFFmpeg / findFFprobe", () => {
  it("returns undefined when the env override points at a missing file", () => {
    process.env.SMASHCUT_FFMPEG_PATH = join(tmpdir(), "missing-ffmpeg");
    process.env.SMASHCUT_FFPROBE_PATH = join(tmpdir(), "missing-ffprobe");
