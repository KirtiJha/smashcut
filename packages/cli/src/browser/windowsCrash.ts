 ...
 *
 * Field feedback (#smashcut-cli-feedback ts=1784116246, win32/x64,
 * SmashCut CLI 0.7.58) hit the exact error
 ...
 * combinations). The reporter recovered by pointing `SMASHCUT_BROWSER_PATH`
 ...
 * (SIGTRAP at launch on macOS arm64); same `SMASHCUT_BROWSER_PATH`
...
export function windowsChromeCrashRemediation(errorMessage: string): string | undefined {
  ...
  return [
    ...
    "The pinned Chromium build is not stable on this Windows host; point smashcut at your installed Chrome instead:",
    "",
    '  set SMASHCUT_BROWSER_PATH="C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe"',
    "",
