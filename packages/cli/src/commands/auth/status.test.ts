...
describe("buildUnconfiguredLines — interactive (TTY / agent-driven)", () => {
  ...
  it("makes browser OAuth the smashcut path", () => {
    expect(text).toContain("smashcut auth login");
  ...
  it("shows only zero-install `npx smashcut` paths, not the separately-installed heygen CLI", () => {
    expect(text).not.toMatch(/heygen auth login/);
    expect(text).toContain("npx smashcut auth login");
    expect(text).toContain("npx smashcut auth login --api-key");
  });

  it("offers the --api-key path as a secondary option", () => {
    expect(text).toContain("smashcut auth login --api-key");
  });
});
...
describe("buildUnconfiguredLines — offline engine readiness", () => {
  ...
  it("surfaces the pip setup hint and doctor pointer when a dep is missing", () => {
    ...
    expect(text).toContain("smashcut doctor");
  });
  ...
  it("recommends auth login and reports the local fallback", () => {
    for (const ctx of [INTERACTIVE, NON_INTERACTIVE]) {
      ...
      expect(payload).toMatchObject({
        ...
        recommended_action: "npx smashcut auth login",
        fallback: "local",
      });
    }
  });
