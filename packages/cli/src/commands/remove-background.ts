...
export const examples: Example[] = [
  [
    ...
    "smashcut remove-background avatar.mp4 -o transparent.webm",
  ],
  [
    "Output ProRes 4444 .mov for editing round-trip",
    "smashcut remove-background avatar.mp4 -o transparent.mov",
  ],
  [
    "Remove background from a single image, output transparent PNG",
    "smashcut remove-background portrait.jpg -o cutout.png",
  ],
  [
    "Separate the layers — emit both the cutout and an inverse-alpha background plate (subject region transparent)",
    "smashcut remove-background avatar.mp4 -o subject.webm --background-output plate.webm",
  ],
  [
    "Force CPU (skip CoreML/CUDA)",
    "smashcut remove-background avatar.mp4 -o transparent.webm --device cpu",
  ],
  [
    "Smaller file at the cost of color match (text-behind-subject won't blend as cleanly)",
    "smashcut remove-background avatar.mp4 -o transparent.webm --quality fast",
  ],
  [
    "Visually-lossless WebM (master / re-encode source)",
    "smashcut remove-background avatar.mp4 -o transparent.webm --quality best",
  ],
  ["Show detected providers without rendering", "smashcut remove-background --info"],
];
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (!args.input) {
      console.error(
        c.error(
          "Input file is required. Run `smashcut remove-background --info` for providers.",
        ),
      );
      failCommand();
    }
...
async function showInfo(json: boolean): Promise<void> {
  ...
  console.log(c.bold("smashcut remove-background — system info"));
  console.log("");
