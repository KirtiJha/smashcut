/**
 * `smashcut cloud` — top-level dispatcher for cloud-render subverbs.
 *
...
export const examples: Example[] = [
  ...
  ["Render a specific project", "smashcut cloud render ./my-video"],
  ["Inspect the project zip without uploading", "smashcut cloud render --dry-run"],
  [
    "Render at 60fps + high quality, save to a path",
    "smashcut cloud render ./my-video --fps 60 --quality high -o ./out.mp4",
  ],
  [
    "Fire-and-forget with a webhook",
    "smashcut cloud render ./my-video --callback-url https://example.com/sc-hook --no-wait",
  ],
  ["Resubmit an already-uploaded zip", "smashcut cloud render --asset-id asst_abc123"],
  [
    "Render from a public HTTPS zip",
    "smashcut cloud render --url https://cdn.example.com/site.zip",
  ],
  ["List recent cloud renders", "smashcut cloud list"],
  ["Fetch one render's status + signed URLs", "smashcut cloud get hfr_abc123"],
  ["Soft-delete a render", "smashcut cloud delete hfr_abc123"],
];

const HELP = `
${c.bold("smashcut cloud")} ${c.dim("<subcommand> [args]")}

Render SmashCut compositions on HeyGen's cloud infrastructure. The
...
${c.bold("AUTH:")}
  Uses the credential you signed in with via ${c.accent("smashcut auth login")}.
  Override the API base with ${c.accent("HEYGEN_API_URL")} (default https://api.heygen.com).
`;

export default defineCommand({
  meta: { name: "cloud", description: "Render SmashCut compositions on the HeyGen cloud" },
  subCommands: {
