...
export const examples: Example[] = [
  ["Find or download Chrome for rendering", "smashcut browser ensure"],
  ["Purge a stale/partial download and re-download", "smashcut browser ensure --force"],
  ["Print the Chrome executable path", "smashcut browser path"],
  ["Remove cached Chrome download", "smashcut browser clear"],
];
...
async function runEnsure(options?: { force?: boolean }): Promise<void> {
  clack.intro(c.bold("smashcut browser ensure"));
...
function runClear(): void {
  clack.intro(c.bold("smashcut browser clear"));
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (!subcommand || subcommand === "") {
      console.log(`
${c.bold("smashcut browser")} ${c.dim("<subcommand>")}
...
${c.bold("EXAMPLES:")}
  ${c.accent("npx smashcut browser ensure")}           ${c.dim("Download Chrome if needed")}
  ${c.accent("npx smashcut browser ensure --force")}   ${c.dim("Purge a stale/partial download and re-download")}
  ${c.accent("npx smashcut browser path")}             ${c.dim("Print path for scripts")}
  ${c.accent("npx smashcut browser clear")}            ${c.dim("Remove cached browser")}
`);
      return;
    }
    ...
    switch (subcommand) {
      ...
      default:
        ...
        console.error(
          `${c.error("Unknown subcommand:")} ${subcommand}\n\nRun ${c.accent("smashcut browser --help")} for usage.`,
        );
        failCommand();
    }
  },
