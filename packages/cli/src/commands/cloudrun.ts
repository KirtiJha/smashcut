...
/**
 * `smashcut cloudrun` — deploy + drive distributed renders on Google
 * Cloud Run + Cloud Workflows.
 *
 * The GCP counterpart to `smashcut lambda`. Thin glue: argument parsing
 * + help here; the work lives in `@smashcut/gcp-cloud-run/sdk`
 ...
 * `deploy` into a small state file under `~/.smashcut/` so `render` and
...
export const examples: Example[] = [
  ["Deploy the Cloud Run render stack", "smashcut cloudrun deploy --project my-gcp-project"],
  [
    "Render a composition on the deployed stack",
    "smashcut cloudrun render ./my-project --width 1920 --height 1080 --wait",
  ],
  [
    "Render a personalised template with variables",
    'smashcut cloudrun render ./my-template --width 1920 --height 1080 --variables \'{"title":"Hello Alice"}\'',
  ],
  [
    "Supersample a 1080p composition to 4K",
    "smashcut cloudrun render ./my-project --width 1920 --height 1080 --output-resolution 4k --wait",
  ],
  [
    "Batch-render N personalised videos from a JSONL file",
    "smashcut cloudrun render-batch ./my-template --batch ./users.jsonl --width 1920 --height 1080 --max-concurrent 10",
  ],
  ["Check progress for a started render", "smashcut cloudrun progress <executionName>"],
  [
    "Pre-upload a project so renders share the upload",
    "smashcut cloudrun sites create ./my-project",
  ],
  ["Tear the stack down", "smashcut cloudrun destroy --project my-gcp-project"],
];

const HELP = `
${c.bold("smashcut cloudrun")} ${c.dim("<subcommand> [args]")}
...
${c.bold("FIRST RUN:")}
  ${c.accent("smashcut cloudrun deploy --project my-gcp-project")}
  ${c.accent("smashcut cloudrun render ./my-project --width 1920 --height 1080 --wait")}
...
type CloudRunAdapter = typeof import("@smashcut/gcp-cloud-run/sdk") &
  typeof import("@smashcut/gcp-cloud-run/terraform");
...
function loadCloudRunAdapter(): Promise<CloudRunAdapter> {
  cloudRunAdapterPromise ??= Promise.all([
    import("@smashcut/gcp-cloud-run/sdk"),
    import("@smashcut/gcp-cloud-run/terraform"),
...
export function isMissingCloudRunAdapterError(error: unknown): boolean {
  return (
    ...
    normalizeErrorMessage(error).includes("@smashcut/gcp-cloud-run")
  );
}

export function missingCloudRunAdapterMessage(subcommand: string): string {
  return (
    `${c.error("@smashcut/gcp-cloud-run is not installed.")} The ${c.accent(`smashcut cloudrun ${subcommand}`)} command needs it at runtime.\n` +
    `Install it alongside the CLI:\n` +
    `  ${c.accent("npm install -g @smashcut/gcp-cloud-run")}\n` +
    `Or, for an opt-in project setup:\n` +
    `  ${c.accent("npm install @smashcut/gcp-cloud-run")}`
  );
}
...
export default defineCommand({
  ...
  args: {
    ...
    repo: {
      type: "string",
      description: "Artifact Registry repo for the built image (default: smashcut)",
    },
    ...
    "render-id": {
      type: "string",
      description: "Client render id / GCS prefix (default: sc-render-<uuid>)",
    },
    "output-key": {
      type: "string",
  ...
  if (missing.length > 0) {
    console.error(
      `[cloudrun] missing stack coordinates: ${missing.join(", ")}. ` +
        `Run \`smashcut cloudrun deploy --project <id>\` first, or pass them as flags.`,
    );
    failCommand();
  }
...
async function runDeploy(args: Record<string, unknown>): Promise<void> {
  ...
  const repo = (args.repo as string | undefined) ?? "smashcut";
  ...
  if (!image) {
    if (!repoRoot) {
      console.error(
        "[cloudrun deploy] --image is required when not running from a smashcut checkout (no Dockerfile context found).",
      );
      failCommand();
    }
    ...
    const tag = new Date()
      ...
      .replace(/[^0-9]/g, "")
      .slice(0, 14);
    image = `${region}-docker.pkg.dev/${project}/${repo}/smashcut-render:${tag}`;
  ...
  console.log(
    `  Next: ${c.accent("smashcut cloudrun render ./my-project --width 1920 --height 1080 --wait")}`,
  );
}
...
async function runSites(args: Record<string, unknown>): Promise<void> {
  ...
  if (!projectDir) {
    console.error("[cloudrun sites create] usage: smashcut cloudrun sites create <projectDir>");
    failCommand();
  }
...
async function runRender(args: Record<string, unknown>): Promise<void> {
  ...
  if (!projectDir) {
    console.error(
      "[cloudrun render] usage: smashcut cloudrun render <projectDir> --width <px> --height <px>",
    );
    failCommand();
  }
  ...
  if (!args.wait) {
    if (args.json) console.log(JSON.stringify(handle, null, 2));
      ...
      console.log(
        `  progress: ${c.accent(`smashcut cloudrun progress ${handle.executionName}`)}`,
      );
    }
    return;
  }
...
async function runProgress(args: Record<string, unknown>): Promise<void> {
  ...
  if (!executionName) {
    console.error("[cloudrun progress] usage: smashcut cloudrun progress <executionName>");
    failCommand();
  }
 ...
 * resolved manifest without starting anything. Mirrors `smashcut lambda
 * render-batch`.
 */
// fallow-ignore-next-line complexity
async function runRenderBatch(args: Record<string, unknown>): Promise<void> {
  ...
  if (!projectDir || !batchPath) {
    console.error(
      "[cloudrun render-batch] usage: smashcut cloudrun render-batch <projectDir> --batch <file.jsonl> --width <px> --height <px>",
    );
    failCommand();
  }
  const width = parsePositiveInt(args.width, "--width");
 ...
 * separately (it differs per batch entry). Mirrors the local `smashcut
 ...
 * one `smashcut render` and `smashcut lambda render` use), then validate
