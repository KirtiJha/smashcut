...
/**
 * Persists `smashcut lambda` stack outputs (bucket, state-machine ARN,
 * region) so `render` / `progress` / `destroy` don't need to re-derive
 * them from CloudFormation on every call.
 *
 * Stored at `<cwd>/.smashcut/lambda-stack-<stackName>.json`. Project-
 * local on purpose: a developer who runs `smashcut lambda deploy` in
 ...
 * agree on the literal `"smashcut-default"`.
 */
export const DEFAULT_STACK_NAME = "smashcut-default";
...
export function requireStack(stackName: string, cwd: string = process.cwd()): StackOutputs {
  ...
  if (!stack) {
    ...
    let hint = `Run \`smashcut lambda deploy${stackName === DEFAULT_STACK_NAME ? "" : ` --stack-name=${stackName}`}\` first.`;
    if (known.length) {
      hint += ` Known stacks here: ${known.join(", ")}.`;
    }
    console.error(
      `[smashcut lambda] no stack state for "${stackName}" at ${stateFilePath(stackName, cwd)}. ${hint}`,
    );
    failCommand();
  }
  return stack;
