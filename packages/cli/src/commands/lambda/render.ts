...
/**
 * `smashcut lambda render <projectDir>` — start a distributed render
 * against the deployed stack. Wraps {@link renderToLambda}. Does NOT
 * poll — use `smashcut lambda progress` for that.
 */
...
async function loadSDK(): Promise<typeof import("@smashcut/aws-lambda/sdk")> {
  return import("@smashcut/aws-lambda/sdk");
}
...
export interface RenderArgs {
   ...
   * flag, mismatches are warnings (matches the local `smashcut render`
   * behavior).
   */
...
export async function runRender(args: RenderArgs): Promise<void> {
  ...
  // `smashcut render` uses. `resolveVariablesArg` exits(1) with a friendly
  ...
  console.log(c.dim(`Poll with: smashcut lambda progress ${handle.renderId}`));
}
 ...
 * `smashcut lambda render`. Extracted for boundary-test coverage of the
