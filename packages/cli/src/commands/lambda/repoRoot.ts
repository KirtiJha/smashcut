/**
 * Resolve the SmashCut repo root. Used by `lambda deploy`/`destroy`
...
export function repoRoot(): string {
  const override = process.env.SMASHCUT_REPO_ROOT;
  ...
  throw new Error(
    "[smashcut lambda] could not find the repo root (no packages/aws-lambda/ above this CLI's source). " +
      "Run `smashcut lambda` from within a smashcut checkout, or set SMASHCUT_REPO_ROOT explicitly.",
  );
}
