...
/**
 * `smashcut lambda render-batch <projectDir> --batch <path.jsonl>` —
...
/** Arguments accepted by `smashcut lambda render-batch`. */
 ...
 * composition's `window.__scVariables`.
 */
export interface BatchEntry {
   ...
   * `window.__scVariables`). Optional — pass `{}` if a row needs the
   * composition's defaults verbatim.
   */
  variables?: Record<string, unknown>;
  /**
   * Optional explicit Step Functions execution name. Defaults to
   * `sc-render-<uuid>` (generated by the SDK). Useful when the caller
 ...
 * for every entry in a `smashcut lambda render-batch` run. Extracted for
