...
/**
 * `smashcut lambda policies role|user|validate` — IAM bootstrap.
 *
 ...
 * attempt at `smashcut lambda deploy` is `User is not authorized to
  ...
  switch (args.verb) {
    case "user": {
      ...
      if (!args.json) {
        console.error(
          c.dim(
            "\n# Attach the above as an inline policy to the IAM user/role that runs `smashcut lambda *`.\n# Scope `Resource` to your stack's ARNs after the first successful deploy.",
          ),
        );
      }
      return;
    ...
    case "validate": {
      if (!args.inputPath) {
        const msg =
          "[lambda policies validate] usage: smashcut lambda policies validate <policy.json>";
        if (args.json) {
      ...
      console.log();
      console.log(
        c.dim("Run `smashcut lambda policies user` to print the full required policy."),
      );
      setCommandExitCode(1);
      return;
    }
