/**
 * `smashcut lambda sites create <projectDir>` — tar + upload a project
...
// `@smashcut/aws-lambda` is a workspace devDependency in `packages/cli`
...
export async function runSitesCreate(args: SitesCreateArgs): Promise<void> {
  ...
  console.log(
    c.dim(
      `Render with: smashcut lambda render ${args.projectDir} --site-id=${handle.siteId}` +
