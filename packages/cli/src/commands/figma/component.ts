/**
 * `smashcut figma component <ref>` — Phase 3: node tree → editable HTML
  ...
  async run({ args }) {
    await withFigmaErrors("figma:component", async () => {
      ...
      if (result.unresolved.length > 0) {
        console.log(
          `${result.unresolved.length} binding(s) reference tokens not yet imported — colors baked as literals (flagged data-figma-unresolved). Run \`smashcut figma tokens\` on the source/library file, then re-import to link them.`,
        );
      }
