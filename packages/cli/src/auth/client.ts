...
// every HeyGen call (both auth types) so the backend can isolate smashcut CLI
...
export const HEYGEN_CLIENT_SOURCE = "smashcut";
...
export function buildAuthHeaders(credential: ResolvedCredential): Record<string, string> {
  ...
  // sent here — an API-key smashcut call is still smashcut usage.
