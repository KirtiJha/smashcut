...
tl.to("[data-sc-id=\\"sc-box\\"]", { keyframes: { "0%": { opacity: 0 }, "50%": { opacity: 0.7 }, "100%": { opacity: 1 } }, duration: 1 }, 0);
window.__timelines["t"] = tl;`;

function makeHtml(script: string) {
  return `<div data-sc-id="sc-stage" data-sc-root style="width: 1280px; height: 720px">
...
const TWEEN_ANIM_ID = `[data-sc-id="sc-box"]-to-200-visual`;
...
describe("addGsapKeyframe", () => {
  it("inserts new keyframe at given percentage", () => {
    ...
    const animId = `[data-sc-id="sc-box"]-to-0-visual`;
    const result = applyOp(parsed, {
  ...
  it("backfills a NEW property into the other keyframes, matching the recast writer", async () => {
    ...
    const { parseGsapScript } = await import("@smashcut/core/gsap-parser");
    const { addKeyframeToScript } = await import("@smashcut/core/gsap-writer-acorn");
...
describe("setGsapKeyframe", () => {
  ...
  it("returns EMPTY for out-of-range keyframeIndex", () => {
    ...
    const result = applyOp(parsed, {
      type: "setGsapKeyframe",
      animationId: `[data-sc-id="sc-box"]-to-0-visual`,
      keyframeIndex: 99,
      value: { opacity: 0 },
    });
    expect(result.forward).toHaveLength(0);
...
tl.to("[data-sc-id=\\"sc-box\\"]", { x: 200, duration: 1 }, 0);
window.__timelines["t"] = tl;`;
    const parsed = fresh(script);
    // position 0 → "0"; x = position group
    const animId = `[data-sc-id="sc-box"]-to-0-position`;
    const result = applyOp(parsed, {
...
describe("splitIntoPropertyGroups", () => {
  it("splits mixed tween into multiple group tweens", () => {
    const script = `var tl = gsap.timeline({ paused: true });
tl.to("[data-sc-id=\\"sc-box\\"]", { x: 100, opacity: 0.5, duration: 1 }, 0);
window.__timelines["t"] = tl;`;
    const parsed = fresh(script);
    // mixed tween has no propertyGroup → no group suffix in id
    const animId = `[data-sc-id="sc-box"]-to-0`;
  ...
  it("no-op when tween has only one property group", () => {
    ...
    const script = `var tl = gsap.timeline({ paused: true });
tl.to("[data-sc-id=\\"sc-box\\"]", { x: 100, y: 50, duration: 1 }, 0);
window.__timelines["t"] = tl;`;
    const parsed = fresh(script);
    const animId = `[data-sc-id="sc-box"]-to-0-position`;
    const result = applyOp(parsed, { type: "splitIntoPropertyGroups", animationId: animId });
...
window.__timelines["t"] = tl;`;
  ...
  function freshSplit() {
    return parseMutable(`<div data-sc-id="sc-stage" data-sc-root style="width:1280px;height:720px">
  <div data-sc-id="sc-hero"></div>
...
tl.to("[data-sc-id=\\"sc-box\\"]", { x: 100, duration: 1 }, 2);
tl.to("[data-sc-id=\\"sc-box\\"]", { x: 200, duration: 1 }, 5);
window.__timelines["t"] = tl;`;

  function freshStagger() {
    return parseMutable(`<div data-sc-id="sc-stage" data-sc-root style="width:1280px;height:720px">
  <div data-sc-id="sc-box" data-start="2" data-end="7"></div>
  ...
  it("moving the clip +1 shifts BOTH tweens by the delta, preserving the stagger", () => {
    const result = applyOp(freshStagger(), { type: "setTiming", target: "sc-box", start: 3 });
  ...
  it("resizing the clip x2 scales each tween's duration by the ratio (not full clip)", () => {
    ...
    const result = applyOp(freshStagger(), { type: "setTiming", target: "sc-box", duration: 10 });
    const script = gsapPatch(result);
...
describe("addLabel", () => {
  ...
  it("addLabel output is not blocked by GSAP validator", async () => {
    const { validateCompositionGsap } = await import("@smashcut/core/gsap-parser");
    const parsed = fresh();
...
describe("removeElement — GSAP cascade", () => {
  ...
  it("applying inverse restores element and GSAP script to original", () => {
    ...
    expect(parsed.document.querySelector('[data-sc-id="sc-box"]')).not.toBeNull();
    expect(getScript(parsed)).toBe(origScript);
  });

  it("emits only element patch when composition has no GSAP script", () => {
    const noScriptHtml = `<div data-sc-id="sc-stage" data-sc-root style="width:1280px;height:720px">
...
tl.to("[data-sc-id=\\"sc-stage\\"]", { scale: 1.05, duration: 1 }, 0);
window.__timelines["t"] = tl;`;
    const parsed = fresh(twoTweenScript);
    const result = applyOp(parsed, { type: "removeElement", target: "sc-box" });
    const newScript = String(result.forward[1]?.value ?? "");
    expect(newScript).not.toContain("sc-box");
    expect(newScript).toContain("sc-stage");
  });
...
tl.to("[data-sc-id=\\"sc-box\\"]", { x: 100, duration: 1 }, 0);
tl.to("[data-sc-id=\\"sc-box\\"]", { x: 200, duration: 1 }, 1);
window.__timelines["t"] = tl;`;
...
const NO_SCRIPT_HTML = `<div data-sc-id="sc-stage" data-sc-root style="width:1280px;height:720px">
  <div data-sc-id="sc-box" style="opacity:0"></div>
</div>`.trim();
...
tl.to('[data-sc-id="sc-hero"]', { x: 100, y: 50, duration: 2 }, 0);
window.__timelines["t"] = tl;`;
const ARC_ANIM_ID = `[data-sc-id="sc-hero"]-to-0-position`;
...
describe("deleteAllForSelector quote-insensitive match (R3 #13b)", () => {
  it("removes a tween authored with double quotes when given a single-quoted selector", () => {
    const html = `<div data-sc-id="sc-stage" data-sc-root style="width: 1280px; height: 720px">
  <div data-sc-id="sc-box"></div>
  <script>var tl = gsap.timeline({ paused: true }); tl.to("[data-sc-id=\\"sc-box\\"]", { x: 1, duration: 1 }, 0); window.__timelines["t"] = tl;</script>
</div>`;
    const parsed = parseMutable(html);
    const result = applyOp(parsed, {
      type: "deleteAllForSelector",
      selector: `[data-sc-id='sc-box']`,
    });
...
describe("handleSetTiming GSAP sync (CF2 #15/#16)", () => {
  ...
  <div id="box" data-sc-id="sc-box" ${attrs}></div>
  ...
  it("#15: a #domId-targeted tween shifts when the clip moves", () => {
    ...
    applyOp(parsed, { type: "setTiming", target: "sc-box", start: 5 });
  ...
  it("#16: a data-duration clip updates data-duration and scales its tween", () => {
    ...
    applyOp(parsed, { type: "setTiming", target: "sc-box", duration: 8 });
    const el = parsed.document.querySelector('[data-sc-id="sc-box"]');
  ...
  it("R5 #3: a start-less clip (no data-start) still shifts its tween (implicit start 0)", () => {
    ...
    applyOp(parsed, { type: "setTiming", target: "sc-box", start: 3 });
...
describe("addWithKeyframes / replaceWithKeyframes — handler self-guards", () => {
  ...
  it("replaceWithKeyframes: stale id whose tween targets another selector is a no-op", () => {
    ...
    const wrong = applyOp(parsed, {
      type: "replaceWithKeyframes",
      animationId: id,
      targetSelector: '[data-sc-id="sc-other"]',
      position: 0,
      duration: 1,
      keyframes: KFS,
    });
