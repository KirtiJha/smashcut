...
<head></head>
<body>
<div id="stage" data-sc-root data-width="1920" data-height="1080" data-duration="5">
  <h1 id="title" data-sc-id="sc-title" data-start="0" data-end="3"
      style="color: #fff; font-size: 64px">Hello World</h1>
  <img id="logo" data-sc-id="sc-logo" src="/logo.png" alt="Logo"
       data-x="100" data-y="200" data-start="0" data-end="5" />
  <p id="body-copy" data-sc-id="sc-body" data-start="1" data-end="4"
...
describe("openComposition + basic mutations", () => {
  ...
  it("setStyle mutates inline styles and serializes back to HTML", async () => {
    const comp = await openComposition(BASE_HTML);
    comp.setStyle("sc-title", { color: "#f00", fontSize: "96px" });
  ...
  it("element handle sugar — same result as direct setStyle", async () => {
    const comp = await openComposition(BASE_HTML);
    comp.element("sc-title").setStyle({ color: "#0f0" });
    expect(comp.getElement("sc-title")?.inlineStyles.color).toBe("#0f0");
  });

  it("setText updates text content", async () => {
    const comp = await openComposition(BASE_HTML);
    comp.setText("sc-title", "Goodbye World");
    expect(comp.getElement("sc-title")?.text).toContain("Goodbye World");
    expect(comp.serialize()).toContain("Goodbye World");
  });

  it("dispatch moveElement writes data-x/data-y attributes", async () => {
    const comp = await openComposition(BASE_HTML);
    comp.dispatch({ type: "moveElement", target: "sc-logo", x: 300, y: 400 });
    const el = comp.getElement("sc-logo");
  ...
  it("serialize round-trip: mutate → serialize → reopen → same state", async () => {
    const comp = await openComposition(BASE_HTML);
    comp.setStyle("sc-title", { color: "#0f0" });
    comp.setText("sc-body", "Round-tripped");
    const html = comp.serialize();

    const comp2 = await openComposition(html);
    expect(comp2.getElement("sc-title")?.inlineStyles.color).toBe("#0f0");
    expect(comp2.getElement("sc-body")?.text).toContain("Round-tripped");
  });
});
...
describe("patch events", () => {
  it("emits a patch event per dispatch with correct path and value", async () => {
    ...
    comp.setStyle("sc-title", { fontSize: "48px" });
  ...
  it("applyPatches origin is tagged ORIGIN_APPLY_PATCHES", async () => {
    ...
    comp.applyPatches([
      { op: "replace", path: "/elements/sc-title/inlineStyles/color", value: "#00f" },
    ]);
...
describe("applyPatches", () => {
  it("applies external RFC 6902 patches to the live document", async () => {
    ...
    comp.applyPatches([
      { op: "replace", path: "/elements/sc-title/inlineStyles/color", value: "#00f" },
      { op: "replace", path: "/elements/sc-title/text", value: "Patched" },
    ]);
    expect(comp.getElement("sc-title")?.inlineStyles.color).toBe("#00f");
    expect(comp.getElement("sc-title")?.text).toContain("Patched");
  });
...
describe("batch()", () => {
  ...
  it("rolls back DOM on throw — model unchanged after throwing batch", async () => {
    const comp = await openComposition(BASE_HTML);
    const beforeColor = comp.getElement("sc-title")?.inlineStyles.color;
    ...
    expect(comp.getElement("sc-title")?.inlineStyles.color).toBe(beforeColor);
  });
...
describe("undo / redo", () => {
  it("undo reverts last mutation, redo re-applies it", async () => {
    const comp = await openComposition(BASE_HTML);
    comp.setStyle("sc-title", { color: "#f00" });
    expect(comp.getElement("sc-title")?.inlineStyles.color).toBe("#f00");
  ...
  it("undo with no history is a no-op", async () => {
    const comp = await openComposition(BASE_HTML);
    const before = comp.getElement("sc-title")?.inlineStyles.color;
    comp.undo(); // no-op
    expect(comp.getElement("sc-title")?.inlineStyles.color).toBe(before);
  });
});
...
describe("T3 embedded mode", () => {
  it("applies override-set on open, mutations layer on top", async () => {
    const comp = await openComposition(BASE_HTML, {
      overrides: { "sc-title.style.color": "#0f0" },
    });
    expect(comp.getElement("sc-title")?.inlineStyles.color).toBe("#0f0");
  });
  ...
  it("getOverrides() returns accumulated override-set", async () => {
    ...
    expect(overrides["sc-title.style.color"]).toBe("#f00");
  });
...
describe("engine helper exports (resolveScoped, findById, escapeHfId, readVariableDefault)", () => {
  it("are importable from the public index and work against a live document", async () => {
    ...
    expect(findById(document as unknown as Document, "sc-title")).not.toBeNull();
    expect(resolveScoped(document as unknown as Document, "sc-title")).not.toBeNull();
