# Standalone SmashCut Slideshow Harness

## 1. Interim framing — why this exists

These patterns are a **temporary workaround** for standalone demos. The durable solution is engine-hosted: a future `smashcut preview --slideshow` / studio present mode will host the composition over the real SmashCut engine, which drives seek-timelines frame-by-frame, owns the gesture frame, and reads the slideshow island directly from the composition. When that path ships, most of what follows collapses.
...
2. `<smashcut-slideshow>` reads the slideshow island from its **own innerHTML** (the wrapper element), not from the composition the player loads. The island must be duplicated into the wrapper.
...
For public or user-facing generated projects, make this wrapper the root `index.html` so opening the project in a browser runs the slideshow. Put the raw SmashCut composition in a separate path such as `composition/index.html`. In repo examples you may still see this file called `demo.html`; that name is a reference pattern, not the preferred handoff for a standalone deck.
...
<html lang="en">
  <head>
    ...
    <!--
      ...
      The global builds register <smashcut-player> and <smashcut-slideshow>
      as custom elements — no import map needed.
    -->
    <script src="../../../packages/player/dist/smashcut-player.global.js"></script>
    <script src="../../../packages/player/dist/slideshow/smashcut-slideshow.global.js"></script>
  ...
  <body>
    <!--
      tabindex="0" is critical — <smashcut-slideshow> binds keydown
...
(function () {
  ...
  var channel = new BroadcastChannel("sc-slideshow:" + location.pathname);
  ...
  function frameDocument() {
    var player = document.querySelector("smashcut-player");
...
The shared `<smashcut-slideshow>` presenter already renders speaker notes as an editable textarea and stores edits in `localStorage`. Do not add deck-specific note editors when the shared player is available.
...
```js
const NOTES_STORAGE_PREFIX = "sc-slideshow:presenter-notes:v1:";

function notesDeckKey(slideshowEl) {
  const explicit = slideshowEl.getAttribute("notes-storage-key");
  if (explicit && explicit.trim()) return explicit.trim();

  const playerSrc = slideshowEl.querySelector("smashcut-player")?.getAttribute("src") || "";
...
function setWorldCamera(world, cam, animate) {
  if (!cam) return;
  if (!animate) world.classList.add("sc-camera-static");
  world.style.transform = cameraTransform(cam);
  world.style.opacity = "1";
  if (!animate) {
    world.getBoundingClientRect();
    world.classList.remove("sc-camera-static");
  }
  currentCamera = cam;
}
...
function updateCameraForTime(t, opts) {
  ...
  var animateCamera = Boolean(
    window.__scCameraTransitionsEnabled && jumpedBetweenSlides && !(opts && opts.staticCamera),
  );
...
During measurement, temporarily remove the transform with `sc-camera-static`, compute all element union rects, then restore `currentCamera` without animation. On initial load, resize, and validation-style seeks, call `updateCameraForTime(t, { staticCamera: true })`. In the standalone wrapper, set `iframe.contentWindow.__scCameraTransitionsEnabled = true` after the player iframe is available. That keeps the exported composition seekable while letting presenter navigation glide between focal points.

Before validation, resolve source font variables. SmashCut lint accepts concrete generic stacks such as `system-ui, sans-serif` and `ui-monospace, monospace`, or real `@font-face` declarations pointing at local font files. It does not accept `font-family: var(--f-body)` / `var(--f-mono)` as a render-safe family.
...
<script>
  (function () {
    ...
    window.__scSetTime = updateVisibility;
...
`<smashcut-slideshow>` must know each scene's time range to map a `sceneId` to a playhead position. Without the engine injecting this at runtime, the composition must post it manually after load.
...
## 6. Audio/SFX — built-in mute control via `<smashcut-slideshow sound>`
...
Normal slide media should stay in the composition. The slideshow player now stops slide media automatically on slide/sequence changes by calling `smashcut-player.stopMedia()`, which pauses iframe `<video>` / `<audio>`, runtime WebAudio, and parent proxies adopted from iframe media. Same-slide fragment reveals do not stop media, and global/deck-level parent audio such as `audio-src` is left alone. Do not hand-roll per-slide cleanup scripts for regular video/audio players.

Every copied `<video>` / `<audio>` with a `src` must be timed for SmashCut ownership:
...
Add the `sound` boolean attribute to `<smashcut-slideshow>` in demo.html. The component renders a speaker/speaker-muted SVG button as the **leftmost item in the nav capsule**, styled identically to the prev/next ghost buttons. No separate mute button in the composition.

```html
<smashcut-slideshow tabindex="0" sound style="..."> ... </smashcut-slideshow>
```

The component:

- Tracks `muted` state (default `false`); exposes a `muted` getter
- Reflects to a `data-sc-muted` attribute on the host when muted
- Applies mute globally to child `<smashcut-player>` media and top-level page `<audio>` / `<video>` elements
- Dispatches `CustomEvent("sc-sound", { detail: { muted }, bubbles: true, composed: true })` on every toggle
...
Wrapper-owned `new Audio(...)` objects are not attached to the DOM, so the parent audio player must mirror the `sc-sound` event onto each clip:

```js
var muted = false;
var slideshow = document.querySelector("smashcut-slideshow");
if (slideshow) {
  slideshow.addEventListener("sc-sound", function (e) {
...
If `sound` is **not** present on `<smashcut-slideshow>` (decks without audio), the mute control is hidden — the capsule shows only nav.
...
function playSfx(name) {
  try {
    parent.postMessage({ type: "sc-sfx", name: name }, "*");
  } catch (e) {}
}
...
<script>
  (function () {
    ...
    // Track mute state from the slideshow component's sc-sound event.
    var muted = false;
    var slideshow = document.querySelector("smashcut-slideshow");
    if (slideshow) {
      slideshow.addEventListener("sc-sound", function (e) {
...
**Sourcing SFX files:** use the HeyGen MCP `search_audio_sounds` tool with `type=sound_effects` and keywords like "whoosh", "click", "transition". Download the results to a local `sfx/` directory next to `demo.html` and reference them by relative path. Do not fetch SFX at render time — the SmashCut determinism rule forbids runtime network requests; pre-download and commit them.
...
| ----------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Island not duplicated in wrapper                      | Slideshow chrome never renders; no slide counter, no prev/next  | Copy the `<script type="application/smashcut-slideshow+json">` block verbatim into the `<smashcut-slideshow>` element in demo.html         |
| Wrapper SFX in the iframe                             | Click/transition sounds silent                                  | Move SFX Audio elements and unlock logic to demo.html; post `{type:'sc-sfx',name}` from index.html                                               |
...
| Keydown bound to the element without focus            | ArrowLeft/Right dead                                            | Add `tabindex="0"` to `<smashcut-slideshow>` so it can receive keyboard focus                                                                 |
...
| Presenter media events are not bridged                | Audience follows slides but not play/pause/seek/mute            | Mirror native media events over `BroadcastChannel("sc-slideshow:" + location.pathname)` in standalone wrappers with interactive media            |
...
| Copied media lacks SmashCut timing                 | Lint errors on untimed media; preview/render diverge            | Add `data-start`, `data-duration`, and `data-has-audio="true"` when audible; avoid `preload="none"`                                              |
