...
test("assembler rejects malformed worker output before writing index.html", () => {
  ...
  assert.throws(
    () =>
      execFileSync(
        process.execPath,
        [assembleScript, "--storyboard", join(project, "STORYBOARD.md"), "--smashcut", project],
...
test("Code editorial preset stages renderer-parity fonts for an empty PR token set", () => {
  ...
  execFileSync(
    process.execPath,
    [buildFrameScript, "--preset", "code-editorial", "--smashcut", project],
    { encoding: "utf8" },
  );
...
test("bundled Code editorial font licenses are shipped beside the assets", () => {
  const fontDir = resolve(
    scriptDir,
    "../../smashcut-creative/frame-presets/code-editorial/fonts",
  );
