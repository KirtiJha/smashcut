...
export function stageAssets({ smashcutDir, frames }) {
  const wanted = new Set();
  for (const f of frames) {
    for (const b of basenamesFromCandidates(f.extra?.asset_candidates)) wanted.add(b);
  }
  const captureDirs = [
    join(smashcutDir, "capture/assets"),
    join(smashcutDir, "capture/assets/videos"), // videos download into a subdir
    join(smashcutDir, "capture/assets/svgs"), // inline SVGs extract into a subdir
    join(smashcutDir, "capture/screenshots"),
  ];
  const assetsDir = join(smashcutDir, "assets");
  const anomalies = [];
  let staged = 0;
  if (wanted.size > 0) {
