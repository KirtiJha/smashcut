...
- **Palette** — `frame.md` (code-editorial) is the color truth; apply its roles per frame. Generic basics → `smashcut-creative/references/house-style.md`.
- **Type** — fonts resolve via `frame.md`'s type tokens; reference them **by role** (display / body / mono / the pack's ramp), never by raw family or px. Code surfaces and mechanism labels use the **mono** role. Typography craft → `smashcut-creative/references/typography.md`.
...
| palette + type tokens                                                           | the project's `frame.md` (code-editorial); basics → `smashcut-creative`     |
