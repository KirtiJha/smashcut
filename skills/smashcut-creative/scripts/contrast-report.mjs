#!/usr/bin/env node
// contrast-report.mjs — SmashCut contrast audit
//
...
//
// Usage:
//   node skills/smashcut-creative/scripts/contrast-report.mjs <composition-dir> \
//     [--samples N] [--out <dir>] [--width W] [--height H] [--fps N]
//
// Env:
//   SMASHCUT_SKILL_PKG_VERSION — pin the @smashcut/producer version used
...
// file server injects the SmashCut runtime and render-seek bridge.
const packages = await importPackagesOrBootstrap(
  ["@smashcut/producer", "@smashcut/core", "@smashcut/core/compiler", "sharp"],
  {
    npmPackages: [
      smashcutPackageSpec("@smashcut/producer"),
      smashcutPackageSpec("@smashcut/core"),
      "sharp@0.34.5",
    ],
  },
);
const sharp = packages.sharp.default;
const { parseFps } = packages["@smashcut/core"];
const {
  createFileServer,
...
const OUT_DIR = resolve(args.out ?? ".smashcut/contrast");
...
const bundle = await bundleCompositionForCapture(packages["@smashcut/core/compiler"], COMP_DIR);
let server;
let session;
try {
