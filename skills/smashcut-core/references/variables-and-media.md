...
```js
const { title, accent } = window.__smashcut.getVariables();
...
- Use `npx smashcut render --variables '{"title":"Q4 Report"}'` or `--variables-file` for render-time overrides.
...
- **Do not** call `video.play()`, `audio.play()`, pause, or seek in composition code. SmashCut owns playback.
