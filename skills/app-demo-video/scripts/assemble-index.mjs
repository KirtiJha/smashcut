...
function hoistApprovedVideos(html, label) {
  ...
  // Match against `scan`, not `html`.
  //
  // `scan` is the same length as `html` with comments and script/style bodies
  // blanked to spaces, so offsets map 1:1 and a real tag appears verbatim. That
  // matters because this pattern spans from `<video` to the next `</video>`: a
  // comment that merely MENTIONS `<video>` — which a frame explaining why it has
  // no z-index naturally does — opens a match that then swallows the genuine
  // element hundreds of lines later. The `scan[offset] !== "<"` guard below
  // correctly spots that the match began inside a comment and bails, but the
  // real video has already been consumed by it and is never hoisted. The frame
  // then fails assembly with "has a <video> inside the sub-composition",
  // pointing at the one thing the author did right.
  //
  // Scanning the blanked copy means such a comment cannot start a match at all.
  const re = /<video\b((?:[^>"']|"[^"]*"|'[^']*')*)>([\s\S]*?)<\/video\s*>/gi;
  const ONE_TAG = /^<video\b((?:[^>"']|"[^"]*"|'[^']*')*)>([\s\S]*?)<\/video\s*>$/i;

  // Decide what a single matched element becomes: itself, or the hoist marker.
  const decide = (attrs, inner, full) => {
  ...
  // Walk the blanked copy for positions, but splice the ORIGINAL html, so the
  // frame keeps its comments and its script/style bodies intact.
  let repaired = "";
  let cursor = 0;
  let m;
  while ((m = re.exec(scan))) {
    const offset = m.index;
    const full = html.slice(offset, offset + m[0].length);
    // Attributes and inner come from the real source, not the blanked copy.
    const parsed = ONE_TAG.exec(full);
    repaired += html.slice(cursor, offset) + decide(parsed?.[1] ?? m[1], parsed?.[2] ?? m[2], full);
