...
//
//   node audio.mjs --script ./SCRIPT.md --storyboard ./STORYBOARD.md --smashcut . --out ./audio_meta.json
//   node audio.mjs sync-durations --audio-meta ./audio_meta.json --storyboard ./STORYBOARD.md
//   node audio.mjs fetch-sfx --storyboard ./STORYBOARD.md --smashcut .
...
function runEngine({ request, smashcutDir, neutral, only, extra = [] }, die) {
  const reqPath = join(smashcutDir, "audio_request.json");
...
function runGenerate(argv) {
  ...
  const scriptPath = resolve(flag(argv, "script", join(smashcutDir, "SCRIPT.md")));
  const outPath = resolve(flag(argv, "out", join(smashcutDir, "audio_meta.json")));
  ...
  runEngine({ request, smashcutDir, neutral, only: "tts,bgm" }, die);
...
function runFetchSfx(argv) {
  ...
  const smashcutDir = resolve(flag(argv, "smashcut", "."));
  const storyboardPath = resolve(flag(argv, "storyboard", join(smashcutDir, "STORYBOARD.md")));
  const outPath = resolve(flag(argv, "audio-meta", join(smashcutDir, "audio_meta.json")));
  ...
  runEngine({ request, smashcutDir, neutral, only: "sfx" }, die);
