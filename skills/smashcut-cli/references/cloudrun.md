...
Use `smashcut cloudrun` only when the user explicitly wants self-managed Google Cloud infrastructure. It deploys Cloud Run, Workflows, and Cloud Storage. For a managed default use `smashcut cloud`; for AWS use `smashcut lambda`.
...
```bash
npx smashcut cloudrun deploy --project <gcp-project> --region us-central1
npx smashcut cloudrun sites create ./project
npx smashcut cloudrun render ./project --width 1920 --height 1080 --wait
npx smashcut cloudrun progress <execution-name>
npx smashcut cloudrun destroy --project <gcp-project>
```

`deploy` enables the required Google APIs, builds or accepts a container image, applies the bundled Terraform module, and stores the resulting coordinates in `~/.smashcut/cloudrun-state.json`. Use deploy flags such as `--image`, `--repo`, `--cpu`, `--memory`, `--max-instances`, and `--timeout` only when the infrastructure needs those overrides.
...
```bash
npx smashcut cloudrun render ./template \
...
Use exactly one of `--variables` and `--variables-file`. Read [`variables-and-media.md`](../../smashcut-core/references/variables-and-media.md#variables) for the composition-side contract.

## JSONL batches

```bash
npx smashcut cloudrun render-batch ./template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10 \
  --site-id <site-id> \
