...
```bash
npx smashcut preview                   # foreground on a TTY; persistent in agent shells
npx smashcut preview --background      # explicit persistent session
npx smashcut preview --foreground --json # ready JSON, then remain attached
npx smashcut preview --background --port 4567 # agent-safe custom port (default 3002)
npx smashcut preview --selection --json # print the current Studio selection and exit
npx smashcut preview --context --json  # print compact agent context from Studio
```
...
Use the actual port and project directory name; treat `index.html` as source-code context, not the preview surface. For example, after `npx smashcut preview --background --port 3017` in `codex-openai-video`, report `http://localhost:3017/#project/codex-openai-video`.
...
The compact context payload includes the selected element's source file, composition path, current timeline time, `data-sc-id` / selector target, bounding box, text content, and a thumbnail URL for the selected element. Prefer `selection.target.hfId` when present; fall back to `selection.target.selector` only when no stable `data-sc-id` exists. If `selection` is `null`, inspect `errors.selection.code` (for example, `no-selection`).

Keep agent context small by asking only for the slices you need:

```bash
npx smashcut preview --context --json --context-fields selection
npx smashcut preview --context --json --context-fields lint
npx smashcut preview --context --json --context-fields selection,lint
```
...
```bash
npx smashcut preview --context --json --context-fields selection --context-detail full
```
...
| Code                       | Meaning                                                                    |
| -------------------------- | -------------------------------------------------------------------------- |
| `preview-not-running`      | Start Studio first with `npx smashcut preview --background`.            |
...
Both `preview` and `play` can open inside an explicit Chromium-compatible browser instead of the OS default. Two use cases: isolated Chromium profile, or external CDP attach (DevTools / Playwright / Puppeteer / browser-MCP). **SmashCut itself does not own CDP automation** — this only exposes the endpoint; whatever connects to it is your problem. Not to be confused with `--browser-gpu` (a `render` flag controlling Chrome GPU access during capture).
...
npx smashcut preview --background --browser-path /usr/bin/chromium --user-data-dir /tmp/sc-profile

# Same plus a CDP endpoint on :9222 (attach DevTools / Playwright / etc.)
npx smashcut play --browser-path /usr/bin/chromium --user-data-dir /tmp/sc-profile --remote-debugging-port 9222
```
...
```bash
npx smashcut render                                # standard MP4 from cwd
npx smashcut render ./my-video --output ./out.mp4  # render from outside the project dir
npx smashcut render --output final.mp4             # named output (no timestamp)
npx smashcut render -c compositions/intro.html -o intro.mp4  # render a specific sub-composition file
npx smashcut render --quality draft                # fast iteration
npx smashcut render --fps 60 --quality high        # final delivery
npx smashcut render --format webm                  # transparent WebM
npx smashcut render --docker                       # byte-identical
```
...
**Parametrized renders:** the composition declares its variables on the `<html>` root with **`data-composition-variables`** — a JSON **array of declarations** (`{id, type, label, default}` per entry) that defines the schema. Scripts inside read the resolved values via `window.__smashcut.getVariables()`. The CLI `--variables '{"title":"Q4 Report"}'` is a JSON **object keyed by id** that overrides those declared defaults for one render; missing keys fall through, so the same composition runs unchanged in dev preview and in production. Sub-comp hosts can also override per-instance with `data-variable-values`. See the `smashcut-core` skill for the full pattern.
...
```bash
npx smashcut feedback --rating 10                              # clean run, no notes
npx smashcut feedback --rating 6 --comment "bg <video> renders grey in multi-scene; worked around with --format png-sequence"
```
...
```text
REPRO COMMAND: <HF_*/PRODUCER_* env> npx smashcut <exact command>   # run from the project directory; do NOT paste absolute paths
...
```bash
npx smashcut publish              # upload current project privately, return stable URL
npx smashcut publish ./my-video   # specific project
npx smashcut publish --public     # allow anyone with the URL to view the claimed project
npx smashcut publish --yes        # skip the confirmation prompt (scripts/CI)
```
