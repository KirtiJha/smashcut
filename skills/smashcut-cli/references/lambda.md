...
Use `smashcut lambda` when the user explicitly wants self-managed AWS infrastructure or needs distributed rendering. It wraps `@smashcut/aws-lambda` and AWS SAM.
...
```bash
npx smashcut lambda deploy
npx smashcut lambda render ./my-project --width 1920 --height 1080 --wait
npx smashcut lambda destroy
```
...
```bash
npx smashcut lambda deploy \
  --stack-name=smashcut-prod \
  --region=us-east-1 \
  --concurrency=8 \
  --memory=10240
```

Builds `packages/aws-lambda/dist/handler.zip` and SAM-deploys the stack (Lambda + Step Functions + S3 + IAM). Idempotent — re-running on the same `--stack-name` is a no-op when nothing changed. Writes `<cwd>/.smashcut/lambda-stack-<name>.json` so later subcommands don't need to call `describe-stacks`.

| Flag              | Default                         | Description                            |
| ----------------- | ------------------------------- | -------------------------------------- |
| `--stack-name`    | `smashcut-default`           | CloudFormation stack name              |
...
```bash
npx smashcut lambda sites create ./my-project
# → siteId: abc1234deadbeef0  (stable across re-runs of the same tree)

npx smashcut lambda render ./my-project --site-id=abc1234deadbeef0 ...
```
...
```bash
npx smashcut lambda render ./my-project \
...
For variable-driven templates, declare the schema in the composition and pass either `--variables` or `--variables-file`, never both. `--strict-variables` checks local project input before any render starts. Also read [`variables-and-media.md`](../../smashcut-core/references/variables-and-media.md#variables).
...
```bash
npx smashcut lambda render-batch ./template \
  --batch ./users.jsonl \
...
```bash
npx smashcut lambda progress sc-render-abcd1234
npx smashcut lambda progress arn:aws:states:us-east-1:...:execution:...
```
...
```bash
npx smashcut lambda policies user                                  # inline policy for an IAM user
npx smashcut lambda policies role                                  # { TrustRelationship, InlinePolicy }
npx smashcut lambda policies validate ./infra/iam/sc-deploy.json   # CI gate
```
...
`smashcut lambda` stores per-stack metadata under `<cwd>/.smashcut/lambda-stack-<name>.json` (bucket name, state-machine ARN, region). Not secret, but AWS-account-identifying. Commit it to a repo or `.gitignore` it per your workflow.
