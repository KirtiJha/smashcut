...
```bash
npx smashcut lint                  # current directory
npx smashcut lint ./my-project     # specific project
npx smashcut lint --verbose        # info-level findings
npx smashcut lint --json           # machine-readable
```
...
```bash
npx smashcut check                    # current directory: the full browser gate
npx smashcut check ./my-project       # specific project
npx smashcut check --json             # agent-readable envelope {ok, lint, runtime, layout, motion, contrast, snapshots}
npx smashcut check --snapshots        # also write overview frames (annotated) + per-finding crops
npx smashcut check --samples 15       # denser timeline sweep (default 9)
npx smashcut check --at 1.5,4,7.25    # explicit hero-frame timestamps
npx smashcut check --at-transitions   # also sample every tween start/end boundary
npx smashcut check --tolerance 4      # allowed overflow px before reporting (default 2)
npx smashcut check --timeout 30000    # initial render-ready + navigation minimum in ms (defaults: 3000 / 10000)
npx smashcut check --no-contrast      # skip the WCAG audit while iterating
npx smashcut check --strict           # exit non-zero on warnings too (default: only errors)
```
...
```bash
npx smashcut check --caption-zone "x0=0;y0=.82;x1=1;y1=1;severity=error;seek=.25,1"
npx smashcut check --frame-check     # media (img/svg/video/canvas) out-of-frame detection
```
...
```bash
npx smashcut snapshot                       # 5 key frames as PNG
npx smashcut snapshot ./my-project          # specific project
npx smashcut snapshot --frames 10           # evenly-spaced N frames
```
...
`smashcut check --snapshots` already writes a `finding-NN-<code>.png` crop for every error finding that carries a bbox, but the same zoom is available standalone once you know what to look at:

```bash
npx smashcut check --snapshots               # reports a finding, e.g. content_overlap on "#cta"
npx smashcut snapshot --zoom "#cta"           # crop the element to verify the defect, at 3x density
npx smashcut snapshot --zoom "100,50,400,300" --zoom-scale 2   # or an exact pixel region
