...
```bash
npx smashcut doctor
npx smashcut doctor --json     # CI / agent output (always exit 0; gate on payload `ok`)
```
...
- A fresh machine has never run SmashCut.

Common issues:

- **Missing FFmpeg** — install via `brew install ffmpeg` (macOS) or your package manager.
- **Missing bundled Chrome** — run `npx smashcut browser ensure`.
...
- **Chrome exits instantly inside an agent sandbox (macOS)** — seatbelt-style sandboxes
  ...
  headless shell — dies at startup. This is a host-level block, not a SmashCut or
...
```bash
npx smashcut browser ensure    # find or download the pinned Chrome
npx smashcut browser path      # print the browser executable path (for scripting)
npx smashcut browser clear     # remove the cached Chrome download
```

Manage the Chrome build SmashCut uses for rendering. The pinned version exists because pixel output drifts across Chrome versions — using the bundled build keeps rendered output reproducible across machines.

Use `path` to embed the binary in scripts: `$(npx smashcut browser path)`.
