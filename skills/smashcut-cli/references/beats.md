...
Use `smashcut beats` when an existing SmashCut project needs the Studio-compatible beat file for its music track. This is a CLI utility, not a complete video workflow.

```bash
npx smashcut beats
npx smashcut beats ./my-video
npx smashcut beats ./my-video --json
```
