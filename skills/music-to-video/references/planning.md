...
`../smashcut-creative/frame-presets/` using the preset table in
`../smashcut-creative/references/design-spec.md` — choose by the track's mood + the brief,
and **only its fonts + colors matter** (templates own composition + motion; the preset only
sets the look). Copy it in **unmodified**:

```bash
cp ../smashcut-creative/frame-presets/<preset>/FRAME.md "$PROJECT_DIR/frame.md"
```
