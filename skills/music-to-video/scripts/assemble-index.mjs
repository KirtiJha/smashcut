...
//
// Reads:  --storyboard STORYBOARD.md, --smashcut <root>, [--audiomap audiomap.json],
...
const audiomapPath = resolve(flag("audiomap", join(smashcutDir, "audiomap.json")));
...
for (const m of mounted) {
  ...
  if (v?.path && existsSync(join(smashcutDir, v.path))) {
    body.push(
...
if (existsSync(join(smashcutDir, bgmRel))) {
