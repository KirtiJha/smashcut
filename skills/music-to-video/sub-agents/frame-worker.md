...
You build one frame's composition file: `compositions/frames/<frame_id>.html`. Siblings build the other frames in parallel. The generic SmashCut law — sub-composition shape, timeline registration, determinism, layout — lives in `smashcut-core` (`references/sub-compositions.md`
...
- Seek-safe per `smashcut-core/determinism-rules.md` (derive variation from indices; swap text / numbers with `tl.set`).
