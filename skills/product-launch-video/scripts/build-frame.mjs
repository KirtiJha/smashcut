...
//
//   node build-frame.mjs --preset capsule --smashcut .
...
const presetDir = resolve(
  flag("preset-dir", join(__dirname, "../../smashcut-creative/frame-presets")),
);
const tokensPath = resolve(flag("tokens", join(smashcutDir, "capture/extracted/tokens.json")));
...
if (brandFonts.length) {
  ...
  const srcDirs = [
    join(smashcutDir, "capture/assets/fonts"),
    join(smashcutDir, "assets/fonts"),
  ...
  const outDir = join(smashcutDir, "assets/fonts");
  const faces = [];
...
if (existsSync(presetSkin)) {
  const skinDir = join(smashcutDir, ".smashcut");
...
console.log(
  `  .smashcut/caption-skin.html: ${skinCopied ? "copied" : "preset ships none — captions will use the default pill"}`,
);
