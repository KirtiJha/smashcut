...
in an authorized SmashCut analytics project; this doc is the source of truth
...
Events attribute to the **same person as the smashcut CLI and studio**
— the shared install id in `~/.smashcut/config.json` (`anonymousId`), stitched
...
`$ip:null`. Opt-out: `SMASHCUT_NO_TELEMETRY=1` / `DO_NOT_TRACK=1` (also CI, dev).
