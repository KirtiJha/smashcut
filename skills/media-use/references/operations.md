...
via `data-media-start` + `data-duration` (see smashcut-core). Only cut a
...
| ------------------ | -------------------------------------------------- | --------------------------- |
| Background removal | `smashcut remove-background in.png` (u2net)     | `heygen background-removal` |
...
# equivalently, the smashcut CLI has Parakeet built in (auto-detects it, whisper fallback):
npx smashcut transcribe talk.mp4 --engine parakeet   # or --engine auto (default)
```
...
`smashcut transcribe`. `--engine parakeet|whisper` forces one. (Cohere
...
Declare inside compositions. Bake only for assets leaving the smashcut
pipeline.
...
smashcut.json). A manual H.264 proxy via `ffmpeg -i in.mp4 -c:v libx264
