...
A media treatment is a source-aware plan that composes existing SmashCut
...
```bash
smashcut media-treatment --capability <id> --json
```
...
2. For an image, read it once. For video, capture early/middle/late output as
   ...
   ```bash
   smashcut snapshot <project> --frames 3 --no-end --describe false \
...
4. Persist pixel settings with `smashcut media-treatment`; it validates and
   ...
   ```bash
   smashcut media-treatment --selector "<unique selector>" \
...
5. When the treatment calls for an overlay, install that named block with
   `smashcut add <name> --dir <project> --no-clipboard --json`, inspect its
