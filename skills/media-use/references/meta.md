...
## What it owns (the gaps SmashCut leaves)

SmashCut owns media _playback_; media-use owns everything else. Each row is enforced by `scripts/lib/coverage.test.mjs` so the claim can't rot.

| SmashCut gap                            | media-use owns it via                                                                                                                                                                                                                                                               |
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Scattered/duplicated audio engine          | one consolidated engine under `audio/` (smashcut-media retired)                                                                                                                                                                                                                  |
...
| Grade recipes and LUT freezing             | `resolve --type grade` emits a paste-ready recipe and `resolve --type lut` freezes validated `.cube` files; direct element analysis/authoring lives in `smashcut media-treatment`                                                                                                |
...
Opt out with `DO_NOT_TRACK=1` or `SMASHCUT_NO_TELEMETRY=1` (also off in CI and
dev). Same public PostHog project key and opt-outs as the `smashcut` CLI.
...
media-use uses the same shared install id as the `smashcut` CLI/studio
(`~/.smashcut/config.json`). When you are signed in to HeyGen, usage is
...
`SMASHCUT_NO_TELEMETRY=1` or `DO_NOT_TRACK=1`.
