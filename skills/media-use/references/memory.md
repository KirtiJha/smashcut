...
```bash
node <SKILL_DIR>/scripts/prefs.mjs get --smashcut . --json      # merged view (project overrides user)
node <SKILL_DIR>/scripts/prefs.mjs record --smashcut . --key destination --value x-feed
node <SKILL_DIR>/scripts/prefs.mjs record --smashcut . --key style_preset --value pin-and-paper --workflow faceless-explainer
```

Only what the user actually confirmed gets recorded — never an inferred or defaulted value. How workflows consume these (a remembered value becomes the recommended default with a receipt, and never skips a question) is the brief contract's rule: `smashcut-core/references/brief-contract.md` § 2, Remembered defaults.
...
```bash
node <SKILL_DIR>/scripts/recipe.mjs freeze --smashcut . --name weekly-promo   # workflow read from BRIEF.md (--workflow only for briefless projects)
node <SKILL_DIR>/scripts/recipe.mjs list --smashcut . --workflow product-launch-video
node <SKILL_DIR>/scripts/recipe.mjs use --smashcut . --name weekly-promo   # also: resolve.mjs --type recipe --entity weekly-promo
```

The freeze is offered once after the final approval (`smashcut-core/references/review-loop.md` § 4), and the intent layer (`/smashcut` → `references/intent-interview.md`, step 1) checks for a match before its first question. Adopting a recipe fills the brief, the design spec, and the storyboard skeleton — and unlike preferences it may skip the questions it answers: the bundle was approved as a whole, and adoption itself is the question.
