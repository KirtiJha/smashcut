...
export async function resolveSfx({ cues, heygenOK, headers, smashcutDir, sfxLibDir }) {
  const sfx = [];
  const anomalies = [];
  const destDir = join(smashcutDir, "assets", "sfx");
  ...
  if (heygenOK) {
    for (const { id, name } of uniq) {
      try {
        ...
        await downloadTo(top.audio_url, join(smashcutDir, file));
        sfx.push({
          id,
          name,
          file,
  ...
  for (const { id, name } of uniq) {
    ...
    const dest = join(smashcutDir, destRel);
