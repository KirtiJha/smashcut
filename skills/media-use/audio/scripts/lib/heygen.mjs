...
// pipeline. The credential resolver matches the smashcut CLI auth: first
// usable source wins — $HEYGEN_API_KEY / $SMASHCUT_API_KEY → a nearby .env → ~/.heygen/
...
export function heygenCredential() {
  const envKey = process.env.HEYGEN_API_KEY || process.env.SMASHCUT_API_KEY;
...
export function heygenAuthHeaders() {
  ...
  if (cred?.expired)
    throw new Error(
      "HeyGen OAuth token expired — run `npx smashcut auth refresh` (or `npx smashcut auth login`)",
    );
  throw new Error(
    "no HeyGen credentials — set $HEYGEN_API_KEY, or run `npx smashcut auth login` (writes ~/.heygen/credentials)",
  );
}
