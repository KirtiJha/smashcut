...
```bash
npx smashcut remove-background subject.mp4 -o transparent.webm          # default: VP9 + alpha
npx smashcut remove-background subject.mp4 -o transparent.mov           # ProRes 4444 (editing)
npx smashcut remove-background portrait.jpg -o cutout.png               # single-image cutout
npx smashcut remove-background subject.mp4 -o subject.webm \
  --background-output plate.webm                                           # both layers, one pass
npx smashcut remove-background subject.mp4 -o transparent.webm --device cpu
npx smashcut remove-background --info                                   # detected providers
```
...
`auto` (default) picks CoreML on Apple Silicon, CUDA when available, otherwise CPU. Force with `--device cpu | coreml | cuda`. CUDA requires `SMASHCUT_CUDA=1` plus a GPU-enabled `onnxruntime-node` build. Use `--info` to inspect detected providers without rendering.
...
<h1 id="headline" style="z-index:2; ...">MAKE IT IN SMASHCUT</h1>
