#!/usr/bin/env node
/**
 * Recipes CLI — the heavyweight tier of SmashCut user memory.
 *
 *   node recipe.mjs freeze --smashcut . --name <n> [--workflow <w>] [--blocks a,b,c]
 ...
 *
 *   node recipe.mjs list --smashcut . [--workflow <w>] [--json]
 *     Two-tier merged listing (project wins), newest approval first.
 *
 *   node recipe.mjs use --smashcut . --name <n> [--json]
 ...
 * layer's business — see smashcut-core/references/review-loop.md § 4 and
 * the intent layer's recipe check (smashcut SKILL.md § 4).
 */
...
try {
  if (verb === "freeze") {
    ...
    const list = listRecipes({ projectDir: args.smashcut, workflow: args.workflow });
    ...
    const used = useRecipe({ projectDir: args.smashcut, name: args.name });
    ...
    fail(
      "usage: recipe.mjs <freeze|list|use> --smashcut . [--name <n>] [--workflow <w>] [--blocks a,b] [--json]",
    );
  }
} catch (err) {
