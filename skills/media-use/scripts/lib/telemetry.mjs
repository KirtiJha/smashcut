// Opt-out usage tracking for media-use, sharing the smashcut CLI/studio
// identity (packages/cli/src/telemetry): the same install id from
// ~/.smashcut/config.json, plus a $identify to the HeyGen account on sign-in,
...
// to ship), same opt-outs (DO_NOT_TRACK / SMASHCUT_NO_TELEMETRY / CI / dev),
...
// CLI + studio share one install identity in ~/.smashcut/config.json
...
function sharedConfigPath() {
  return join(homedir(), ".smashcut", "config.json");
}
...
function writeSharedConfig(config) {
  const dir = join(homedir(), ".smashcut");
  ...
  try {
    ...
    console.error(
      [
        ...
        "If you sign in to HeyGen, usage links to your account email or username. Opt out with SMASHCUT_NO_TELEMETRY=1 or DO_NOT_TRACK=1.",
      ].join("\n"),
    );
