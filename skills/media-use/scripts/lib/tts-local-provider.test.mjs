...
test("win32: routes the smashcut tts call through node + npx-cli, never bare npx", async () => {
  const captured = [];
  ...
  assert.deepEqual(captured[0].args.slice(1, 4), ["smashcut", "tts", "hello there"]);
...
test("non-win32: spawns plain npx unchanged", async () => {
  ...
  assert.deepEqual(captured[0].args.slice(0, 3), ["smashcut", "tts", "hola"]);
