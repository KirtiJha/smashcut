#!/usr/bin/env node
/**
 * Remembered defaults CLI — the lightweight tier of SmashCut user memory.
 *
 *   node prefs.mjs get --smashcut . [--json]
 ...
 *
 *   node prefs.mjs record --smashcut . --key destination --value x-feed [--workflow <w>]
 ...
 *
 * Consumption rules live in smashcut-core/references/brief-contract.md § 2
...
if (verb === "get") {
  const merged = mergedPreferences(args.smashcut);
  if (args.json) {
  ...
  fail(
    "usage: prefs.mjs <get|record> --smashcut . [--key <k> --value <v> --workflow <w>] [--json]",
  );
}
