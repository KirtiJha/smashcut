---
name: smashcut-three
description: Three.js and WebGL adapter patterns for SmashCut. Use when creating deterministic Three.js scenes, WebGL canvas layers, AnimationMixer timelines, camera motion, shader-driven visuals, or canvas renders that respond to SmashCut sc-seek events.
---

# Three.js for SmashCut

SmashCut supports Three.js through its `three` runtime adapter. The adapter does not own your scene. It publishes SmashCut time and dispatches a seek event so your composition can render the exact frame.

## Contract

- Create the scene, camera, renderer, materials, and assets synchronously when possible.
- Render from SmashCut time, not wall-clock time.
- Listen for the `sc-seek` event and render exactly that time.
...
- **Always set `data-duration="<seconds>"` on the root `[data-composition-id]` element.** Unlike CSS/WAAPI/Lottie, the `three` adapter has no duration auto-inference — it only forwards time via `sc-seek`/`__scThreeTime`, it doesn't inspect your scene for an `AnimationClip`/`AnimationMixer` length. Without `data-duration` (and no GSAP timeline), the render engine has no way to know how long to capture and fails with "Composition has zero duration". `npx smashcut lint` errors on this (`root_composition_missing_duration_source`).

The adapter sets `window.__scThreeTime` and dispatches `new CustomEvent("sc-seek", { detail: { time } })` on each seek.
...
<script type="module">
  ...
  window.addEventListener("sc-seek", (event) => {
    renderAt(event.detail.time);
  });

  renderAt(window.__scThreeTime || 0);
</script>
```
...
For anything under `three/addons/`, use an importmap so bare specifiers resolve. The SmashCut lint recognizes both this form and the inline `+esm` import above — pick whichever your composition needs.
...
- SmashCut adapter source: `packages/core/src/runtime/adapters/three.ts`.
