---
name: smashcut-animejs
description: Anime.js adapter patterns for SmashCut. Use when writing Anime.js animations or timelines inside SmashCut compositions, registering animations on window.__scAnime, making Anime.js seek-driven and deterministic, or translating Anime.js examples into render-safe SmashCut HTML.
---

# Anime.js for SmashCut

SmashCut can seek Anime.js instances through its `animejs` runtime adapter. The composition owns the animation objects; SmashCut owns the clock.
...
- Register every returned animation or timeline on `window.__scAnime` — **explicitly. There is no working auto-discovery on v4** (see Avoid).
...
The adapter seeks every registered instance with `instance.seek(timeMs)`, where `timeMs` is SmashCut time **in milliseconds** (`ctx.time` seconds × 1000). It also calls `pause()` and `play()` on each instance; anything exposing those three methods works, whatever created it.
...
Use GSAP for complex scene sequencing unless the user specifically asks for Anime.js. GSAP is still the primary SmashCut authoring path.
...
```bash
npx smashcut lint
npx smashcut validate
```

## Credits And References

- SmashCut adapter source: `packages/core/src/runtime/adapters/animejs.ts`.
