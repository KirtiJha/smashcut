---
name: smashcut-lottie
description: Lottie and dotLottie adapter patterns for SmashCut. Use when embedding lottie-web JSON animations, .lottie files, @lottiefiles/dotlottie-web players, registering instances on window.__scLottie, or making After Effects exports deterministic in SmashCut.
---

# Lottie for SmashCut

SmashCut can seek both `lottie-web` and dotLottie players through its `lottie` runtime adapter. Lottie is a strong fit because the animation timeline is already encoded in the asset; SmashCut only needs a player object it can seek.
...
- Register every returned animation or player on `window.__scLottie`.
...
```js
window.__scLottie = window.__scLottie || [];
window.__scLottie.push(backgroundAnim);
window.__scLottie.push(iconAnim);
window.__scLottie.push(confettiAnim);
```

SmashCut seeks them all to the same composition time.

## Composition Duration

The render engine needs the composition's total length. GSAP timelines report duration automatically; a Lottie-only composition has no timeline object, so the runtime reads the registered animation's native length directly — `totalFrames / frameRate` for `lottie-web`, or the player's own `duration` for dotLottie. `data-duration` on the root element is optional for Lottie compositions: as long as every animation is registered on `window.__scLottie` (per the contract above), the runtime has a finite duration to work with even when you set `loop: true`.
...
- Translating Remotion Lottie usage into plain SmashCut HTML.
...
- Loading a player asynchronously and registering it after SmashCut validation has already inspected the page.
...
- SmashCut adapter source: `packages/core/src/runtime/adapters/lottie.ts`.
