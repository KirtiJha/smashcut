# GSAP Effects for SmashCut

Drop-in animation patterns. Snippets show mechanism only, inside a standard scene clip (smashcut-core); assume `tl` exists.
...
```bash
python skills/smashcut-creative/scripts/extract-audio-data.py audio.mp3 -o audio-data.json
python skills/smashcut-creative/scripts/extract-audio-data.py video.mp4 --fps 30 --bands 16 -o audio-data.json
```
...
**Do NOT use async `fetch()`** — SmashCut reads `window.__timelines` synchronously after page load; building the timeline inside `.then()` means it isn't ready when capture starts.
...
WebGL / Three.js: SmashCut patches `THREE.Clock` for deterministic time — update uniforms from audio data each frame. DOM elements: fine under ~20 elements, slower than Canvas beyond that.
