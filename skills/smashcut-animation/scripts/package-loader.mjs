...
//   • requires an interactive y/N (or an explicit $SMASHCUT_SKILL_BOOTSTRAP_DEPS=1)
...
const VERSION_OVERRIDE_ENV = "SMASHCUT_SKILL_PKG_VERSION";
const BOOTSTRAP_ENV = "SMASHCUT_SKILL_DEPS_BOOTSTRAPPED";
const BOOTSTRAP_CONFIRM_ENV = "SMASHCUT_SKILL_BOOTSTRAP_DEPS";
const NODE_MODULES_ENV = "SMASHCUT_SKILL_NODE_MODULES";
...
export async function bundleCompositionForCapture(compiler, projectDir) {
  const compiledDir = mkdtempSync(join(tmpdir(), "smashcut-skill-bundle-"));
  try {
...
// @smashcut/producer predates the isTransientBrowserError re-export. The
 ...
 * @smashcut/producer): on a transient failure, close the crashed session
 ...
 * "Runtime ready: false").
 *
 * `producer` is the imported @smashcut/producer namespace;
...
export function smashcutPackageSpec(packageName) {
  const override = process.env[VERSION_OVERRIDE_ENV]?.trim();
  if (override) return `${packageName}@${override}`;

  const version = readBundledSmashcutVersion();
  if (version) return `${packageName}@${version}`;

  // Global skill installs have no smashcut package.json
  ...
  process.stderr.write(
    [
      `smashcut: could not determine the bundled version for ${packageName}; using @latest.`,
...
function resolvePackageEntry(packageName) {
  ...
  for (const base of bases) {
    ...
    try {
      return createRequire(join(normalized, "__smashcut_skill_loader__.cjs")).resolve(
        packageName,
      );
    } catch {
...
function readBundledSmashcutVersion() {
...
function readPackageVersion(packageJsonPath) {
  try {
    ...
    if (manifest.name === "smashcut" || manifest.name === "@smashcut/cli") {
...
async function confirmBootstrap(packageSpecs) {
  ...
  try {
    const answer = await rl.question(
      [
        "SmashCut helper package(s) are missing.",
...
function bootstrapWithNpmInstall(packageNames) {
  const installRoot = mkdtempSync(join(tmpdir(), "smashcut-skill-deps-"));
  const npmArgs = [
    "install",
    "--silent",
    "--no-audit",
