...
const HELPERS = [
  join(REPO_ROOT, "skills", "smashcut-animation", "scripts", "animation-map.mjs"),
  join(REPO_ROOT, "skills", "smashcut-creative", "scripts", "contrast-report.mjs"),
];

describe("SmashCut skill helpers", () => {
  for (const helper of HELPERS)
    it(`${helper.split("/").at(-1)} bundles modular input and uses rational fps`, () => {
      const root = mkdtempSync(join(tmpdir(), "smashcut-skill-helper-test-"));
      const packageDir = join(root, "node_modules", "@smashcut", "producer");
      const corePackageDir = join(root, "node_modules", "@smashcut", "core");
      ...
      writeFileSync(
        ...
        JSON.stringify({ name: "@smashcut/producer", type: "module", exports: "./index.mjs" }),
      );
      writeFileSync(
      ...
      try {
        const result = spawnSync(
          ...
          [helper, compositionDir, "--fps", "30000/1001", "--out", join(root, "output")],
          {
            ...
            env: {
              ...process.env,
              SMASHCUT_SKILL_NODE_MODULES: join(root, "node_modules"),
            },
          },
        );
...
describe("package-loader parity", () => {
  it("package-loader.mjs is byte-identical to smashcut-creative's copy (the stated contract)", () => {
    const here = readFileSync(
      join(REPO_ROOT, "skills", "smashcut-animation", "scripts", "package-loader.mjs"),
      "utf8",
    );
    const sibling = readFileSync(
      join(REPO_ROOT, "skills", "smashcut-creative", "scripts", "package-loader.mjs"),
      "utf8",
    );
...
function runHelper(helper, root, compositionDir) {
  const result = spawnSync(process.execPath, [helper, compositionDir, "--out", join(root, "out")], {
    encoding: "utf8",
    env: { ...process.env, SMASHCUT_SKILL_NODE_MODULES: join(root, "node_modules") },
  });
...
describe("transient-init retry", () => {
  for (const helper of HELPERS) {
    it(`${helper.split("/").at(-1)} retries a transient zero-duration init once with a fresh session`, () => {
      const root = mkdtempSync(join(tmpdir(), "smashcut-skill-retry-test-"));
      try {
...
describe("proxy-driver tweens", () => {
  it("counts an onUpdate driver's span instead of reporting it as a dead zone", () => {
    const root = mkdtempSync(join(tmpdir(), "smashcut-skill-proxy-test-"));
    try {
  ...
  it("inherits a driver the TIMELINE owns, without counting a spacer under it", () => {
    const root = mkdtempSync(join(tmpdir(), "smashcut-skill-parent-driver-test-"));
    try {
