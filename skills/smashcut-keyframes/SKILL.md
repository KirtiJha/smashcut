---
name: smashcut-keyframes
description: >
  Use when a SmashCut composition needs a punch-in, punch-out, zoom, reframe,
  ...
  masks, SVG morph/draw, text trails, 3D depth, or `smashcut keyframes` diagnostics.
...
Use `smashcut-animation` for broad scene recipes. Use `smashcut-cli` for full command docs. Use `references/keyframe-patterns.md` only when choosing implementation mechanisms, not visual style.

## Creator editing boundary

Keyframes own visual motion, not clip assembly. Source-range hard cuts, trim,
splice, and reorder belong to `/smashcut-core`: author one media element per
...
| Match cut or whip pan                   | `/smashcut-animation` owns the visual handoff; `/smashcut-registry` supplies primitives; keyframes preserve authored geometry, direction, and velocity. There is no automatic matching-frame discovery.               |
...
| Directional wipe cut or iris/reveal cut | Animate a mask/clip boundary across overlapping visual clips; `/smashcut-animation` owns the handoff choreography.                                                                                                       |
...
| Constant source retime                  | `/smashcut-core` owns normalized `data-playback-rate` (`0.1..5`) for render-safe picture and pitch-preserved sound. It is constant for the whole media element.                                                          |
...
When editing picture and sound together, load `/smashcut-core`, this skill for
visual motion, and `/smashcut-audio` for fades, crossfades, volume automation,
ducking/carve, or effects on the placed tracks.

A visual transition or cropping treatment is not a temporal source trim or
splice. `/smashcut-core` owns the timeline, clip timing, and source ranges;
keyframes only animate the visible handoff or crop on wrappers inside those clips.
For copyable combined picture/sound recipes, use `/smashcut-core` → `references/creator-editing-recipes.md`.
...
4. Verify with `smashcut lint`, `smashcut check`, `smashcut keyframes`, one focused `--shot`, and snapshots at proof times.
...
- push every instance to `window.__scAnime`
...
Keyframe camera position, camera target, object transform, material opacity, shader uniforms, and postprocess intensity through deterministic state. Render from SmashCut time. Use `--ghost` because marker boxes cannot see internal canvas motion.
...
npx smashcut lint
npx smashcut check
npx smashcut keyframes .
npx smashcut keyframes . --json
npx smashcut keyframes . --runtime all
npx smashcut keyframes . --selector "<selector>" --shot "<file>" --samples <n>
npx smashcut keyframes . --selector "<selector>" --shot "<file>" --layout strip --from <t0> --to <t1>
npx smashcut keyframes . --shot "<file>" --ghost --angle <angle>
npx smashcut snapshot . --at <times>
```
...
Run `smashcut lint`, `smashcut check`, `smashcut keyframes`, one focused `--shot`, and snapshots. Confirm first frame, proof poses, final-minus-hold, exact final, subject-owned motion, and no debug overlays.
