...
4. Registers the player on `window.__scRemotion` with `seekTo(frame)`,
...
> 2. Mount the Player and register on `window.__scRemotion`:
>
>    ```tsx
>    const playerRef = useRef<PlayerRef>(null);
>    useEffect(() => {
>      playerRef.current?.pause();
>      window.__scRemotion = window.__scRemotion || [];
>      window.__scRemotion.push({
...
{
  "rule": "r2hf/use-state",
  "message": "useState detected — Remotion compositions that drive animation via React state are not deterministic frame-capture targets in SmashCut",
