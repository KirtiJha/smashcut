...
[Lottie adapter](https://github.com/KirtiJha/smashcut/blob/main/packages/core/src/runtime/adapters/lottie.ts)
that supports both `lottie-web` and `@lottiefiles/dotlottie-web`. The
adapter auto-discovers animations registered on `window.__scLottie`
...
- `window.__scLottie.push(anim)` is what hooks the animation into HF's
  per-frame seek
...
1. Copy `hello.json` from the Remotion project into `sc-src/assets/`.
...
onto `window.__scLottie` and the adapter will seek all of them in sync:

```js
window.__scLottie.push(anim1);
window.__scLottie.push(anim2);
window.__scLottie.push(anim3);
```
...
Per the [Lottie adapter](https://github.com/KirtiJha/smashcut/blob/main/packages/core/src/runtime/adapters/lottie.ts)
