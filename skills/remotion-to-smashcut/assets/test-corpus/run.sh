...
#   - HF CLI built at packages/cli/dist/cli.js (run `bun run --filter @smashcut/cli build`
...
require_render_tier_tools() {
  if [[ ! -f "$HF_CLI" ]]; then
    ...
    echo "       Run 'bun run --filter @smashcut/cli build' in $REPO_ROOT" >&2
    return 2
  fi
...
run_render_tier() {
  ...
  if ! (cd "$fixture_dir" && \
        node "$HF_CLI" render sc-src/ --output hf.mp4 --quiet >/dev/null 2>&1); then
    echo "    ✗ HF render failed"
    write_result "$fixture_name" "fail" stage "sc-render"
    return 0
  fi
...
echo "remotion-to-smashcut corpus run"
echo "=================================="
