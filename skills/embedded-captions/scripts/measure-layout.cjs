...
// Locate smashcut' bundled puppeteer. render-and-composite.sh exports
// SMASHCUT_ROOT; standalone we also try the in-repo path + ~/Downloads, and
...
if (!puppeteer) {
  console.error(
    "[measure] could not locate puppeteer — set SMASHCUT_ROOT to a built smashcut checkout",
  );
  process.exit(3);
}

// Resolve smashcut' bundled GSAP. The templates load GSAP from a CDN
