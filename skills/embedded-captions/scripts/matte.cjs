#!/usr/bin/env node
/*
 * matte.cjs — subject matting via smashcut' built-in `remove-background`
 ...
 * weights — the model auto-downloads once (~168 MB) to ~/.cache/smashcut/.
 *
 ...
 *
 * Pipeline:
 *   source.mp4 → smashcut remove-background → ProRes 4444 .mov (lossless
 ...
 * Env:    SMASHCUT_ROOT — smashcut checkout (default ~/Downloads/smashcut)
 */
...
function hfCli() {
  ...
  console.error("[matte] cannot find smashcut cli — set SMASHCUT_ROOT to a built checkout");
  process.exit(3);
}
...
async function main() {
  ...
  // 1) subject matte via smashcut (ProRes 4444 keeps the alpha lossless).
  ...
  console.log(
    `[matte] smashcut remove-background (u2net_human_seg${cached ? "" : "; first run downloads ~168 MB"})… model load takes ~1-2 min with no output — not hung`,
  );
