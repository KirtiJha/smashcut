 ...
 *
 * smashcut' renderer only auto-supplies its ~18 CANONICAL_FONTS. Every other
 ...
 * On a clean/offline/CI machine it silently degrades. (smashcut' own font
 ...
 * and inlines just those @font-face blocks into a <style id="sc-embedded-fonts">.
 * Idempotent (re-running replaces the block). Families already declared in the
 * HTML, smashcut-canonical families, and CSS generics are left untouched.
 *
...
const MARKER = "sc-embedded-fonts";
...
// inline style="") and the data-font-family attribute smashcut also honors.
