 ...
 * deterministic on ANY machine: smashcut' renderer only auto-supplies its
 * ~18 CANONICAL_FONTS; every other family falls back to a generic font unless
 * the HTML ships a local @font-face. (smashcut' own font linter says exactly
...
const header =
  ...
  `   These are the template fonts NOT in smashcut' auto-resolved set; inlining\n` +
