...
`smashcut check` is not a visual gate. It passes compositions that render nothing: a blank plot
...
3. **Load GSAP and register a paused root timeline**, then snapshot with
   `smashcut snapshot . --at 0.05,1.2,2.5,4.0 --no-end` and read the contact sheet.
...
slop; the SmashCut wordmark in `logo-brand-close` is the subject. Judge the placeholder content,
