...
- **Block** (`registry/blocks/`, type `smashcut:block`) — a full standalone composition with fixed dimensions and duration. Caption styles, VFX effects, title cards, lower thirds.
- **Component** (`registry/components/`, type `smashcut:component`) — a reusable snippet with no fixed dimensions or duration. CSS effects, text treatments, overlays that adapt to any composition size.
...
- Overflow: call `window.__smashcut.fitTextFontSize()` on every group
...
```bash
smashcut lint                    # 0 errors required
smashcut check --no-contrast  # 0 console errors required
```
...
#    no longer exists is worse, because `smashcut add <name>` resolves the
...
# 5. Publish to smashcut.dev so reviewers can preview
npx smashcut publish
...
# 8. Push and open PR with smashcut.dev link
git push origin feat/registry-{name}
gh pr create --title "feat(registry): {name}" --body "preview: {smashcut.dev-url}"
```
...
- [ ] `smashcut lint` → 0 errors
- [ ] `smashcut check` → 0 console errors
...
- [ ] `npx smashcut publish` run (claim your project URL)
