...
1. **Snapshots** — run `npx smashcut snapshot --at <proof-times>` and inspect overflow, off-canvas content, text collisions, empty frames, wrong content, and unreadable motion.
...
3. **Recheck** — rerun `npx smashcut lint`, `npx smashcut check`, and the affected snapshots. Return the result to the orchestrator without rendering.
