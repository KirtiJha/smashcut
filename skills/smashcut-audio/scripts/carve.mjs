 ...
 * panel to click — identical functions from `@smashcut/core`, identical
 ...
 * room for both.
 *
 * Needs `ffmpeg` on PATH (to decode the audio) and `@smashcut/core` resolvable
 * from the composition's project (`npm i -D @smashcut/core`) — the CLI bundles
...
const usage = `carve.mjs --comp <file.html> [--bed <elementId>] [--voice <elementId> ...]
  ...
  --core      directory to resolve @smashcut/core from (default: the comp's)`;
...
/**
 * Load the carve analysis out of `@smashcut/core`.
 *
...
export async function loadCore(fromDir) {
   ...
   *      the skill was installed, which has no @smashcut/core; the
   *      composition's project is what holds the dependency. So a bare
   *      `import("@smashcut/core/audio-carve")` from here cannot work — bare
  ...
  const load = async (subpath) => {
    const spec = `@smashcut/core/${subpath}`;
    try {
      ...
      const pkgPath = require.resolve("@smashcut/core/package.json");
      const pkg = JSON.parse(readFileSync(pkgPath, "utf-8"));
      const entry = pkg.exports?.[`./${subpath}`];
      const target = typeof entry === "string" ? entry : (entry?.import ?? entry?.default ?? null);
      if (!target) {
        fail(
          `@smashcut/core does not export ./${subpath}\n` +
            `  found at: ${pkgPath} (version ${pkg.version})\n` +
            `  update it:  npm i -D @smashcut/core`,
        );
      }
  ...
  try {
    ...
    fail(
      `cannot load @smashcut/core from ${fromDir}\n` +
        `  is it installed there?  npm i -D @smashcut/core\n` +
        `  or point at one:   --core <dir containing node_modules/@smashcut/core>\n` +
