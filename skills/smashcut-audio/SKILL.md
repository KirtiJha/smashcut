...
name: smashcut-audio
description: >
  Use when audio already placed in a SmashCut composition needs to be mixed:
  ...
  for several tracks at once (`<sc-audio-group>`).
...
Clip timing remains `/smashcut-core`: audio/video trims and source ranges use
...
no rate envelope; preprocess a derived synchronized asset. SmashCut does not
provide automatic waveform sync or drift correction.
For copyable cut/crossfade/retime recipes, use `/smashcut-core` → `references/creator-editing-recipes.md`.

Three attributes carry everything, on the audio/video element itself — or, for
the first two, on an `<sc-audio-group>` bus (see "One bus for many tracks"):
...
`<sc-audio-group>` element with that id and the group becomes a real submix bus:
...
on PATH and `@smashcut/core` installed in the project (`npm i -D
@smashcut/core`) — the CLI inlines core rather than shipping it, so it cannot
