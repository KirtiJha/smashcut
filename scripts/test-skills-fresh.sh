...
#      package.json (`file:` CLI dep). It does NOT init a smashcut project here
#      — the video workflows run `npx smashcut init` inside their own subdirs.
...
#      ~/.npm/_npx cache, so `npx smashcut` resolves to the LOCAL build from any
...
# WORKSPACE ROOT, not a smashcut project: the video workflows run
# `npx smashcut init` inside their own subdirs, so a project at the root would
# make a skill find a stray composition here. We only need a package.json with the
# `file:` CLI dep so `npx smashcut` (and the skills' init/render calls from
...
cat > package.json <<JSON
{
  ...
  "type": "module",
  "dependencies": {
    "smashcut": "file:$HF_CLI_PKG"
  }
}
JSON

ok "package.json points smashcut → file:$HF_CLI_PKG"
...
[[ -x "node_modules/.bin/smashcut" ]] || fail "node_modules/.bin/smashcut missing after install."
ok "node_modules/.bin/smashcut → local CLI"

# --------- step 4.5: sandbox-private npm global prefix (npx-leak guard) ---------
# The file: dep only covers `npx smashcut` run somewhere UNDER $TEST_DIR — npx
...
[[ -x "$NPM_GLOBAL_PREFIX/bin/smashcut" ]] || fail "$NPM_GLOBAL_PREFIX/bin/smashcut missing after install."
ok "npx smashcut now resolves to the local build from ANY directory (launch with the env below)"
...
WORKFLOWS=(product-launch-video faceless-explainer embedded-captions \
           ...
           remotion-to-smashcut slideshow)
DOMAIN=(smashcut-core smashcut-creative smashcut-animation smashcut-cli media-use smashcut-registry)
