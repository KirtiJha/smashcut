...
function writeConsumerFixture(packDir, packedWorkspaces) {
    ...
    JSON.stringify(
      {
        ...
        type: "module",
        dependencies,
        // Each packed tarball pins its inter-@smashcut deps to the exact
        ...
        // every @smashcut/* to the sibling local tarball so the check is
  ...
  writeFileSync(
    ...
    `import { existsSync } from "node:fs";\n` +
      ...
      `}\n` +
      `const terraform = await import("@smashcut/gcp-cloud-run/terraform");\n` +
...
function verifyPackedConsumer(packDir, packedWorkspaces) {
  ...
  const cliOutput = execFileSync(
    join(fixtureDir, "node_modules", ".bin", "smashcut"),
    ["--help"],
    {
      cwd: fixtureDir,
      encoding: "utf8",
      env: { ...process.env, SMASHCUT_TELEMETRY_DISABLED: "1" },
    },
  );
  if (!cliOutput.toLowerCase().includes("smashcut")) {
    throw new Error("Packed CLI help did not identify SmashCut");
  }
...
function main() {
  const packDir = mkdtempSync(join(tmpdir(), "smashcut-pack-"));
  try {
