...
`smashcut` repo is opened as the working directory. They are NOT part of
...
- `skills/` — **marketplace-distributable**, installed into other projects
  via `npx smashcut skills` or `npx skills add heygen-com/hyperframes`.
...
3. Accept, and the agent produces `weekly-changelog-<range>.mp4` gated by
   `smashcut check` (0 errors) + `seam-gate verify` (0 fail/warn).

TTS uses the tracked `skills/smashcut-media/scripts/heygen-tts.mjs`
...
- A headless Chrome for SmashCut rendering (`smashcut doctor` will
