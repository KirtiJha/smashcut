...
`config.json` in `~/.smashcut` — not on the telemetry id. So a full reset is
one command:

```bash
rm -rf ~/.smashcut    # clears telemetry id, canary cohorts, and breaker state
```

`smashcut telemetry status` prints both paths if you want to inspect or
