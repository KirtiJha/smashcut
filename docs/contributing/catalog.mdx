...
description: Add a reusable block or component to the SmashCut registry.
---

The Catalog is generated from the registry in the SmashCut repository. You
...
Open a [GitHub issue](https://github.com/KirtiJha/smashcut/issues) with a
...
Ask your agent to use the `/smashcut-registry` skill, or follow the same
workflow below.
...
[registry item schema](https://github.com/KirtiJha/smashcut/blob/main/packages/core/schemas/registry-item.json).
...
```json
{
  ...
  "url": "https://static.heygen.ai/smashcut-oss/registry-assets/deadbeefdeadbeef.jpg",
...
so your composition HTML, the Catalog preview renderer and `smashcut add`
...
`npx smashcut lint` cannot validate a registry directory, because the CLI
...
```bash
npx smashcut init scratch && cd scratch
npx smashcut add my-block
npx smashcut check
```
...
from the registry URL in `smashcut.json`. Test `npx smashcut add my-block`
...
- a `smashcut.dev` preview from `npx smashcut publish`;
