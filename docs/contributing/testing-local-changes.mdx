...
description: Run an unreleased SmashCut CLI build against a real project outside the monorepo.
---

Package tests do not prove that the CLI behaves correctly in an ordinary video project. After the focused tests pass, run the changed command against a project outside the SmashCut repository.
...
The resolved binary should point into the local SmashCut checkout. Run it from a separate project:
...
your SmashCut checkout first (an absolute path, since you are now inside the
video project):

```bash
cd /path/to/smashcut/packages/cli
bun unlink
```
...
```bash
node /path/to/smashcut/packages/cli/dist/cli.js preview /path/to/a/video-project
```
...
If a globally installed CLI shadows the link, inspect `which smashcut`, remove the global package, and link again. Preview starts at port `3002` and automatically tries a free port; pass `--port` only when a fixed port is required.
