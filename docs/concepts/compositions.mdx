---
title: Compositions
description: "The fundamental building block of a Smashcut video."
---
...
<Tabs>
  <Tab title="External file">
    ...
    can preserve it explicitly with `data-sc-media-start-basis="global"`; new
...
its own text and colors. SmashCut does not wire `data-var-*` attributes into
...
3. Read them inside the composition with
   `window.__smashcut.getVariables()`, which layers the host's values over
...
<html data-composition-variables='[
  ...
  <body>
    <div data-composition-id="card" data-width="1920" data-height="1080">
      ...
      <script>
        const { title, color } = __smashcut.getVariables();
...
`@smashcut/core`, `extractCompositionMetadata()` reads the same
