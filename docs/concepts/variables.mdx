...
{"id":"appName","type":"string","label":"App name",
 ...
 "portrays":["subject_name"],"default":"SmashCut"}
```
...
Use `window.__smashcut.getVariables()` only when the result needs conditions,
loops, or derived values:

```js
const { featured = false } = window.__smashcut.getVariables();
