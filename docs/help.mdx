...
description: "Start with what you can see, try the smallest useful fix, and get back to your SmashCut task."
---
...
1. Open a terminal in the SmashCut project folder.
2. Run `npx smashcut preview`.
...
That distinction separates a timeline mistake from non-deterministic animation. SmashCut animation must be seekable: the same frame should look the same every time it is requested.

Run `npx smashcut check`, then give your agent the smallest affected scene and the exact timestamp.
...
<CardGroup cols={2}>
  ...
  <Card title="Search GitHub issues" icon="github" href="https://github.com/KirtiJha/smashcut/issues">
