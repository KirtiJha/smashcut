---
title: "How SmashCut fits into an application"
...
Building on SmashCut means using an editable HTML composition as the shared source for automation, playback, editing, and rendering.
...
playback. When the composition uses an animation runtime, SmashCut seeks its
...
```bash
npx smashcut check
npx smashcut render --output video.mp4
```
...
`@smashcut/sdk` opens composition HTML, gives elements stable `data-sc-id` identifiers, applies typed edit operations, emits JSON patches, supports undo and redo, and persists through adapters.

```ts
import { openComposition } from "@smashcut/sdk";

const html = `<div class="clip" data-start="0" data-duration="5"
  data-sc-id="sc-title">Old title</div>`;

const comp = await openComposition(html);
comp.setText("sc-title", "A new title");
...
`@smashcut/player` is a web component that loads a composition in an isolated iframe and exposes video-like playback controls.

```html title="index.html"
<script
  type="module"
  src="https://cdn.jsdelivr.net/npm/@smashcut/player"
></script>
