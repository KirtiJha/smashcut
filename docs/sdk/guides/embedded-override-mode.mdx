...
const storedOverrides = {
  "sc-title.text": "Acme Corp Launch",
  "sc-logo.attr.src": "/customers/acme/logo.png",
  "sc-subtitle.style.color": "#0EA5E9",
};
...
```typescript
comp.setText("sc-cta", "Get started free");
comp.setStyle("sc-cta", { backgroundColor: "#22C55E" });

const nextOverrides = comp.getOverrides();
// { "sc-title.text": "Acme Corp Launch", ..., "sc-cta.text": "Get started free", ... }
...
| Key | Meaning |
|-----|---------|
| `"sc-title.text"` | Inner text of element `sc-title` |
| `"sc-logo.attr.src"` | `src` attribute on `sc-logo` |
| `"sc-x.style.fontSize"` | Inline `fontSize` style on `sc-x` |
| `"sc-card"` with value `null` | Removal marker — element was deleted |
...
Sub-composition elements use scoped IDs — `"sc-host/sc-leaf.text"` — where the host and leaf are separated by `/`.
...
```typescript
import type { FontValue, ImageValue } from "@smashcut/sdk";
...
```typescript
import { openComposition, ORIGIN_APPLY_PATCHES } from "@smashcut/sdk";
import type { JsonPatchOp, OverrideSet } from "@smashcut/sdk";
