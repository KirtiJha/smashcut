...
comp.setText("sc-title", "Draft A");
comp.setText("sc-title", "Draft B");
...
interface JsonPatchOp {
  ...
  path: string;   // JSON Pointer (RFC 6901), e.g. "/elements/sc-title/style/color"
  value?: unknown;
}
```
...
| `ORIGIN_APPLY_PATCHES` | `"@smashcut/sdk:applyPatches"` | Automatically applied by `applyPatches()` |

Both are exported from `@smashcut/sdk`.
...
comp.dispatch(
  { type: "setStyle", target: "sc-title", styles: { color: "#0EA5E9" } },
  { origin: "ui:color-picker" },
);

comp.batch(() => {
  comp.setText("sc-cta", "Buy Now");
...
`ORIGIN_APPLY_PATCHES` is a namespaced string (`"@smashcut/sdk:applyPatches"`) rather than a `Symbol` so it survives `postMessage`, structured clone, and JSON round-trips — important when the host forwards patch events across frames or workers.
