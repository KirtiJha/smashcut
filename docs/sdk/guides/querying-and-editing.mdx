...
Each `ElementSnapshot` (aliased from `SmashCutElement`) carries:

| Field | Type | Notes |
|-------|------|-------|
| `id` | `string` | Leaf `sc-id` — unique within its sub-composition scope |
...
```typescript
import type { FindQuery } from "@smashcut/sdk";
...
// Filter to elements inside a specific sub-composition (by host sc-id)
const innerIds = comp.find({ composition: "sc-intro-scene" });
...
comp.setText("sc-inner-title", "New text"); // may silently no-op

// Correct — use the scopedId returned by find() or getElements()
const [id] = comp.find({ name: "inner-title" }); // returns "sc-scene/sc-inner-title"
...
```typescript
comp.setText("sc-title", "Shipped.");
comp.setText("sc-subtitle", "Available now in all regions.");
```
...
comp.setStyle("sc-card", { opacity: null });
```
...
```typescript
comp.setAttribute("sc-hero-img", "src", "/assets/hero-v2.jpg");
comp.setAttribute("sc-hero-img", "alt", "Product screenshot");

// Remove an attribute
comp.setAttribute("sc-hero-img", "loading", null);
```
...
Inserts an HTML fragment as a child of `parent` at zero-based sibling position `index`. Pass `null` for `parent` to insert at the document body root. Returns the minted `sc-id` of the inserted root element.

```typescript
const newId = comp.addElement("sc-card", 0, `<span class="badge">New</span>`);
// newId is a fresh stable sc-id, e.g. "sc-a3k7"

// Append at the end (index >= child count)
comp.addElement("sc-card", 999, `<div class="footer-note">v2.0</div>`);
```
...
```typescript
comp.batch(() => {
  comp.setText("sc-title", "Summer Drop");
  comp.setStyle("sc-title", { color: "#FF6B35", fontSize: "112px" });
  comp.setTiming("sc-title", { start: 0, duration: 3.5 });
  comp.setAttribute("sc-logo", "src", "/assets/logo-summer.svg");
});
...
console.log(sel.ids); // ["sc-title", "sc-subtitle"]
...
<CardGroup cols={2}>
  ...
  <Card title="Types reference" icon="brackets-curly" href="/sdk/reference/types">
    SmashCutElement, FindQuery, ElementHandle, SelectionProxy, and more.
  </Card>
