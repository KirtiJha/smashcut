...
<Note>
  This API lives on the `@smashcut/sdk/editing` subpath, available since `@smashcut/sdk@0.7.22`. If the import fails to resolve, upgrade: `npm install @smashcut/sdk@latest`.
</Note>
...
function resolveElementAffordances(
  liveEl: HTMLElement,
  modelEl: Pick<SmashCutElement, "text" | "animationIds" | "start"> | null,
...
**Browser-only.** `resolveElementAffordances` calls `getComputedStyle` internally. It must not be imported or called in Node, a server action, or any non-browser path. The SDK adapter (`@smashcut/sdk/editing`) is separate from the pure core resolver precisely to keep the Node SDK path free of browser globals.
...
The full type definitions — `EditingAffordances`, `DomEditCapabilities`, `EditingSectionApplicability`, `AffordanceContext`, `SmashCutElement` — are documented at [`/sdk/reference/types`](/sdk/reference/types).

The pure DOM-free resolver (`resolveEditingAffordances`, `EditableElementFacts`) lives in `@smashcut/core` and is documented at [`/packages/core`](/packages/core).
