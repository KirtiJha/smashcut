...
```typescript
// Move sc-title to start at 1.0 s, last 2.5 s, on track 0
comp.setTiming("sc-title", { start: 1.0, duration: 2.5, trackIndex: 0 });
```
...
comp.setElementTiming({
  "sc-title": { start: 0.5, duration: 2.0 },
  "sc-logo": { start: 0.0, duration: 5.0, trackIndex: 1 },
  "sc-cta": { start: 3.0, duration: 2.5 },
});
```
...
const animId = comp.addWithKeyframes(
  "#sc-badge", // CSS selector targeting the element
...
```typescript
const newAnimId = comp.replaceWithKeyframes(oldAnimId, "#sc-badge", 1.0, 1.2, [
