...
```typescript
import type { PreviewAdapter } from "@smashcut/sdk";
```
...
<ParamField path="elementAtPoint" type="(x, y, opts?) => ElementAtPointResult | null">
  Synchronous hit-test at composition coordinates `(x, y)`. Returns the nearest `[data-sc-id]` element under the point, or `null` for a transparent hit (the composition root, an opacity-0 element, or nothing at all). Requires a same-origin iframe — cross-origin access throws a DOMException. The `atTime` option reflects GSAP state at the current playhead; seeking to a speculative time is not supported.
</ParamField>
...
The `id` is the element's `data-sc-id` value; `tag` is its lowercase tag name (e.g. `"div"`, `"img"`).
...
```typescript
import { createMemoryAdapter } from "@smashcut/sdk";
...
**Node.js only.** Returns a `PersistAdapter` that reads and writes files under a root directory. Import from the `@smashcut/sdk/adapters/fs` subpath — this module uses Node `fs/promises` and is excluded from the browser-safe main bundle.
...
The adapter writes the current composition at `{root}/{path}` and stores version snapshots in `{root}/.sc-versions/{path}/`. Version keys encode `Date.now()` and a monotonic counter (`"1750000000000-0001"`), so `listVersions()` returns them newest-first by lexicographic descending sort.
...
<Warning>
  `createFsAdapter` uses Node.js `fs/promises`. Do not import it in browser or edge environments — import from `@smashcut/sdk/adapters/fs` (the subpath) so bundlers can tree-shake it.
</Warning>
...
```typescript
import { createHeadlessAdapter } from "@smashcut/sdk";
...
```typescript
import { createIframePreviewAdapter } from "@smashcut/sdk";
...
} from "@smashcut/sdk/adapters/iframe";
```

```typescript
import { openComposition, createIframePreviewAdapter } from "@smashcut/sdk";
...
|--------|---------------|
| `PersistAdapter`, `PreviewAdapter`, `PersistVersionEntry` | `@smashcut/sdk` (types only) |
| `createMemoryAdapter` | `@smashcut/sdk` |
| `createHeadlessAdapter` | `@smashcut/sdk` |
| `createIframePreviewAdapter`, `resolveNearestHfElement` | `@smashcut/sdk` |
| `PaintQueryOptions` | `@smashcut/sdk` (type only) |
| `elementPaintsInk`, `compositionPaintsAt`, `imageAlphaOpaqueAt`, `alphaIsOpaque`, `mapPointToImagePixel`, `INTRINSIC_PAINT_TAGS` | `@smashcut/sdk/adapters/iframe` |
| `createFsAdapter`, `FsAdapterOptions` | `@smashcut/sdk/adapters/fs` |
