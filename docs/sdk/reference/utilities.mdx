...
description: "History module, persist queue, document utilities, constants, and error types exported from @smashcut/sdk."
---

This page covers everything exported from `@smashcut/sdk` that is not covered in [`openComposition`](/sdk/reference/open-composition), the [`Composition` interface](/sdk/reference/composition), [edit operations](/sdk/reference/edit-operations), [types](/sdk/reference/types), or [adapters](/sdk/reference/adapters).
...
```typescript
import { createHistory } from "@smashcut/sdk";
import type { HistoryModule, HistoryOptions, HistoryEntry } from "@smashcut/sdk";
```
...
```typescript
import { openComposition, createHistory } from "@smashcut/sdk";
...
comp.setText("sc-title", "Draft v1");
...
```typescript
import { createPersistQueue } from "@smashcut/sdk";
import type { PersistQueueModule, PersistQueueOptions } from "@smashcut/sdk";
```
...
```typescript
import { openComposition, createPersistQueue } from "@smashcut/sdk";
...
comp.setText("sc-title", "Autosaved");
...
```typescript
import { buildDocument, buildRoots, flatElements } from "@smashcut/sdk";
```

Low-level utilities for building the `SdkDocument` model and `SmashCutElement` trees from parsed HTML. These are the same functions the SDK uses internally on every `openComposition()` call. You rarely need them directly — they are exposed for hosts that parse HTML outside of a session, or for testing.
...
Parses an HTML string into the SDK document model. Calls `ensureHfIds` first so every element gets a stable `data-sc-id`. Uses `linkedom` for DOM parsing — node-safe, works in agents, CI, and server-side code. Returns an `SdkDocument` snapshot; mutations on the live session do not update the returned value.

### buildRoots

```typescript
function buildRoots(document: Document): SmashCutElement[];
```

Builds the element tree from an already-parsed (sc-id-stamped) `linkedom` `Document`. Walks the live DOM directly — no serialize/re-parse round trip. This is what the session's query API uses against its mutable document after each op.

### flatElements

```typescript
function flatElements(roots: readonly SmashCutElement[]): SmashCutElement[];
```
...
```typescript
import { buildDocument, flatElements } from "@smashcut/sdk";
...
```typescript
import { resolveScoped, findById, bareId, escapeHfId, isNewHostBoundary } from "@smashcut/sdk";
```
...
Strip a scoped id down to its leaf segment: `bareId("sc-host/sc-leaf")` returns `"sc-leaf"`. A no-op on an already-bare id.
...
Escape an id for safe interpolation into a `querySelectorAll` attribute-value selector (backslashes and double quotes). Use this if you're writing a raw `[data-sc-id="..."]` selector yourself instead of going through `resolveScoped`/`findById`.
...
```typescript
import { readVariableDefault } from "@smashcut/sdk";
```
...
```typescript
import { ORIGIN_APPLY_PATCHES } from "@smashcut/sdk";

const ORIGIN_APPLY_PATCHES = "@smashcut/sdk:applyPatches";
```
...
The value is a namespaced string rather than a `Symbol` so it survives realm boundaries — `postMessage`, structured clone, and JSON serialization all preserve it correctly. T3 embedded hosts that forward patch events across frames or workers rely on this property. The namespace prefix (`@smashcut/sdk:`) makes accidental collision with a host-chosen origin string negligible.

### ORIGIN\_LOCAL

```typescript
import { ORIGIN_LOCAL } from "@smashcut/sdk";
...
```typescript
import { UnsupportedOpError } from "@smashcut/sdk";
```
...
```typescript
import { resolveNearestHfElement } from "@smashcut/sdk";
...
Walks from `el` upward through `parentElement`, returning the nearest ancestor (inclusive) that carries `[data-sc-id]` and is not `[data-sc-root]`. Returns `null` when the walk exits the DOM without finding a match, when the matched node is the composition root, or when `isVisible(node)` returns `false` for the matched node.
...
```typescript
import { resolveElementAffordances } from "@smashcut/sdk/editing";
```

Determines which editing operations are available for a live element given its current DOM state and model. Imported from the `@smashcut/sdk/editing` subpath. Full documentation is in the [Editing Affordances guide](/sdk/guides/editing-affordances).
