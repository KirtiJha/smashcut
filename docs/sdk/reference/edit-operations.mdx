...
```typescript
comp.dispatch(
  { type: "setStyle", target: "sc-title", styles: { color: "#FFD60A" } },
  { origin: "agent" },
);
```
...
```typescript
comp.batch(() => {
  comp.dispatch({ type: "setText", target: "sc-title", value: "Launch Day" });
  comp.dispatch({ type: "setStyle", target: "sc-title", styles: { fontSize: "96px" } });
  comp.dispatch({ type: "setTiming", target: "sc-title", start: 0.5, duration: 3 });
});
```
...
| Code | Meaning |
|------|---------|
| `E_TARGET_NOT_FOUND` | The `target` sc-id does not exist in the document. |
...
These ops target one or more elements by explicit sc-id. `target` accepts a single `HfId` string or an `HfId[]` array; when an array is given the op is applied to each id individually within a single batch.
...
| `type` | Key fields | What it does |
|--------|-----------|--------------|
| `addElement` | `parent`, `index`, `html` | Inserts an HTML fragment. Returns the minted sc-id via the typed `comp.addElement()` method. |
...
`parent` is the sc-id of the parent element, or `null` to insert at the document body root. `index` is the zero-based sibling index (append if `>= childCount`). `html` must be a single-root HTML fragment and must not contain `<script>`.
...
Use the typed `comp.addElement(parent, index, html)` method to get back the minted sc-id.
...
comp.dispatch({
  ...
  entries: [
    { target: "sc-bg",   zIndex: 0 },
    { target: "sc-logo", zIndex: 10 },
    { target: "sc-text", zIndex: 20 },
  ],
});
```
...
| `type` | Key fields | What it does |
|--------|-----------|--------------|
| `addGsapTween` | `target`, `tween` | Adds a new tween for a target sc-id. Returns the minted `animationId` via the typed method. |
