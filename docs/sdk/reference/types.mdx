---
title: "Types"
description: "Core types exported from @smashcut/sdk, with pointers to the adapter, history, and persistence types."
---

This page documents the core types exported from `@smashcut/sdk`, verified against `packages/sdk/src/types.ts` and the export barrel at `packages/sdk/src/index.ts`. Adapter, history, and persistence types live with the APIs that take them — see [Adapters](/sdk/reference/adapters), [Utilities](/sdk/reference/utilities), [Composition](/sdk/reference/composition), and [openComposition](/sdk/reference/open-composition).
...
A stable SmashCut element identifier — the value of the `data-sc-id` attribute stamped on every editable element by `ensureHfIds`. Use `HfId` wherever you would write a raw `string` to signal that the value is an element id.
...
<ResponseField name="id" type="string">
  The element's `data-sc-id` value — unique within the top-level document.
</ResponseField>
...
<ResponseField name="children" type="readonly SmashCutElement[]">
  Direct child elements. Build a flat list with `flatElements()` from `@smashcut/sdk` utilities.
</ResponseField>
...
```typescript
type ElementSnapshot = SmashCutElement;
```

Type alias for `SmashCutElement`. Used in return types of `getElements()` and `getElement()` to signal that the value is a read-only snapshot — it does not update after subsequent mutations.
...
<ResponseField name="roots" type="readonly SmashCutElement[]">
...
const overrides: OverrideSet = {
  "sc-title.text": "Customer headline",
  "sc-logo.attr.src": "/customers/acme/logo.png",
  "sc-title.style.color": "#0EA5E9",
  "sc-old-badge": null,                         // element removed
...
<ResponseField name="composition" type="string">
  Restrict results to elements inside a specific sub-composition, identified by the host element's sc-id.
</ResponseField>
...
const subIds = comp.find({ composition: "sc-scene-2", tag: "img" });
```
...
A single keyframe entry for `addWithKeyframes` and `replaceWithKeyframes`. This is a structural shape, not a runtime export — you pass plain objects of this form to those methods; it is **not** re-exported from the `@smashcut/sdk` barrel, so there is nothing to import.
...
<ResponseField name="ids" type="readonly string[]">
  The sc-ids in the current selection at the moment `comp.selection()` was called.
</ResponseField>
...
```typescript
const title = comp.element("sc-title");
...
`@smashcut/parsers`; `VARIABLE_TYPES` is the matching runtime list, useful
