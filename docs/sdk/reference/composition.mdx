...
```typescript
comp.setStyle("sc-title", { color: "#FFD60A", fontSize: "96px" });
comp.setStyle("sc-card", { borderRadius: null }); // removes border-radius
```
...
```typescript
comp.setText("sc-headline", "Product launch — June 2026");
```
...
```typescript
comp.setAttribute("sc-logo", "src", "/assets/logo-v2.png");
comp.setAttribute("sc-video", "autoplay", null); // removes autoplay
```
...
comp.setTiming("sc-cta", { trackIndex: 1 });
```
...
```typescript
comp.removeElement("sc-old-badge");
```
...
Insert an HTML fragment as a child of `parent` at zero-based sibling `index`. Pass `null` for `parent` to insert at the document body root. The fragment must be single-root and must not contain `<script>` tags. Returns the minted `sc-id` of the inserted root element.

```typescript
const newId = comp.addElement("sc-scene", 0, '<div class="clip">New clip</div>');
...
comp.setElementTiming({
  "sc-title":   { start: 0,   duration: 2   },
  "sc-caption": { start: 1.5, duration: 1.5 },
  "sc-cta":     { start: 3,   duration: 2   },
});
```
...
```typescript
comp.setHold("sc-hero", { start: 2, end: 5, fill: "freeze" });
```
...
```typescript
const animId = comp.addGsapTween("sc-title", {
  method: "from",
  position: 0,
  duration: 0.6,
  ease: "power3.out",
...
<Note>
  For elements inside inlined sub-compositions, use `scopedId` (e.g. `"sc-host/sc-leaf"`) as the dispatch target, not `id`. Top-level elements have `scopedId === id`.
</Note>
...
| `composition` | `string` | Filter to elements inside a specific sub-composition host by its `sc-id`. |
...
Return a copy of the current selection as an array of `sc-id` strings. Returns `[]` when nothing is selected.
...
```typescript
comp.setSelection(["sc-title", "sc-subtitle"]);
...
```typescript
comp.dispatch({ type: "setStyle", target: "sc-card", styles: { borderRadius: "24px" } });
comp.dispatch({ type: "reorderElements", entries: [{ target: "sc-bg", zIndex: 0 }] });
...
```typescript
comp.dispatch({ type: "setText", target: ["sc-title", "sc-subtitle"], value: "Coming soon" });
```
...
```typescript
comp.setText("sc-title", "Final title");
await comp.flush();
comp.dispose();
```
