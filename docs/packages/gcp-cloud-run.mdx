---
title: "@smashcut/gcp-cloud-run"
description: "Google Cloud Run and Workflows adapter for distributed SmashCut rendering."
---

The GCP Cloud Run package runs SmashCut' distributed render primitives on Google Cloud. It ships a Cloud Run HTTP service, a Node client SDK, and a Terraform module for provisioning the bucket, service, workflow, and service accounts.

```bash
npm install @smashcut/gcp-cloud-run
```

## When to Use

**Use `@smashcut/gcp-cloud-run` when you need to:**
...
| Import | Description |
|--------|-------------|
| `@smashcut/gcp-cloud-run` | Server handler, event types, GCS transport, and client SDK exports |
| `@smashcut/gcp-cloud-run/server` | Cloud Run HTTP service entry point |
| `@smashcut/gcp-cloud-run/sdk` | Lightweight Node SDK for deploying sites, starting renders, polling progress, and estimating cost |
...
The service is intentionally close to the AWS Lambda adapter: thin cloud transport around the same `@smashcut/producer/distributed` primitives.
...
```typescript
import { getRenderProgress, renderToCloudRun } from "@smashcut/gcp-cloud-run/sdk";

const handle = await renderToCloudRun({
  projectDir: "./my-composition",
  config: { fps: 30, width: 1920, height: 1080, format: "mp4" },
  bucketName: "smashcut-render-my-project",
