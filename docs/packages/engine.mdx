---
title: "@smashcut/engine"
description: "Low-level, seekable frame capture and encoding primitives."
---

`@smashcut/engine` is the low-level layer beneath the
[Producer](/packages/producer). It opens a page that implements the SmashCut
...
The page at `serverUrl` must expose `window.__sc` with a `duration` and a
...
<CardGroup cols={2}>
  ...
  <Card title="Deterministic rendering" icon="repeat" href="/concepts/determinism">
    Understand what SmashCut controls and what the environment still affects.
  </Card>
</CardGroup>
