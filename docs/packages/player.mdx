---
title: "@smashcut/player"
description: "Embeddable web component for playing SmashCut compositions in any web page."
---

The player package provides a `<smashcut-player>` custom element that embeds a
SmashCut composition in plain HTML or a framework application.
...
[`@smashcut/studio`](/packages/studio) exports `resolveIframe` for consumers
that need to pass the inner iframe to Studio's timeline hooks:

```ts
import { resolveIframe, useTimelinePlayer } from "@smashcut/studio";
...
communicate with the SmashCut runtime through `postMessage`.
