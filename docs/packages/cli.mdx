...
`npx smashcut <command> --help` to see exactly what your copy accepts.
...
| Render somewhere else    | [`cloud`](#smashcut-cloud), [`lambda`](#smashcut-lambda), [`cloudrun`](#smashcut-cloudrun)                                                                                             |
...
| Connect other tools      | [`auth`](#smashcut-auth), [`skills`](#skills), [`figma`](#figma-and-events), [`telemetry`](#telemetry), [`feedback`](#feedback)                                                              |
...
default to the current directory: `npx smashcut check ./my-video`. A few
...
`smashcut auto-updated to v0.1.5`. It never crosses a major version. Anywhere
it cannot install — `npx`, an unknown installer — you get a notice instead:

```
  Update available: 0.1.4 → 0.1.5
  Run: npx smashcut@latest
```

Both stay silent in CI and non-TTY shells. `SMASHCUT_NO_UPDATE_CHECK=1` turns
...
npx smashcut init my-video --example blank --video video.mp4 --non-interactive

# With the Tailwind browser runtime
npx smashcut init my-video --example blank --tailwind
...
| `--skip-skills`     | Currently ignored. Set `SMASHCUT_SKIP_SKILLS=1` to opt out in CI or tests.                                                                                                                                                                                                         |
...
Use the `/smashcut-core` skill when editing these projects, so agents follow
...
```bash
npx smashcut add claude-code-window          # a block
npx smashcut add shader-wipe                 # a component
npx smashcut add captions                    # every block with this tag
npx smashcut add shader-wipe --dir ./my-video
npx smashcut add shader-wipe --no-clipboard --json   # headless / CI
```
...
`add` reads [`smashcut.json`](#smashcut-json) to learn which registry to
...
```bash
npx smashcut catalog                          # table of everything
npx smashcut catalog --type block --tag social
npx smashcut catalog --json
npx smashcut catalog --human-friendly         # picker; installs on select
```
...
```bash
npx smashcut capture https://stripe.com
npx smashcut capture https://linear.app -o linear-capture
npx smashcut capture https://example.com --json
```
...
`SMASHCUT_OPENROUTER_MODEL` overrides the model.
...
```bash
# Local transcription
npx smashcut transcribe audio.mp3
npx smashcut transcribe video.mp4 --model medium.en --language en

# Import from another tool
npx smashcut transcribe subtitles.srt
npx smashcut transcribe openai-response.json

# Export a caption sidecar
npx smashcut transcribe transcript.json --to srt
```
...
| `--timeout`       | Whisper spawn timeout in ms, overriding the auto-scaled default. Raise it on slow or emulated CPUs where large models take many seconds per audio second. Whisper only — Parakeet has its own. Minimum 5000. Env: `SMASHCUT_TRANSCRIBE_TIMEOUT_MS`. |
...
```bash
npx smashcut tts "Welcome to SmashCut"
npx smashcut tts "Intro" --voice bf_emma --output narration.wav
npx smashcut tts "Slow and clear" --speed 0.8
npx smashcut tts script.txt
npx smashcut tts --list
```
...
npx smashcut remove-background avatar.mp4 -o transparent.webm

# ProRes 4444 for an editing round-trip
npx smashcut remove-background avatar.mp4 -o transparent.mov

# One image → transparent PNG
npx smashcut remove-background portrait.jpg -o cutout.png

# Cutout and inverse-alpha background plate in one pass
npx smashcut remove-background avatar.mp4 -o subject.webm --background-output plate.webm

# What providers does this machine have?
npx smashcut remove-background --info
```
...
`~/.cache/smashcut/background-removal/models/` on first run and is reused
...
`SMASHCUT_CUDA=1` and supply a GPU-enabled build.
...
npx smashcut media-treatment --capabilities --json
npx smashcut media-treatment --capability kuwahara --json

# Measure a selected local source
npx smashcut media-treatment --selector '#hero' --analyze --json

# Apply, preview, or clear
npx smashcut media-treatment --selector '#hero' \
  --grading '{"preset":"skin-soft","intensity":0.6}' --apply
npx smashcut media-treatment --file compositions/scene.html --selector 'video' \
  --grading '{"preset":"warm-daylight"}' --apply --dry-run --json
npx smashcut media-treatment --selector '#hero' --clear
```
...
```bash
npx smashcut beats [dir]
npx smashcut beats [dir] --json   # { ok, file, count, bpm }
```
...
`npx smashcut browser ensure` if it is missing. Results match Studio's to
...
npx smashcut normalize-audio --reference target-audio --target user-audio --json
```
...
```bash
npx smashcut preview [dir]
npx smashcut preview --port 4567
npx smashcut preview --background     # keep running after the command exits
npx smashcut preview --foreground     # stay attached in a non-interactive shell
npx smashcut preview --status --json  # inspect a managed preview from an agent
npx smashcut preview --list           # every running preview
```
...
Your composition opens in SmashCut Studio, and edits to `index.html` and its
...
(Vite with full HMR, when `@smashcut/studio` is in your `node_modules`), or
...
```bash
npx smashcut present [dir]
npx smashcut present [dir] --port 3004
```
...
Upload the project and get back a stable `smashcut.dev` URL.

```bash
npx smashcut publish [dir]
npx smashcut publish --yes
npx smashcut publish --update <url-or-id>
npx smashcut publish --space <space-id>
```
...
whoever opens it on `smashcut.dev` can sign in, claim the project, and keep
editing in the web app.

Sign in with `npx smashcut auth login` first when you want an owned, stable
...
```bash
npx smashcut lint [dir]
npx smashcut lint [dir] --verbose   # include info-level findings
npx smashcut lint [dir] --json
```
...
```bash
npx smashcut check [dir]
npx smashcut check [dir] --json         # {ok, lint, runtime, layout, motion, contrast, snapshots}
npx smashcut check [dir] --snapshots
npx smashcut check [dir] --at 1.5,4,7.25
npx smashcut check [dir] --strict       # warnings fail too
```
...
```bash
npx smashcut snapshot my-project --at 2.9,10.4,18.7
npx smashcut snapshot my-project --frames 10
npx smashcut snapshot my-project --zoom '#headline'
npx smashcut snapshot my-project --at 1.5,4.3 --against ref.mp4
```
...
```bash
npx smashcut keyframes [dir]
npx smashcut keyframes [dir] --selector "#card" --shot card-motion.png
```
...
```bash
npx smashcut compare ./variant-a ./variant-b --labels "A,B"
```
...
```bash
npx smashcut grade-compare --for frame.png --luts warm.cube,cool.cube
```
...
`npx smashcut inspect --help` if you are maintaining automation that still
calls them.
...
```bash
# Fast iteration
npx smashcut render --output output.mp4

# Deterministic output
npx smashcut render --docker --output output.mp4

# Transparent, for overlays and lower thirds
npx smashcut render --format webm --output overlay.webm

npx smashcut render --output output.mp4 --fps 60 --quality high
npx smashcut render --gpu --output gpu.mp4
```
...
| `--frames-cache-dir`                                     | `<tmpdir>/smashcut-extract-cache-<uid>` | Where extracted source frames cache. `off`, `none`, `false`, or `0` disables it. `doctor` reports the live value.   |
...
- **The player never became ready.** After navigation, `window.__sc` readiness
...
>
  <script>
    const vars = window.__smashcut.getVariables();
...
npx smashcut render --output default.mp4

# Override; missing keys fall through to the defaults
npx smashcut render --variables '{"title":"Q4 Report","theme":"dark"}' --output q4.mp4

npx smashcut render --variables-file ./vars.json --output out.mp4
```
...
```bash
npx smashcut render --format webm --output captions.webm
...
`SMASHCUT_EXTRACT_CACHE_DIR` or the default. Under 2 GB free it fails, since
...
`npx smashcut info [dir]` prints project metadata: name, resolution,
duration, element counts by type, track count, and total size.
`npx smashcut compositions` lists every composition in the project with its
...
npx smashcut upgrade --check         # check and exit, no prompt
npx smashcut upgrade --check --json  # for agents
npx smashcut upgrade --yes           # upgrade a global install without prompting
npx smashcut upgrade --project       # bump pinned CLI scripts in package.json
```
...
`smashcut@<version>` script pins in a project's `package.json`.
`--check --json` returns:
...
```bash
npx smashcut browser ensure           # find it, or download it
npx smashcut browser ensure --force   # discard any cached download and re-fetch
npx smashcut browser path             # print the executable path
npx smashcut browser clear            # remove the cached download
```

`path` prints only the path, so it composes:
`$(npx smashcut browser path)`. Use `--force` when a download was
...
```bash
npx smashcut feedback --rating 10
npx smashcut feedback --rating 7 --comment "render succeeded but GSAP timeline didn't animate"
npx smashcut feedback --rating 3 --comment "GSAP timeline froze on seek" --file-issue
```
...
```bash
npx smashcut telemetry enable
npx smashcut telemetry disable
npx smashcut telemetry status
```
...
`smashcut auth login`, your HeyGen account email — or username, if the
...
Turn it all off with `SMASHCUT_NO_TELEMETRY=1` or the command above.
...
`SMASHCUT_NO_TELEMETRY=1`, `DO_NOT_TRACK=1`, and dev builds. See
[Canary rollouts](/contributing/canary-rollouts).

### `skills`

Install or refresh the SmashCut skills that AI coding tools read.
...
npx smashcut skills update pr-to-video
```
...
The CLI installs from the current SmashCut GitHub source and links the global
bundles into whichever compatible agents it finds. After scaffolding, `init`
also refreshes the core set plus any SmashCut skills you already have.
...
`smashcut skills` already handles this. You do not need an environment
variable.

If you called the upstream command directly instead, set it yourself:

```bash
GIT_CLONE_PROTECTION_ACTIVE=0 npx skills add ./skills
```

### `figma` and `events`

`npx smashcut figma` imports through the Figma REST API, with three
...
`npx smashcut events` lets an installed workflow skill emit an anonymous
...
1. `HEYGEN_API_KEY`
2. `SMASHCUT_API_KEY` (a SmashCut alias for the same thing)
...
```bash
smashcut auth status
smashcut auth status --json
```
...
```bash
smashcut auth logout
smashcut auth logout --keep-api-key   # clear only the OAuth session
smashcut auth logout --yes            # no prompt
```
...
| `SMASHCUT_API_KEY` | Alias for `HEYGEN_API_KEY`.                      |
...
```bash
smashcut auth login                          # one-time
smashcut cloud render ./my-video             # zip, upload, poll, download
smashcut cloud render ./my-video --no-wait   # submit and exit with the render_id
smashcut cloud list                          # recent renders
```

### `cloud render [<projectDir>]`

End to end: zips the project, uploads it through the direct-to-S3 asset flow,
submits `POST /v3/smashcut/renders`, polls
`GET /v3/smashcut/renders/{id}` until it finishes or fails, and streams the
video to disk.

The zip excludes root `renders` and `snapshots`, `.git`, `node_modules`, `dist`,
`.next`, `coverage`, dotfiles, and anything your `.smashcutignore` rules
match. Those rules use gitignore syntax and apply to `smashcut publish` too.
...
paths you have verified are unneeded to `.smashcutignore`, or pre-host the
large media and reference it by URL.

Render parameters mirror local `smashcut render` where they overlap:
...
smashcut cloud render --callback-url https://example.com/sc-hook --no-wait

# Re-render something already uploaded, or render straight from a public URL.
smashcut cloud render --asset-id asst_abc123
...
```bash
smashcut cloud render . --idempotency-key "$(uuidgen)"
```
...
```bash
smashcut cloud list
smashcut cloud list --limit 50 --json
smashcut cloud list --all
```
...
smashcut cloud get hfr_abc123 --json
```
...
smashcut cloud delete hfr_abc123 --no-confirm --json
```

### Which one should you use?

- **`smashcut render`** — the fastest loop. Use it while authoring.
- **`smashcut cloud render`** — zero infrastructure. HeyGen runs it, you pay
  per credit. Use it when you do not want Chrome, ffmpeg, or AWS on your
  machine.
- **`smashcut lambda render`** — your own AWS, chunked in parallel. Use it
  when you have already invested in AWS and want the work on your account.

`cloud` reuses whatever credential `smashcut auth status` resolves. Override
...
The command group wraps the `@smashcut/aws-lambda` SDK plus AWS SAM, so an
end-to-end render is three commands:

```bash
smashcut lambda deploy
smashcut lambda render ./my-project --width 1920 --height 1080 --wait
smashcut lambda destroy   # when you're done
```
...
| `--variables`           | —                                   | JSON object reaching every chunk worker as `window.__scVariables`                                                  |
| `--variables-file`      | —                                   | Read those from a JSON file                                                                                       |
| `--strict-variables`    | off                                 | Fail instead of warn on an undeclared or mistyped key                                                             |
| `--execution-name`      | `sc-render-<uuid>`                  | Step Functions execution name                                                                                     |
...
`<cwd>/.smashcut/lambda-stack-<stackName>.json`, so the other subcommands do
...
smashcut lambda render ./my-project --site-id=abc1234deadbeef0 --width 1920 --height 1080
```
...
```bash
smashcut lambda render ./my-project \
  --width=1920 --height=1080 --fps=30 --format=mp4 \
  --chunk-size=240 --max-parallel-chunks=16 \
  --wait

smashcut lambda render ./my-template --site-id=abc1234deadbeef0 \
...
{"outputKey": "renders/carol.mp4", "variables": {"name": "Carol"}, "executionName": "sc-carol-001"}
```

```bash
smashcut lambda render-batch ./my-template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10
```
...
Batch dispatched: 3 started, 0 failed-to-start.
  ...
  ✓ line 1  renders/alice.mp4  arn:aws:states:us-east-1:1234:execution:hf:sc-render-...
  ✓ line 2  renders/bob.mp4    arn:aws:states:us-east-1:1234:execution:hf:sc-render-...
  ✓ line 3  renders/carol.mp4  arn:aws:states:us-east-1:1234:execution:hf:sc-carol-001
```

Poll each one with `smashcut lambda progress <renderId>`, or use the returned
...
```bash
smashcut lambda progress sc-render-abcd1234
```
...
smashcut lambda policies user

# { TrustRelationship, InlinePolicy } for a CloudFormation service role.
smashcut lambda policies role

# Does a checked-in policy still cover the CLI?
smashcut lambda policies validate ./infra/iam/smashcut-deploy.json
```
...
`smashcut lambda` keeps per-stack metadata under
`<cwd>/.smashcut/lambda-stack-<name>.json`, so the verbs never have to call
...
The Google Cloud counterpart to [`lambda`](#smashcut-lambda): distributed
rendering on Cloud Run plus Cloud Workflows, driven from your laptop or CI. It
wraps the `@smashcut/gcp-cloud-run` SDK plus `terraform` (the module shipped
with the package) and `gcloud` or Cloud Build for the image.

```bash
smashcut cloudrun deploy --project my-gcp-project
smashcut cloudrun render ./my-project --width 1920 --height 1080 --wait
smashcut cloudrun destroy --project my-gcp-project
```
...
to `smashcut`. Sizing is `--cpu` (1, 2, 4, or 8; default 4), `--memory`
...
The other verbs mirror [`lambda`](#smashcut-lambda) one for one, and so do
their flags:
...
coordinates cache in `~/.smashcut/cloudrun-state.json` — project id, region,
...
`smashcut init` writes a `smashcut.json` at the root of every new project,
and `smashcut add` reads it to learn which registry to pull from and where
...
| Field              | Description                                                                             |
| ------------------ | --------------------------------------------------------------------------------------- |
| `registry`         | Base URL of the registry `add` pulls from. Defaults to the public SmashCut registry. |
...
<CardGroup cols={2}>
  ...
  <Card title="Studio" icon="palette" href="/packages/studio">
    The editor UI that powers `smashcut preview`. Use directly to embed in your own app.
  </Card>
