...
| --- | --- | --- | --- | --- |
| `data-fx-chain` | The effects, in signal order | JSON | `<audio>`, `<video>`, `<sc-audio-group>` | Yes |
| `data-automation` | Envelopes on volume or an effect parameter | JSON | `<audio>`, `<video>`, `<sc-audio-group>` | Yes |
...
can also sit on an `<sc-audio-group>`, where they apply to the group's whole bus;
...
| A group (`<sc-audio-group>`) | **Composition time.** A group has no `data-start`, so its clock starts at zero |
...
```html
<style>sc-audio-group { display: none !important; }</style>

<sc-audio-group id="voiceover" data-label="Voiceover" data-volume="1"></sc-audio-group>
...
- `<sc-audio-group>` is optional metadata: `data-label`, `data-volume`,
...
- The element must be inert. The runtime injects
  `sc-audio-group{display:none!important}` so an unknown custom element cannot
...
`HF_AUDIO_FX` in `@smashcut/core/audio-fx` is the source of truth.
...
SFX-shaped is a voice. Needs `ffmpeg` on `PATH` and `@smashcut/core`
