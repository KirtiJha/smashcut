...
<!--
  ...
  Add class="sc-tracking-in" to text elements, then animate the CSS variable
...
>
  .sc-tracking-in {
    opacity: 0;
    color: var(--sc-tracking-color, inherit);
    letter-spacing: calc(var(--sc-tracking-value, 0.28em) * var(--sc-tracking-reach, 1));
    transform: translateY(var(--sc-tracking-y, 8px));
...
<script>
  (function () {
    ...
    host.style.setProperty("--sc-tracking-reach", String(reach));
    host.style.setProperty("--sc-tracking-y", rise);
    host.style.setProperty("--sc-tracking-color", tone);
  })();
</script>
...
<!--
  ...
  tl.to(".sc-tracking-in", {
    opacity: 1,
    "--sc-tracking-value": "0.02em",
    y: 0,
    duration: 0.9,
    ease: "power3.out",
    stagger: 0.1,
...
<InstallCommand command="npx smashcut add tracking-in" item="tracking-in" />
