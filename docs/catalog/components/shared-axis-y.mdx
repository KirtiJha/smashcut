...
<!--
  Shared Axis Y - typography primitive for SmashCut.
...
<div
  class="sc-catalog-shared-axis-y"
...
<script>
  (function () {
    ...
    var roots = document.querySelectorAll(".sc-catalog-shared-axis-y");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--sc-word-axis-y", String(sign * reach));
...
<!--
  ...
  tl.fromTo('.sc-catalog-shared-axis-y span', { opacity: 0, '--sc-word-y': '32px' }, { opacity: 1, '--sc-word-y': '0px', duration: 0.22, stagger: 0.075, ease: 'power2.out' }, startTime);
-->
```
...
<InstallCommand command="npx smashcut add shared-axis-y" item="shared-axis-y" />
