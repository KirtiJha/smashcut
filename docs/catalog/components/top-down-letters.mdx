...
<!--
  Top-Down Letters - typography primitive for SmashCut.
...
<div
  class="sc-catalog-top-down-letters"
...
<style>
  .sc-catalog-top-down-letters {
    color: var(--sc-letters-color, #18181b);
  ...
  .sc-catalog-top-down-letters span {
    display: inline-block;
    transform: translateY(calc(var(--sc-char-y, 0px) * var(--sc-char-distance, 1)));
...
<script>
  (function () {
    ...
    var roots = document.querySelectorAll(".sc-catalog-top-down-letters");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--sc-char-distance", String(distance));
      root.style.setProperty("--sc-letters-color", tone);
...
<!--
  ...
  tl.fromTo('.sc-catalog-top-down-letters span', { opacity: 0, '--sc-char-y': '-42px' }, { opacity: 1, '--sc-char-y': '0px', duration: 0.22, stagger: 0.035, ease: 'power3.out' }, startTime);
-->
```
...
<InstallCommand command="npx smashcut add top-down-letters" item="top-down-letters" />
