      ...
      <div id="root" data-composition-id="chat-thread" data-duration="12" data-fps="30">
        ...
        <script>
          (function () {
            ...
            var DEFAULT_THREAD =
              "recv:what r u using for the launch video??|sent:wait look \ud83d\udc40|card:SmashCut \u2014 Write HTML, render pixel-perfect video~hyperframes.heygen.com|recv:OMG IT'S HTML|emoji:\ud83e\udd2f\ud83e\udd2f\ud83e\udd2f|sent:renders in 4K. no editor";
...
<InstallCommand command="npx smashcut add chat-thread" item="chat-thread" />
...
| `messages` | `recv:what r u using for the launch video??|sent:wait look 👀|card:SmashCut — Write HTML, render pixel-perfect video~hyperframes.heygen.com|recv:OMG IT&#39;S HTML|emoji:🤯🤯🤯|sent:renders in 4K. no editor` | string | Pipe-separated thread. Each entry picks its type by prefix: recv: or sent: for a text bubble, emoji: for a large emoji row, img for a media placeholder, card:Title~domain for a link card. Example: recv:hey|sent:look 👀|card:SmashCut~hyperframes.heygen.com|emoji:🤯🤯🤯 |
...
<div
  ...
  data-variable-values='{"contact":"Rachel","unread":12,"messages":"recv:what r u using for the launch video??|sent:wait look 👀|card:SmashCut — Write HTML, render pixel-perfect video~hyperframes.heygen.com|recv:OMG IT&#39;S HTML|emoji:🤯🤯🤯|sent:renders in 4K. no editor","beat":1.7,"dots":"on","receipt":"on"}'
></div>
```
