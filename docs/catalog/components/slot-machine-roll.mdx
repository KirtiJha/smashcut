...
<!--
  Slot Machine Roll - typography primitive for SmashCut.
...
<div
  class="sc-catalog-slot-machine-roll"
...
<style>
  .sc-catalog-slot-machine-roll {
    color: var(--sc-slot-color, #18181b);
  ...
  .sc-catalog-slot-machine-roll {
    display: inline-flex;
    gap: 6px;
    height: var(--sc-slot-row, 74px);
    overflow: hidden;
    font:
      900 var(--sc-slot-row, 74px)/1 ui-monospace,
      SFMono-Regular,
      Menlo,
      monospace;
  }
  .sc-catalog-slot-machine-roll span {
    display: block;
    height: var(--sc-slot-row, 74px);
    overflow: hidden;
  }
  .sc-catalog-slot-machine-roll i {
    display: block;
    font-style: normal;
    transform: translateY(
      calc(
        var(--sc-slot-base, 0px) + var(--sc-slot-y, 0px) * var(--sc-slot-dir, 1) *
          var(--sc-slot-gain, 1)
      )
    );
  }
...
<script>
  (function () {
    ...
    var roots = document.querySelectorAll(".sc-catalog-slot-machine-roll");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--sc-slot-row", row + "px");
      root.style.setProperty("--sc-slot-gain", String(row / 74));
      root.style.setProperty("--sc-slot-dir", String(dir));
      // Rolling down starts one row up, so the zero still sits in frame at rest.
      root.style.setProperty("--sc-slot-base", (dir === 1 ? 0 : -row) + "px");
      root.style.setProperty("--sc-slot-color", tone);
      root.textContent = "";
...
<!--
  ...
  tl.fromTo('.sc-catalog-slot-machine-roll i', { '--sc-slot-y': '0px' }, { '--sc-slot-y': '-74px', duration: 0.52, stagger: 0.06, ease: 'power3.inOut' }, startTime);
-->
```
...
<InstallCommand command="npx smashcut add slot-machine-roll" item="slot-machine-roll" />
