...
<!--
  Text Stagger - transitions.dev-inspired primitive for SmashCut.
...
<div
  class="sc-transition-text-stagger"
...
<style>
  .sc-transition-text-stagger {
    display: flex;
    flex-wrap: wrap;
    gap: 0.24em;
    color: var(--sc-stagger-ink, #111827);
    font:
      900 74px/0.95 Inter,
      system-ui,
      sans-serif;
    font-size: var(--sc-stagger-size, 74px);
    letter-spacing: -0.045em;
  }
  .sc-transition-text-stagger span {
    display: inline-block;
    background: linear-gradient(
      90deg,
      var(--sc-stagger-ink, #111827),
      var(--sc-stagger-ink, #111827),
      var(--sc-stagger-accent, #38bdf8),
      var(--sc-stagger-ink, #111827)
    );
...
<script>
  (function () {
    ...
    var roots = document.querySelectorAll(".sc-transition-text-stagger");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--sc-stagger-size", size + "px");
      root.style.setProperty("--sc-stagger-ink", tone);
      root.style.setProperty("--sc-stagger-accent", accent);
...
<!--
  ...
  tl.fromTo('.sc-transition-text-stagger span', { opacity: 0, y: 48, filter: 'blur(14px)' }, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.54, stagger: 0.09, ease: 'power3.out' }, 0.35);
  tl.to('.sc-transition-text-stagger span', { backgroundPosition: '100% 0%', duration: 0.72, stagger: 0.04, ease: 'power2.inOut' }, 1.05);
-->
```
...
<InstallCommand command="npx smashcut add text-stagger" item="text-stagger" />
