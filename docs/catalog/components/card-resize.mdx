...
<!--
  Card Resize - transitions.dev-inspired primitive for SmashCut.
...
<div
  class="sc-transition-card-resize"
...
>
  <div class="sc-transition-card-media"></div>
  <div class="sc-transition-card-copy">
...
<style>
  .sc-transition-card-resize {
    ...
    --sc-card-l2: rgba(24, 24, 27, 0.45);
    --sc-card-l3: rgba(24, 24, 27, 0.18);
    --sc-card-hair: rgba(24, 24, 27, 0.14);
    display: grid;
    ...
    padding: 18px;
    overflow: hidden;
    border: 1px solid var(--sc-card-border, #e4e4e7);
    border-radius: 26px;
    background: var(--sc-card-bg, #ffffff);
    color: var(--sc-card-ink, #18181b);
    font-family: Inter, system-ui, sans-serif;
    box-shadow: var(--sc-card-shadow, 0 26px 80px rgba(0, 0, 0, 0.12));
    will-change: width, height, transform;
  }
  .sc-transition-card-media {
    width: 92px;
    height: 92px;
    border-radius: 20px;
    background: var(--sc-card-media, var(--sc-card-l3));
    box-shadow: inset 0 0 0 1px var(--sc-card-hair);
  }
  .sc-transition-card-copy {
    display: grid;
    align-content: center;
    gap: 7px;
    min-width: 0;
  }
  .sc-transition-card-copy span {
    color: var(--sc-card-muted, #71717a);
    font-size: 13px;
    font-weight: 850;
    text-transform: uppercase;
  }
  .sc-transition-card-copy strong {
    white-space: nowrap;
    font-size: 22px;
    letter-spacing: -0.01em;
  }
  .sc-transition-card-copy p {
    margin: 0;
    color: var(--sc-card-muted, #71717a);
    font-size: 15px;
    line-height: 1.45;
...
<script>
  (function () {
    ...
    var medias = {
      ocean: "var(--sc-card-l3)",
      forest:
        "repeating-linear-gradient(135deg, var(--sc-card-hair) 0 2px, transparent 2px 4px), var(--sc-card-l3)",
      ember:
        "radial-gradient(circle 11px at 64px 28px, var(--sc-card-l2) 0 100%, transparent 100%), linear-gradient(to top, var(--sc-card-l2) 0 24px, transparent 24px), var(--sc-card-l3)",
    };
    var shadows = {
    ...
    var roots = document.querySelectorAll(".sc-transition-card-resize");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--sc-card-bg", surface.bg);
      roots[i].style.setProperty("--sc-card-ink", surface.ink);
      roots[i].style.setProperty("--sc-card-muted", surface.muted);
      roots[i].style.setProperty("--sc-card-border", surface.border);
      roots[i].style.setProperty("--sc-card-l2", surface.l2);
      roots[i].style.setProperty("--sc-card-l3", surface.l3);
      roots[i].style.setProperty("--sc-card-hair", surface.hair);
      roots[i].style.setProperty("--sc-card-media", media);
      roots[i].style.setProperty("--sc-card-shadow", shadow);
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.sc-transition-card-resize', { '--sc-card-w': '330px', '--sc-card-h': '132px', opacity: 0, y: 24 }, { '--sc-card-w': '560px', '--sc-card-h': '210px', opacity: 1, y: 0, duration: 0.68, ease: 'power3.inOut' }, 0.35);
  tl.fromTo('.sc-transition-card-copy p', { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.28, ease: 'power2.out' }, 0.82);
-->
```
...
<InstallCommand command="npx smashcut add card-resize" item="card-resize" />
