...
<!--
  Menu Morph - transitions.dev-inspired primitive for SmashCut.
...
<div
  class="sc-transition-menu-morph"
...
>
  <button class="sc-transition-menu-button" type="button" aria-label="Menu">
    <span></span><span></span><span></span>
  </button>
  <div class="sc-transition-menu-actions">
    <div>Text</div>
    <div>Image</div>
    <div>Audio</div>
  </div>
...
<style>
  ...
  .sc-transition-menu-button {
    z-index: 2;
    display: grid;
    place-items: center;
    gap: 5px;
    width: var(--sc-menu-size, 68px);
    height: var(--sc-menu-size, 68px);
    border: 0;
    border-radius: 999px;
    background: var(--sc-menu-bg, #18181b);
    box-shadow: 0 22px 58px rgba(0, 0, 0, 0.22);
  }
  .sc-transition-menu-button span {
    width: calc(var(--sc-menu-size, 68px) * 6 / 17);
    height: 3px;
    border-radius: 999px;
    background: #ffffff;
    transform-origin: center;
  }
  .sc-transition-menu-actions {
    position: absolute;
    display: flex;
    gap: 10px;
    bottom: 20px;
  }
  .sc-transition-menu-actions div {
    padding: 9px 13px;
    border-radius: 999px;
    background: #ffffff;
    color: #18181b;
...
<script>
  (function () {
    ...
    var roots = document.querySelectorAll(".sc-transition-menu-morph");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--sc-menu-size", size + "px");
      roots[i].style.setProperty("--sc-menu-bg", tone);
      var row = roots[i].querySelector(".sc-transition-menu-actions");
      if (!row) {
        continue;
      }
...
<!--
  ...
  tl.fromTo('.sc-transition-menu-button', { scale: 0.72, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.34, ease: 'back.out(1.8)' }, 0.3);
  tl.to('.sc-transition-menu-button span:nth-child(1)', { y: 8, rotation: 45, duration: 0.24, ease: 'power2.inOut' }, 0.78);
  tl.to('.sc-transition-menu-button span:nth-child(2)', { opacity: 0, duration: 0.18, ease: 'power1.out' }, 0.78);
  tl.to('.sc-transition-menu-button span:nth-child(3)', { y: -8, rotation: -45, duration: 0.24, ease: 'power2.inOut' }, 0.78);
  tl.fromTo('.sc-transition-menu-actions div', { y: 28, opacity: 0, scale: 0.86 }, { y: 0, opacity: 1, scale: 1, duration: 0.26, stagger: 0.06, ease: 'back.out(1.6)' }, 0.96);
-->
```
...
<InstallCommand command="npx smashcut add menu-morph" item="menu-morph" />
