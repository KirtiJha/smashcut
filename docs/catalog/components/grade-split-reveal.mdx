...
description: "The same picture twice, one flat and one finished, with a hard edge sweeping between them. It stops in the middle long enough to compare, then commits and the graded look takes the frame. Both looks are real SmashCut color grading run through the WebGL shader, so the preview and the render match."
---
...
<!--
  ...
  Both looks are real SmashCut color grading, not a CSS filter. The runtime hands each
...
<InstallCommand command="npx smashcut add grade-split-reveal" item="grade-split-reveal" />
