...
<!--
  ...
  Markup:
    <span class="sc-number-wheel" data-value="30222"></span>

  Paste this snippet once. It builds one column per digit. Animate the generated
  .sc-number-wheel-strip elements with the data-target-y custom property.
  ...
  still lands on its own digit, so only the run in between changes.
    ...
    - spin (single | double | triple, default single): how many full 0 to 9
      ...
      the strip and push --sc-number-target-y further, so the reel covers more
...
>
  .sc-number-wheel {
    ...
    gap: var(--sc-wheel-gap, 0.02em);
    color: var(--sc-wheel-color, inherit);
    font-variant-numeric: tabular-nums;
  }
  .sc-number-wheel-digit {
    display: inline-block;
    height: 1em;
    overflow: hidden;
  }
  .sc-number-wheel-strip {
    display: grid;
    transform: translateY(0em);
    will-change: transform;
  }
  .sc-number-wheel-strip span {
    height: 1em;
    line-height: 1em;
  }
</style>
...
<script>
  (function () {
    ...
    document.querySelectorAll(".sc-number-wheel").forEach((el) => {
      if (el.dataset.hfNumberWheelReady === "1") return;
      el.style.setProperty("--sc-wheel-gap", gap);
      el.style.setProperty("--sc-wheel-color", tone);
      ...
      for (const char of value) {
        ...
        frame.className = "sc-number-wheel-digit";
        const strip = document.createElement("span");
        strip.className = "sc-number-wheel-strip";
        strip.style.setProperty("--sc-number-target-y", `${-stops}em`);
...
<!--
  ...
  tl.to(".sc-number-wheel-strip", {
    y: (_, strip) => strip.style.getPropertyValue("--sc-number-target-y"),
    duration: 1.1,
    ease: "power3.out",
    stagger: 0.05,
  }, startTime);
...
<InstallCommand command="npx smashcut add number-wheel" item="number-wheel" />
