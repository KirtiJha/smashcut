...
<!--
  ...
  Paste the markup, CSS and script once, then animate .sc-simulated-cursor
  with x/y values in your paused GSAP timeline. Trigger the click pulse by
  scaling/fading .sc-simulated-cursor-pulse.
  ...
  driving the pointer position and the ring scale and fade.
    ...
    - tone (light | dark | accent, default light): pointer fill, outline and
      ...
      screens, dark flips both for light screens, accent rides --sc-ui-accent.
...
<div
  ...
  data-composition-variables='[
    ...
    {"id": "tone", "type": "enum", "role": "style", "label": "Tone", "description": "Pointer fill and click ring colour: light reads on dark screens, dark reads on light ones, accent rides --sc-ui-accent.", "default": "light", "options": [{"value": "light", "label": "Light"}, {"value": "dark", "label": "Dark"}, {"value": "accent", "label": "Accent"}]},
...
>
  <div class="sc-simulated-cursor-pulse"></div>
...
<style>
  .sc-simulated-cursor {
    ...
    left: 0;
    top: 0;
    width: calc(44px * var(--sc-cursor-scale, 1));
    height: calc(44px * var(--sc-cursor-scale, 1));
    transform: translate3d(var(--sc-cursor-x, 0px), var(--sc-cursor-y, 0px), 0);
    pointer-events: none;
    z-index: 999;
    filter: drop-shadow(0 8px 18px rgba(0, 0, 0, 0.24));
  }
  .sc-simulated-cursor svg {
    width: 100%;
    height: 100%;
  }
  .sc-simulated-cursor svg path {
    fill: var(--sc-cursor-fill, #ffffff);
    stroke: var(--sc-cursor-edge, rgba(0, 0, 0, 0.45));
  }
  .sc-simulated-cursor-pulse {
    display: var(--sc-cursor-ring-display, block);
    position: absolute;
    left: calc((20px - var(--sc-cursor-ring-size, 22px) / 2) * var(--sc-cursor-scale, 1));
    top: calc((20px - var(--sc-cursor-ring-size, 22px) / 2) * var(--sc-cursor-scale, 1));
    width: calc(var(--sc-cursor-ring-size, 22px) * var(--sc-cursor-scale, 1));
    height: calc(var(--sc-cursor-ring-size, 22px) * var(--sc-cursor-scale, 1));
    border: calc(var(--sc-cursor-ring-width, 2px) * var(--sc-cursor-scale, 1)) solid
      var(--sc-cursor-ring, rgba(255, 255, 255, 0.85));
    border-radius: 999px;
    opacity: 0;
    transform: scale(0.2);
  }
...
<script>
  (function () {
    ...
    var tones = {
      ...
      accent: {
        fill: "var(--sc-ui-accent, #3ce6ac)",
        edge: "rgba(0, 0, 0, 0.45)",
        ring: "var(--sc-ui-accent, #3ce6ac)",
      },
    };
    var pulses = {
    ...
    var roots = document.querySelectorAll(".sc-simulated-cursor");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--sc-cursor-scale", String(size / 44));
      root.style.setProperty("--sc-cursor-fill", tone.fill);
      root.style.setProperty("--sc-cursor-edge", tone.edge);
      root.style.setProperty("--sc-cursor-ring", tone.ring);
      root.style.setProperty("--sc-cursor-ring-size", pulse.size);
      root.style.setProperty("--sc-cursor-ring-width", pulse.width);
      root.style.setProperty("--sc-cursor-ring-display", pulse.display);
    }
  })();
</script>
...
<!--
  ...
  tl.to(".sc-simulated-cursor", { "--sc-cursor-x": "760px", "--sc-cursor-y": "420px", duration: 0.8, ease: "power2.inOut" }, startTime);
  tl.fromTo(".sc-simulated-cursor-pulse", { opacity: 0.8, scale: 0.2 }, { opacity: 0, scale: 2.4, duration: 0.45, ease: "power2.out" }, startTime + 0.8);
-->
```
...
<InstallCommand command="npx smashcut add simulated-cursor" item="simulated-cursor" />
...
| `tone` | `light` | `light`, `dark`, `accent` | Pointer fill and click ring colour: light reads on dark screens, dark reads on light ones, accent rides --sc-ui-accent. |
