...
<InstallCommand command="npx smashcut add chat-message" item="chat-message" />
...
| `cardTitle` | `SmashCut — Write HTML, render pixel-perfect video` | string | Link-card title (Link card type only). |
...
<div
  ...
  data-variable-values='{"type":"sent","text":"wait look 👀","cardTitle":"SmashCut — Write HTML, render pixel-perfect video","cardDomain":"hyperframes.heygen.com","tail":"on","delivered":"on"}'
></div>
```
