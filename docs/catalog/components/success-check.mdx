...
<!--
  Success Check - transitions.dev-inspired primitive for SmashCut.
...
<div
  class="sc-transition-success-check"
  aria-label="Success"
...
<style>
  .sc-transition-success-check {
    display: grid;
    place-items: center;
    width: var(--sc-check-size, 82px);
    height: var(--sc-check-size, 82px);
    border-radius: 999px;
    background: var(--sc-check-halo, #dcfce7);
    color: var(--sc-check-ink, #16a34a);
  }
  .sc-transition-success-check svg {
    width: var(--sc-check-mark-size, 64px);
    height: var(--sc-check-mark-size, 64px);
    fill: none;
    stroke: currentColor;
    stroke-width: 5;
    stroke-linecap: round;
    stroke-linejoin: round;
    filter: blur(var(--sc-check-blur, 0px));
    transform: rotate(calc(var(--sc-check-rotate, 0deg) * var(--sc-check-spin, 1)))
      scale(var(--sc-check-scale, 1));
  }
  .sc-transition-success-check circle {
    opacity: var(--sc-check-ring-opacity, 0.28);
  }
  .sc-transition-success-check path {
    stroke-dasharray: 36;
    stroke-dashoffset: var(--sc-check-dash, 0);
  }
</style>
...
<script>
  (function () {
    ...
    var roots = document.querySelectorAll(".sc-transition-success-check");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--sc-check-halo", tint.halo);
      roots[i].style.setProperty("--sc-check-ink", tint.ink);
      roots[i].style.setProperty("--sc-check-size", size.box);
      roots[i].style.setProperty("--sc-check-mark-size", size.mark);
      roots[i].style.setProperty("--sc-check-ring-opacity", ring);
      roots[i].style.setProperty("--sc-check-spin", spin);
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.sc-transition-success-check svg', { opacity: 0, '--sc-check-scale': 0.72, '--sc-check-rotate': '-18deg', '--sc-check-blur': '7px' }, { opacity: 1, '--sc-check-scale': 1, '--sc-check-rotate': '0deg', '--sc-check-blur': '0px', duration: 0.34, ease: 'back.out(1.8)' }, startTime); tl.fromTo('.sc-transition-success-check path', { '--sc-check-dash': 36 }, { '--sc-check-dash': 0, duration: 0.42, ease: 'power2.out' }, startTime + 0.12);
-->
```
...
<InstallCommand command="npx smashcut add success-check" item="success-check" />
