...
<!doctype html>
<!--
  touch-indicator -- SmashCut video primitive (pointers / interaction / demonstrate)
  ...
  <template> contents (see skills/smashcut-core/references/sub-compositions.md),
  ...
  COMPILED FROM: smashcut-corpus-data/mobile/gesture-actor-spec.md (the
  ...
  ripple (EDIT ZONE: .sc-touch-indicator__contact, unchanged from the
  original spec) plus a quiet radial pulse centered on the root
  (.sc-touch-indicator__pulse, new). Both are token-painted and low-opacity
    ...
    <template>
      <style>
        ...
        .sc-touch-indicator__actor {
          ...
          --sc-gesture-size: clamp(46px, 5cqw, 111px);
          position: absolute;
          left: 0;
          top: 0;
          width: var(--sc-gesture-size);
          height: var(--sc-gesture-size);
          opacity: 0;
          visibility: hidden;
          pointer-events: none;
          will-change: transform, opacity;
      ...
      >
        <div class="sc-touch-indicator__pulse"></div>
        <div class="sc-touch-indicator__actor">
          <div class="sc-touch-indicator__contact"></div>
          <div class="sc-touch-indicator__core"></div>
        </div>
      </div>
      ...
      <script>
        (function () {
          ...
          function buildTouchIndicator(tl, root, { at = 0, elasticity = 1 } = {}) {
            ...
            // window.__scVariablesByComp, scoped by composition id. Reading
            // window.__scVariables directly here would silently ignore every
            // host override -- window.__smashcut.getVariables() is the
            ...
            const pulse = root.querySelector(".sc-touch-indicator__pulse");
            const actor = root.querySelector(".sc-touch-indicator__actor");
            const core = root.querySelector(".sc-touch-indicator__core");
            const contact = root.querySelector(".sc-touch-indicator__contact");
            if (vars.polarity === "dark") actor.classList.add("sc-touch-indicator__actor--dark");
...
<InstallCommand command="npx smashcut add touch-indicator" item="touch-indicator" />
