...
Three.js is a first-party seek-safe runtime. The adapter publishes SmashCut
time as `window.__scThreeTime` and dispatches an `sc-seek` event on each seek, so
...
<DocsVideo
  title="SmashCut video: Camera Orbit"
...
<DocsVideo
  title="SmashCut video: Capstone Region Depth"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/capstone-region-depth.mp4#t=0.1"
  loop
/>
