...
SmashCut owns media *playback*. A sibling media pipeline resolves everything
...
<Note>
  ...
  - A cutout that reveals into view goes inside a non-timed `<div>`, and the
    *wrapper* is what gets animated. SmashCut owns clip visibility, so
...
`smashcut lint` also flags the asset at info level (`hevc_preview_codec`) so
you know a proxy is in play. Proxying is optional. Pass `--no-proxy` per
command, or set `media.autoProxy: false` in `smashcut.json` project-wide.
...
<DocsVideo
  title="SmashCut video: Proxy Footage"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/proxy-footage.mp4#t=0.1"
  loop
/>
