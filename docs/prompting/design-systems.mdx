...
A SmashCut project can carry a design spec: `frame.md`. Its frontmatter holds the machine-readable brand — exact hex values, font families, weight relationships. The prose below the frontmatter carries intent and the brand's Do's and Don'ts.
...
<Note>
**Capstone thread** — the entire [Level 7 film](/prompting/capstone) obeys a supplied `frame.md`: the real SmashCut brand. The closing lockup is where the system shows most. ABC Solar Display carries the display type, and the mint→cyan CTA gradient makes its single sanctioned appearance (cut from the film, below).
</Note>
...
<DocsVideo
  title="SmashCut video: Capstone Region Render"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/capstone-region-render.mp4#t=0.1"
  loop
/>
