...
description: "What to say to migrate an existing Remotion (React) composition's source into SmashCut HTML — and what to expect the agent to refuse."
---
...
Not a new video — a **translation**. The [`/remotion-to-smashcut`](/prompting/overview) workflow reads an existing Remotion (React) composition's source and rewrites it as SmashCut HTML: the same frames, the same timing, the same visual output, running on GSAP instead of React's frame-callback model.
...
> /remotion-to-smashcut Port the composition at `src/HeroReveal.tsx` (Remotion project root: `.`) to SmashCut. Keep the same duration, fps, and dimensions. Write `TRANSLATION_NOTES.md` for anything that doesn't translate cleanly.
...
The skill lints the source before translating anything, and **refuses** rather than approximates when it hits a pattern SmashCut' seek-driven model can't represent deterministically:

- **State-driven animation** — `useState`, `useReducer`, or `useEffect`/`useLayoutEffect` with real dependencies. SmashCut seeks to an arbitrary frame and expects the same pixels every time; a state machine that reacts to its own history can't guarantee that. The skill recommends a runtime-interop pattern instead of silently producing a translation that "looks right" but isn't frame-accurate.
...
> After translating, render both the Remotion original and the SmashCut version and report the SSIM diff.
...
- ❌ `/remotion-to-smashcut make something like my Remotion intro`
