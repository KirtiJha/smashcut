...
- ✅ `/slideshow ... I'll present it with smashcut present` — or, if you actually need a rendered video, use `/music-to-video` (beat-synced) or `/general-video`.
...
<Tip>
  Both prompts here are unnarrated. `/music-to-video` has no TTS by design; if you want a spoken voice-over instead of a music bed, that's a different workflow (see the router in `/smashcut`). For the six-part skeleton these prompts share, see [Prompt anatomy](/prompting/anatomy); for adjectives that map to eases and transitions, [Vocabulary](/prompting/vocabulary).
</Tip>

<Note>
**Capstone thread** — the [Level 7 film](/prompting/capstone)'s Rhythm region cuts media cards onto a real analyzed beat grid from `smashcut beats` — the film's only sanctioned hard cuts, every one on a detected beat while the camera keeps traveling (cut from the film, below).
</Note>
...
<DocsVideo
  title="SmashCut video: Capstone Region Rhythm"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/capstone-region-rhythm.mp4#t=0.1"
  loop
/>
