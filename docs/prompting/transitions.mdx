...
<Note>
  The catalog pages linked here are standalone demos of each effect. In a real multi-scene build the agent wires the same effects through the `@smashcut/shader-transitions` package (`HyperShader.init`) — you never need to say that; naming the transition is enough.
</Note>
...
<DocsVideo
  title="SmashCut video: Validate Transitions"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/validate-transitions.mp4#t=0.1"
  loop
/>
