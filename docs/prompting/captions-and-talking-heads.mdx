...
<DocsVideo
  title="SmashCut video: Captions Anchor Rail"
...
<Tip>
  For the beat-timestamped skeleton these prompts share, see [Prompt anatomy](/prompting/anatomy); for adjectives that map to motion and emphasis settings, [Vocabulary](/prompting/vocabulary). To build a video from scratch instead of dressing existing footage, start at the router in `/smashcut`.
</Tip>
