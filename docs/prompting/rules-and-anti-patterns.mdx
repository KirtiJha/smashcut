...
`smashcut check` flags a text block covered by another element as `text_occluded`, and
...
**Don't skip the slash command.** Without `/smashcut`, the agent guesses at HTML video conventions instead of loading the framework's actual rules.
- ❌ `make me a video of...`
- ✅ `/smashcut make me a video of...`
