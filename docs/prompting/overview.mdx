---
title: Prompt Guide
description: "How to prompt AI agents to author SmashCut videos — setup, the two prompt shapes, and the map of this guide."
---

import { DocsVideo } from "/snippets/docs-video.jsx";

<DocsVideo
  title="SmashCut video: Capstone Timeline Default"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/capstone-timeline-default-v2.mp4#t=0.1"
  loop
/>
*By the end of this guide, you can build this with a prompt.*

SmashCut is built for AI agents — compositions are plain HTML, the CLI is non-interactive, and the framework ships [skills](https://github.com/vercel-labs/skills) that teach agents the patterns docs alone don't cover. This guide shows how to prompt agents effectively once skills are installed — the vocabulary that changes output, the iteration patterns that save time, and the rules that prevent breakage.

<Note>
  **Before you prompt**, have three things in place: the skills installed (below), a scaffolded project (`npx smashcut init my-video`), and the live preview running (`npx smashcut preview`) so you can judge each render the moment it lands. Prompting without the preview open turns every iteration into a blind guess.
</Note>
...
The installer shows a picker. Select the **core skills** below — every project needs them. In Claude Code, restart the session after installing; the skills register as **slash commands**. Start at `/smashcut`: it orients you to the whole surface and routes "make me a video" requests to the right workflow.

**Core skills — install all of these**

| Slash command           | What it loads                                                              |
| ----------------------- | -------------------------------------------------------------------------- |
| `/smashcut`          | **Read first.** The entry skill — capability map + video router; sends "make me a video" intent to the right workflow |
| `/smashcut-core`     | Composition contract — HTML structure, `data-*` attributes, clips, tracks  |
| `/smashcut-animation`| All animation — motion rules, scene blueprints, transitions, and the runtime adapters (GSAP, Lottie, Three.js, Anime.js, CSS, WAAPI, TypeGPU) |
| `/smashcut-creative` | Creative direction — design spec, palettes, typography, narration, beats   |
| `/smashcut-cli`      | Dev-loop CLI — `init`, `lint`, `check`, `preview`, `render`, `doctor`    |
| `/media-use`            | Media OS — TTS voiceover (`tts`), `transcribe`, `remove-background`, plus BGM / SFX / image resolution |
| `/smashcut-registry` | Block and component installation via `smashcut add`                     |
| `/smashcut-keyframes`| Seek-safe keyframe authoring across runtimes, plus `smashcut keyframes` diagnostics |
...
**Optional workflows — add the ones that match your inputs** (`/smashcut` routes to whichever you've installed)
...
Claude Design uses a different setup. Download [`claude-design-smashcut.md`](https://github.com/heygen-com/hyperframes/blob/main/docs/guides/claude-design-smashcut.md) from GitHub (click the ↓ button), then **attach it to your chat** (don't paste the URL — file attachments produce better output):
...
Claude Design produces a valid first draft (brand identity, scene content, animations, transitions). Download the ZIP and refine in any AI coding agent with `npx smashcut preview` running. See the [Claude Design guide](/guides/design-tools) for the full workflow.



## The two prompt shapes

Most successful SmashCut prompts fall into one of two shapes.
...
> Using `/smashcut`, create a 10-second product intro with a fade-in title over a dark background and subtle background music.
...
**Warm start — turn context into a video.** You give the agent something to work with — a URL, a doc, a CSV, a transcript — and ask it to synthesize that into a video. This is where SmashCut shines because the agent does the research/summarization step *and* the production step in one flow.

> Take a look at this GitHub repo https://github.com/heygen-com/hyperframes and explain its uses and architecture to me using `/smashcut`.

> Turn this CSV into an animated bar chart race using `/smashcut`.
...
| **Automation or companion?** | `automation` — the matched workflow's pipeline executes the brief end to end · `companion` — build it together in `/general-video` with every SmashCut capability on the table |
...
1. `npx smashcut init my-video` — scaffold a project (skills install automatically)
2. Open the project in Claude Code (or Cursor / Codex)
3. Prompt with `/smashcut` and one of the shapes above
4. `npx smashcut preview` — watch in the browser as the agent edits
5. Iterate with small targeted prompts
6. `npx smashcut lint && npx smashcut check` — the gate: structure, runtime errors, layout collisions, motion, and contrast. Both must pass before you render
7. `npx smashcut render --output final.mp4` when you're happy
...
<DocsVideo
  title="SmashCut video: Example Product Launch"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/example-product-launch-v2.mp4#t=0.1"
  loop
/>
...
<DocsVideo
  title="SmashCut video: Example Stat Countup"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/example-stat-countup.mp4#t=0.1"
  loop
/>
*Rendered from the prompt above, unedited.*
...
<DocsVideo
  title="SmashCut video: Recreate Globe Oneshot"
  src="https://static.heygen.ai/smashcut-oss/docs/images/prompting/recreate-globe-oneshot-v2.mp4#t=0.1"
  loop
/>
