...
description: "Deploy distributed SmashCut rendering to Cloud Run, Cloud Workflows, and Google Cloud Storage."
---
...
[SmashCut Cloud](/deploy/cloud). For a small preview application and render
...
- `@smashcut/gcp-cloud-run` installed alongside the CLI

The CLI can build the renderer automatically when it runs from a SmashCut
...
resulting bucket, service URL, and workflow name in `~/.smashcut/`.
...
```bash
smashcut cloudrun render ./my-project \
  --width 1920 \
  --height 1080 \
  --wait
```
...
```bash
smashcut cloudrun render ./card-template \
  --width 1920 \
  --height 1080 \
  --variables '{"name":"Ada"}' \
  --wait
...
```bash
smashcut cloudrun render-batch ./card-template \
...
```bash
smashcut cloudrun progress <execution-name>
...
`@smashcut/gcp-cloud-run/sdk` exposes `deploySite`, `renderToCloudRun`, and
