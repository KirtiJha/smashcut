...
description: "Choose the smallest SmashCut rendering surface for local work, an application backend, managed cloud, or infrastructure you operate."
---

Every rendering path starts with the same SmashCut project. The difference is
...
| Submit a render without managing infrastructure | **SmashCut Cloud** | Nothing beyond the request and result |
...
`@smashcut/producer` is the complete programmable render pipeline. It is the
...
`@smashcut/engine` opens a seekable composition and captures exact frames.
...
## No infrastructure: use SmashCut Cloud
...
```bash
npx smashcut cloud render ./project --output video.mp4
```
