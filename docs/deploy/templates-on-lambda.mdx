...
description: "Render one SmashCut composition with different variable values, individually or from a JSONL batch."
---

A SmashCut template is a composition with declared variables. The same
...
```bash
smashcut lambda render ./my-template \
  --width 1920 \
  --height 1080 \
  --variables '{"title":"Hello Ada","accent":"#ff4d67"}' \
...
```bash
smashcut lambda progress <render-id>
```
...
```bash
smashcut lambda sites create ./my-template
```
...
SmashCut checks this before starting the execution.
...
SmashCut variables fill the role that `inputProps` serves in a Remotion
Lambda render:

| Remotion pattern        | SmashCut equivalent                                             |
| ----------------------- | ------------------------------------------------------------------ |
...
For a backend service, `@smashcut/aws-lambda/sdk` exposes `deploySite`,
