---
name: smashcut-handoff
description: |
  Produce a SmashCut-valid HTML composition — paused GSAP timeline, data
  attributes, scene structure — that any AI coding agent can immediately
  refine with `npx smashcut lint` and `npx smashcut preview`. Use when
  the brief mentions "video", "reel", "motion graphic", "title card",
  "animated explainer", or pairs Open Design with SmashCut for export.
triggers:
  - "smashcut"
  - "video"
  - "reel"
...
od:
  ...
  example_prompt: "Design a 15-second Instagram reel announcing dark mode for Taskflow (#6C5CE7). Output as a SmashCut composition I can render locally."
---

# SmashCut Handoff — for Open Design

> **Drop this file at `skills/smashcut-handoff/SKILL.md` inside your local
...
[SmashCut](https://github.com/heygen-com/hyperframes) composition — plain
HTML + CSS + a paused GSAP timeline. The CLI (`npx smashcut render`, run
from the project directory) turns the HTML into an MP4. You author the HTML;
the user runs the render locally.

**SmashCut replaces the default video-artifact workflow.** Do NOT emit a
...
GSAP only. Treat the [`claude-design-smashcut.md`](https://github.com/heygen-com/hyperframes/blob/main/docs/guides/claude-design-smashcut.md)
companion document as the **upstream spec for SmashCut structural rules** —
...
3. **Any AI coding agent** (Claude Code, Codex, Cursor, …) — `npx smashcut
   lint`, `npx smashcut preview`, then iterate timing, eases, shader
   choices, pacing
...
- Structural validity (passes `npx smashcut lint` with zero errors)
...
These are SmashCut-structural and non-negotiable. Open Design's
...
3. **Scenes are children of `#stage`.** Each scene is `<div class="scene
   ...
   - `data-track-index="0"` (SmashCut uses tracks for layering; visual
...
4. **GSAP timeline registered paused.** A single timeline created with
   ...
   SmashCut engine drives the playhead.
...
7. **Shader transitions ONLY at scene boundaries**, and at most 2–3 in the
   whole video. Use SmashCut' built-in shader blocks
...
Use SmashCut' built-in shader blocks at scene boundaries. Pick by mood:
...
<head>
  ...
  <style>
    ...
    .scene.clip { /* SmashCut toggles visibility per playhead */ }
...
<script>
  ...
  // Forward SmashCut preview tokens (frame=, paused=, …) into the iframe
...
```bash
npx smashcut lint        # should pass with zero errors
npx smashcut preview     # opens the studio
npx smashcut render      # writes MP4
```

---

## Anti-AI-slop blacklist (SmashCut-specific)
...
- **No wall-clock JS animations** — `setTimeout`, `setInterval`,
  ...
  Lottie are supported via [SmashCut' Frame Adapter](https://hyperframes.heygen.com/concepts/frame-adapters)
...
[`claude-design-smashcut.md`](https://github.com/heygen-com/hyperframes/blob/main/docs/guides/claude-design-smashcut.md)
...
- Audio-reactive animation (`sc-seek` + `window.__scAudio`)
- Captions / TTS integration
- The `smashcut add` registry of blocks and components
