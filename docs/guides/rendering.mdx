...
<DocsVideo
  ...
  src="https://static.heygen.ai/smashcut-oss/docs/images/showcase/render-loop-demo-v2.mp4"
  poster="https://static.heygen.ai/smashcut-oss/docs/images/showcase/render-loop-demo-v2.jpg"
/>
...
```bash
npx smashcut render --output final.mp4
```

If you omit `--output`, SmashCut writes the result under `renders/`.
...
# ProRes editing file
npx smashcut render --format mov --output master.mov

# Short looping preview
npx smashcut render --format gif --fps 15 --output preview.gif

# RGBA frames in a directory
npx smashcut render --format png-sequence --output frames
```
...
```bash
npx smashcut render --format webm --output overlay.webm
npx smashcut render --format mov --output overlay.mov
```
...
source is local, and run `npx smashcut check`. Disable proxying only when the
...
npx smashcut render --quality draft --output review.mp4

# Larger final master
npx smashcut render --quality high --output master.mp4

# Explicit frame rate
npx smashcut render --fps 60 --output final-60fps.mp4
```
...
```bash
npx smashcut render --docker --output final.mp4
```
...
For several variable-driven versions, use batch rendering. For remote infrastructure, use SmashCut cloud, AWS Lambda, or Google Cloud Run.
...
```json
{ "smashcut_renderer": "smashcut", "smashcut_version": "0.7.107" }
```
...
uppercases tag names on read, so a `.webm` reports `SMASHCUT_RENDERER`.
...
<Note>
  ...
  command: a tag that is present means the file *claims* to be SmashCut output, not that
  SmashCut wrote it. A tag that is absent means just as little, because re-encoding, remuxing, or
