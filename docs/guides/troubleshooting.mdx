...
<Steps>
  ...
  <Step title="Inspect the visible result">
    ...
    ```bash
    npx smashcut snapshot --at 0,3,8
    ```
  </Step>
...
Verify with `ffmpeg -version`, then rerun `npx smashcut doctor`.
...
SmashCut normally creates and uses a compatible preview proxy. If the frame remains black:

1. confirm automatic proxying is not disabled;
2. run `npx smashcut doctor`;
3. run `npx smashcut lint --verbose` to identify affected files;
...
SmashCut owns media playback. Use timing and media attributes to describe when the file should play.
...
`npx smashcut lint` normally catches this.
...
Run `npx smashcut benchmark` when tuning worker settings.
...
Then search [GitHub issues](https://github.com/KirtiJha/smashcut/issues) or open a report with:

- the exact error;
- steps to reproduce;
- SmashCut version and operating system;
