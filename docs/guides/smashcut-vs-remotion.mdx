---
title: "SmashCut or Remotion?"
description: "The same three-second title card written in Remotion React and in SmashCut HTML, plus an honest read on which tool fits your project."
---
...
Remotion asks for a React component. SmashCut asks for an HTML file.
...
**SmashCut.** The markup declares when things are on screen. A GSAP timeline
...
reason about in your head. The SmashCut one is a page you can open in a
...
| License | [Apache 2.0](https://github.com/KirtiJha/smashcut/blob/main/LICENSE) | Its own license — free for individuals and companies up to three people, paid above that. Check the [current terms](https://www.remotion.pro/license) |

The seeking model is the interesting one. Because SmashCut drives the
...
[SmashCut Studio](/studio) edits the same DOM the renderer captures. Remotion
...
Both render on AWS Lambda. SmashCut also renders locally, on HeyGen's hosted
...
SmashCut asks you to follow rules — paused timeline, no wall clocks, no
unseeded randomness — and breaks quietly if you don't.

## What SmashCut does better
...
```bash
npx skills add ./skills --skill remotion-to-smashcut
```
...
- [How a SmashCut composition works](/concepts/compositions)
