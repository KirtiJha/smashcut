---
title: "Send to SmashCut"
description: "Author a Claude Design as a single-file SmashCut composition for the \"Send to SmashCut\" URL import."
---

# Claude Design → "Send to SmashCut" (Single-File Import)

> This guide is **fully self-contained**: everything needed to author a valid "Send to SmashCut" composition is inline here. Do not rely on fetching any other document — produce the composition from this guide alone.

Your medium is a **SmashCut composition**: plain HTML + CSS + a paused GSAP timeline. When the user clicks **"Send to SmashCut"**, that composition is imported into a hosted HeyGen project, previewed in SmashCut, and rendered to MP4 in the cloud. You produce a **valid first draft** — a SmashCut motion-design agent enhances it afterward (sound, media, polish). You are not producing the final video.

---

## How this path differs from download-ZIP

There are two ways a Claude Design composition can reach SmashCut. **This guide is only about "Send to SmashCut."**

| | Download-ZIP flow | **Send to SmashCut (this guide)** |
| --- | --- | --- |
...
| Next step | a coding agent polishes locally, renders via CLI | **enhanced inside SmashCut; rendered in the cloud** |
...
1. **You (Claude Design)** — author a valid SmashCut composition as a single self-contained HTML.
2. **Send to SmashCut** — one click. The importer fetches your HTML, validates it, and creates a hosted HeyGen project. **Import is free.**
3. **Enhance in SmashCut** — a motion-design agent adds what your export can't: sound effects, background music, and (later) HeyGen media. **Enhance turns are free.**
...
The SmashCut runtime, GSAP, and shader-transitions are loaded from the jsdelivr CDN (`cdn.jsdelivr.net/npm/@smashcut/...`, `gsap@…`) — the render pipeline fetches those at render time by design. GSAP is version-pinned (`gsap@3.14.2`); the `@smashcut` packages are served unversioned (latest published runtime), matching what a real import ships. Do **not** inline the runtime.
...
<html lang="en" style="overflow:hidden; margin:0">
  <head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/@smashcut/core/dist/smashcut.runtime.iife.js"></script>
...
Your export is **silent** and visual-only. Sending it to SmashCut levels it up with things Claude Design can't add:
...
## Self-review checklist (run before "Send to SmashCut")
