...
description: "Use the hosted SmashCut MCP connector to create and render without installing the local CLI."
---

import { DocsVideo } from "/snippets/docs-video.jsx";

SmashCut MCP is a hosted connector for supported AI chat products. It lets
the chat call SmashCut creation and rendering tools through your HeyGen
...
<DocsVideo
  ...
  src="https://static.heygen.ai/smashcut-oss/docs/images/showcase/mcp-flow-demo-v2.mp4"
  poster="https://static.heygen.ai/smashcut-oss/docs/images/showcase/mcp-flow-demo-v2.jpg"
/>
...
```text
https://mcp.heygen.com/mcp/smashcut/
```

You need a HeyGen account and access to the SmashCut connector.
...
<Tabs>
  <Tab title="Claude">
    ...
    4. In a new chat, open the **+** menu, choose **Connectors**, and enable SmashCut.
  ...
  <Tab title="ChatGPT">
    If SmashCut appears in **Settings → Apps**, choose **Connect** and complete HeyGen authorization.
...
Host products change their connector interfaces independently. If a menu or button is missing, check that host’s current plan and workspace permissions before treating it as a SmashCut failure.
...
billing can change independently of SmashCut, so confirm them in the account
...
2. disconnect and reconnect SmashCut;
...
Use [Share feedback](/guides/feedback) when the failure belongs to SmashCut. Use the host product’s support when the connector cannot be installed or invoked at all.

<Warning>
  Only connect the production SmashCut server shown on this page. As with any external connector,
