...
```bash
npx smashcut render --resolution 4k --output final-4k.mp4
```

SmashCut keeps the composition's layout unchanged and captures it at a higher device-pixel ratio. A 1920×1080 composition becomes a 3840×2160 file.
...
```bash
npx smashcut init my-video --resolution 4k
```
...
SmashCut checks these conditions before opening the browser. Render at the composition's native size and resize separately when a combination is unsupported.
