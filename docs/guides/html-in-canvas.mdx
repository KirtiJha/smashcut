...
<DocsVideo
  ...
  src="https://static.heygen.ai/smashcut-oss/docs/images/showcase/html-in-canvas-demo-v2.mp4"
  poster="https://static.heygen.ai/smashcut-oss/docs/images/showcase/html-in-canvas-demo-v2.jpg"
/>
...
```bash
npx smashcut add vfx-iphone-device
```
...
```bash
npx smashcut add html-in-canvas
```
...
SmashCut enables `CanvasDrawElement` when it launches Chrome and checks whether the API is actually available. The render engine can fall back to its normal capture path when the fast-capture API is unavailable or unsafe for a composition.

To refresh the browser used by the CLI:

```bash
npx smashcut browser ensure --force
```
...
The browser's paint-record path does not reproduce every compositor effect. Filters, backdrop filters, some 3D transforms, masks, blend modes, and similar effects can force SmashCut back to the normal capture path.
...
npx smashcut lint
npx smashcut check
npx smashcut render --output html-in-canvas.mp4
```
