...
A SmashCut slideshow stays interactive. The result is a live, navigable deck—not a linear MP4.


<DocsVideo
  title="A real deck being navigated — fragments, a branch, and presenter mode"
  src="https://static.heygen.ai/smashcut-oss/docs/images/showcase/slideshow-demo-v1.mp4"
  poster="https://static.heygen.ai/smashcut-oss/docs/images/showcase/slideshow-demo-v1.jpg"
/>
...
```text
Using /smashcut, turn this product strategy outline into a presentation for
...
<Warning>
  Do not run `smashcut render` on a deck. It does not fail — a deck has no
  ...
  Use `smashcut present` for the live deck, or `smashcut snapshot` for
  per-slide stills.
</Warning>
...
```bash
npx smashcut present <project-directory>
```
