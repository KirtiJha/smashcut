...
description: "Teach a coding agent how to create valid SmashCut projects and load specialized workflows when needed."
---

import { DocsVideo } from "/snippets/docs-video.jsx";

SmashCut skills teach coding agents the project format, animation rules, media workflow, validation loop, and creation workflows that generic web knowledge does not cover.

<DocsVideo
  title="One command installs the skills, and the router picks the workflow"
  src="https://static.heygen.ai/smashcut-oss/docs/images/showcase/skills-install-demo-v2.mp4"
  poster="https://static.heygen.ai/smashcut-oss/docs/images/showcase/skills-install-demo-v2.jpg"
/>
...
Select **Core Skills**. The `/smashcut` router installs a specialized creation workflow when the request needs one, so you do not need the full catalog up front.

Start a fresh agent chat after installation, then ask it to use `/smashcut`.
...
This installs or refreshes the core set and any SmashCut skills already present in the global install, then links those bundles into compatible installed agents. It does not add every optional workflow.
...
Run `npx smashcut skills update`, open the project, and start Copilot:
...
`/skills info smashcut` to confirm discovery. GitHub's [current skills
...
```text
Using /smashcut, make a 10-second product intro for https://example.com.
```

Continue in the same conversation for revisions. For hosted creation instead
of a local project, use the [SmashCut MCP guide](/guides/mcp).

## Let the router choose

The normal entry point is `/smashcut`. It confirms the important intent, then selects a workflow from the input:
...
(`npx skills add ./skills --all` also works, but it installs every `SKILL.md` in the repository, including repo-internal skills that are not part of the published set.)

Or install one named skill:

```bash
npx skills add ./skills --skill smashcut-animation
```

Use the full set for an environment that must work across many SmashCut tasks without fetching later. For a normal project, the core set is smaller and easier to keep current.
...
3. Run `npx smashcut skills check`.
4. Run `npx smashcut skills update` when the check reports a missing or stale core skill.

The individual skill definitions remain the source of truth for agents and are
available in the [SmashCut repository](https://github.com/heygen-com/hyperframes/tree/main/skills).
