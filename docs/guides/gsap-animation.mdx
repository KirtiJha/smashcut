...
description: "Create a paused timeline that SmashCut can seek to any frame."
---

import { DocsVideo } from "/snippets/docs-video.jsx";

GSAP is the primary animation runtime for SmashCut compositions. You author the motion; SmashCut owns the playhead.


<DocsVideo
  title="GSAP keyframes on screen, and the ease curve being shaped in Studio"
  src="https://static.heygen.ai/smashcut-oss/docs/images/showcase/gsap-keyframes-demo-v2.mp4"
  poster="https://static.heygen.ai/smashcut-oss/docs/images/showcase/gsap-keyframes-demo-v1.jpg"
/>
...
6. Let SmashCut control clip visibility and media playback.

The normal timeline methods are `to()`, `from()`, `fromTo()`, and `set()`. GSAP supports many CSS properties, but the [SmashCut animation skill](https://github.com/heygen-com/hyperframes/tree/main/skills/smashcut-animation) contains the render-safe patterns used by agents.
...
This is the clearest choice for a complete composition. If the root omits `data-duration`, SmashCut can infer length from registered timelines and supported media after the page initializes.
...
Do not manually add the child timeline to the parent's GSAP timeline. SmashCut maps the parent playhead into the nested scene.
...
npx smashcut lint
npx smashcut check
npx smashcut snapshot --at 0,0.6,2.9
```
