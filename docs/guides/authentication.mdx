...
<Steps>
  <Step title="Sign in">
    ...
    ```bash
    npx smashcut auth login --api-key            # hidden-input prompt
    echo "$HEYGEN_API_KEY" | npx smashcut auth login --api-key   # from stdin
    ```
  </Step>
  <Step title="Confirm what's configured">
    ```bash
    npx smashcut auth status
    ```
...
2. `SMASHCUT_API_KEY` — alias, for parity with other tools
3. `~/.heygen/credentials` — written by `smashcut auth login` (or `heygen auth login`)
...
|------------|----------------|---------------------------|------------------|
| **Voice (TTS)** | HeyGen → ElevenLabs → Kokoro | `HEYGEN_API_KEY` → `SMASHCUT_API_KEY` → `~/.heygen` · then `ELEVENLABS_API_KEY` | Kokoro: `pip install kokoro-onnx soundfile` |
...
Run `npx smashcut doctor` to check which local dependencies are installed.
The media workflows run `smashcut auth status` before generation and tell
you which path they will use.

<Note>
  `npx smashcut tts` itself is the local Kokoro CLI. Hosted HeyGen and
...
`npx smashcut publish` works while signed out. It uploads the project and
prints a URL containing a claim token. Open that URL and authenticate in the web
app to claim the project.

Sign in with `npx smashcut auth login` before publishing when you want the CLI
...
- [Open the authentication command reference](/packages/cli#smashcut-auth)
- [Render with SmashCut Cloud](/deploy/cloud)
