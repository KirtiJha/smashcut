...
```bash
git clone https://github.com/YOUR_USERNAME/smashcut.git
cd smashcut
bun install
bun run build
```

SmashCut uses Bun for workspace operations. Do not add a `pnpm-lock.yaml` or replace `bun.lock`.
...
```bash
npx smashcut lint ./my-video
npx smashcut check ./my-video
```
...
Use [GitHub Issues](https://github.com/KirtiJha/smashcut/issues) for bugs and proposals. A useful bug report includes the smallest reproduction, the first exact error, `npx smashcut info`, and the operating system.

By contributing, you agree that your work is licensed under the [Apache 2.0 License](https://github.com/KirtiJha/smashcut/blob/main/LICENSE) and follows the [Code of Conduct](https://github.com/KirtiJha/smashcut/blob/main/CODE_OF_CONDUCT.md).
