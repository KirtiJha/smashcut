export const CatalogOverviewPlayer = ({ title, children }) => {
  ...
  const scenes = JSON.stringify([
    ...
    },
    {
      ...
      video:
        "https://static.heygen.ai/smashcut-oss/docs/images/catalog/blocks/vfx-iphone-device.mp4",
      mediaStart: 9.25,
    },
    {
  ...
  const bootstrap = [
    ...
    "#stage,smashcut-player{position:absolute;inset:0;display:block;width:100%;height:100%}",
    ...
    "async function show(index){var token=++request;var scene=scenes[index];try{var html=await compositionFor(scene);if(token!==request)return;var player=document.createElement('smashcut-player');player.setAttribute('srcdoc',html);player.setAttribute('controls','');player.setAttribute('muted','');player.addEventListener('ready',function(){player.seek(scene.mediaStart||0);if(!reduced)player.play()},{once:true});document.getElementById('stage').replaceChildren(player);document.getElementById('label').textContent=scene.section;document.getElementById('error').style.display='none'}catch(error){if(token===request){document.getElementById('label').textContent=scene.section;document.getElementById('error').style.display='grid'}}}",
