#!/usr/bin/env bash
# Real-AWS smoke + benchmark for the SmashCut Lambda adapter.
#
...
#   --stack-name <name>        (default: smashcut-lambda-smoke-<unique-run-id>)
...
SMOKE_RUN_ID="${SMASHCUT_SMOKE_RUN_ID:-$(hf_new_smoke_run_id)}"
STACK_NAME="${STACK_NAME:-smashcut-lambda-smoke-${SMOKE_RUN_ID}}"
...
Real-AWS smoke + benchmark for the SmashCut Lambda adapter. Builds the
...
Flags:
  ...
  --stack-name <name>           SAM stack name (default: smashcut-lambda-smoke-<unique-run-id>)
...
if ! (cd "$SAM_DIR" && sam deploy \
        ...
        --tags "SmashcutSmokeRun=$SMOKE_RUN_ID" \
        --parameter-overrides \
