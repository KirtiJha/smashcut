...
This directory ships a reference `Dockerfile.example` for adopters who want to run SmashCut distributed renders **outside AWS Lambda**. The image bakes Node 22 + `chrome-headless-shell` + `ffmpeg` + the producer source, and works on Kubernetes Jobs, Argo Workflows, Cloud Run Jobs, ECS Fargate, or plain `docker run`.
...
```bash
docker build -t smashcut-chunk-runner:local -f examples/k8s-jobs/Dockerfile.example .
```
...
```ts
import { plan, renderChunk, assemble } from "@smashcut/producer/distributed";
...
If you want AWS Lambda specifically, use `smashcut lambda deploy` instead — it ships a turnkey deployment. See [docs/deploy/aws-lambda.mdx](../../docs/deploy/aws-lambda.mdx).
