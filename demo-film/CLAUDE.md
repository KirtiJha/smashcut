# SmashCut Composition Project

## Skills — USE THESE FIRST

**Always invoke the relevant skill before writing or modifying compositions.** Skills encode framework-specific patterns (e.g., `window.__timelines` registration, `data-*` attribute semantics, shader-compatible CSS rules) that are NOT in generic web docs. Skipping them produces broken compositions.

**Doing anything with SmashCut?** Start at `/smashcut` — it tells you what SmashCut can do and which skill or workflow handles your intent (make a video, TTS / BGM, prep footage, author / animate, render, install blocks), confirms your brief up front (the intent layer), and routes every "make me a…" request (a video, a deck, a composition port) to the right workflow. Read it first, especially when there's no project context to orient you. The workflows it routes to:
...
- `/general-video` — fallback for any other video (title card, longer brand / sizzle reel, multi-scene montage, static loop, custom composition) and the home of **companion mode** — co-create with the full SmashCut toolbox; the original smashcut authoring flow, any length.

**Porting an existing composition?** `/remotion-to-smashcut` translates a Remotion (React) composition into SmashCut HTML — a source migration, separate from the creation workflows above.

The domain skills (`/smashcut-core`, `/smashcut-animation`, `/smashcut-keyframes`, `/smashcut-creative`, `/smashcut-cli`, `/media-use`, `/smashcut-audio`, `/smashcut-registry`, `/figma`) and the full capability map live inside `/smashcut` — it is the single source of truth for which skill handles which intent.
...
> **Tailwind v4 projects** (`smashcut init --tailwind`): see `/smashcut-core` → `references/tailwind.md`.

> **Skill missing or stale?** Run `npx smashcut skills update <name>` to install/refresh
> the specific skill you need (the `/smashcut` router does this automatically before
> entering a workflow), or bare `npx smashcut skills update` to refresh the core set plus
...
npx smashcut preview --background  # agent-safe persistent Studio preview
npx smashcut preview --status      # verify the persistent preview is listening
npx smashcut preview --stop        # stop it when review is finished
npm run check        # lint + runtime + layout + motion + contrast (one command)
npm run render       # render to MP4
npm run publish      # publish and get a shareable link
npx smashcut lint --verbose  # include info-level findings
npx smashcut lint --json     # machine-readable output for CI
npx smashcut docs <topic> # reference docs in terminal
```

> **Agents must use `npx smashcut preview --background` for Studio handoff.** Do not rely
...
> **Pinned CLI version.** These scripts pin an exact `smashcut@X.Y.Z` so this project re-renders identically over time. Weeks later that pin lags fixes shipped since. To move up: `npx smashcut@latest upgrade --project . --check` (shows the delta), then `npx smashcut@latest upgrade --project .` to rewrite the pins. Always unpinned — the pinned script re-runs the old version against itself.
